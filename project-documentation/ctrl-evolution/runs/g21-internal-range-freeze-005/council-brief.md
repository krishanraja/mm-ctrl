# G21 internal range freeze council brief v5

Run: `G21-INTERNAL-RANGE-FREEZE-005`
...
7. Judge implemented behaviour, not intent. Exact fixture verification, arbitrary-input safety, semantic authority and diagnostic quality are separate contracts.
...
- The runtime validator is total over unknown input. Sparse arrays, malformed nested values and unexpected exceptions must fail closed without widening authority or escaping validation.
- The canonical verifier separately requires the exact expected profile and frozen fixture bytes.
- Each accepted internal record must carry a code-issued semantic receipt. The receipt binds the complete record meaning and exact audience authority, including content, claims, lifecycle role and authority time.
- A single safe fictional novel variant is pre-issued as a positive runtime control. Property order may differ without changing meaning. Arbitrary novel prose, relabelled semantics and unissued receipts must fail.
- The in-process receipt registry is a deterministic canary stand-in, not production authentication. Production issuance would require a trusted server-verifiable boundary.
- External fictional evidence is bound as a complete record. Structural similarity cannot author new evidence.
...
- Evidence time, audience authority identifiers, authority type, audience and authority time are bound exactly.
- The lifecycle sequence is initial, contradicted and corrected. Its evidence identities and semantic lifecycle roles are bound.
...
- Consequential exceptions and final calls remain assigned to named human roles.
- The manifest freezes the complete repository-local TypeScript import closure for the diagnostic entry point, with compiler and runtime versions.
...
Criterion version: `human-agency:g21-internal-range-freeze-v5`
...
Criterion version: `epistemic-integrity:g21-internal-range-freeze-v5`

Pass only if assertion kind, subject, source capability, standing, depth, time, uncertainty, countercase and route specificity are earned. Structural validation, trusted semantic issuance and exact fixture verification must not masquerade as one another.

Hard veto `EPISTEMIC-RANGE-01`: fabricated reality, future evidence, unearned depth, incapable source typing, unsupported causation, newest-record-wins, counterfeit authority, answer-key leakage, semantic receipt substitution or synchronised fixture drift can pass the contract that claims to prevent it.

### Subject, Audience and Lifecycle Safety

Criterion version: `subject-audience-lifecycle-safety:g21-internal-range-freeze-v5`
...
Hard veto `SAFETY-RANGE-01`: authored evidence can be mistaken for real personhood or consent, source or subject capability can be relabelled, private material can widen audience, authority or semantic receipts can be forged, or malformed nested input can escape validation.

### Consequential Usefulness

Criterion version: `consequential-usefulness:g21-internal-range-freeze-v5`
...
Criterion version: `living-brain-integrity:g21-internal-range-freeze-v5`
...
Hard veto `BRAIN-RANGE-01`: correction erases still-valid evidence, lineage can be misdirected, broken current views pass, oracle data leaks, source plus oracle can drift together undetected, receipt identity can reverse meaning or stored context is represented as learned judgement.

### Human Comprehension and Access

Criterion version: `human-comprehension-and-access:g21-internal-range-freeze-v5`
...
Criterion version: `behavioural-and-implementation-reality:g21-internal-range-freeze-v5`
