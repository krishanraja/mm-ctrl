...
**CANONICAL_LEDGER_SYNC:** Pending. The verified Supabase target still reports `M-013` v1 `in_progress` and no `M-014`. The official connector accepted readback but refused the append because its current transaction is read-only. No partial write occurred. Git commit `fe90ce2a8ed3a0fcde19b2e20cb5524e1bef6729` is the complete redacted recovery point. When write access is restored, append `M-013` v2 `complete` for the verified G13 contract, divergence, synthesis and rendered proof, then open `M-014` v1 `in_progress` for founder cold review. Do not use or recover access from any credential exposed in conversation.
