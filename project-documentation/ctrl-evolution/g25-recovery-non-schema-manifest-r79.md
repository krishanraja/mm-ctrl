...
A schema squash is not the whole product. The captured production inventory has 183 active Edge Functions. At capture time, Git had source directories for 115 deployable functions plus `_shared`, and all 115 existed live; 68 additional live functions had no matching local source directory. Git now has 121 deployable function directories. The six later local-only routes are the two portable-Brain routes, `measure-standard`, and the three R115 candidate-standard routes; their repository presence does not retroactively claim a production deployment. Production also has eight installed extensions, 15 scheduled jobs, five Storage buckets, two publications covering ten tables, 31 roles with 19 memberships and two named Vault entries.
