...
The isolated verifier will prove:

- all 15 ordinary-role routes are denied;
- service and owner access remain;
- all 15 function digests remain exact;
- all 23 enabled trigger attachments remain;
- the internal `handle_new_user` caller remains.

Actual trigger behavior still needs a safe runtime smoke before production consideration. Catalog presence is not runtime proof.
