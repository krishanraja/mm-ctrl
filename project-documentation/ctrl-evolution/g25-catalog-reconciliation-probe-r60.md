...
R57 exposed two uncertainties that static files cannot honestly settle: 77 identity candidates appear in migration history but not in generated types, while 116 current generated-schema JSON columns can conceal identifiers that their column names do not reveal. R60 compiles one metadata-only PostgreSQL query covering all 193 targets exactly once.
