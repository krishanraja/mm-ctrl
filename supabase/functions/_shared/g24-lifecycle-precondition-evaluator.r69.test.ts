...
import { dispatchEvaluateLifecyclePreconditionsStructuralR69 } from './g24-lifecycle-precondition-evaluator.r69.mjs'

const validInput = () => ({
  schema_version: 'ctrl.g24.executable-input.evaluate-lifecycle-preconditions.r69.v1',
...
const expected = (status: 'verified_not_runnable' | 'hold') => ({
  schema_version: 'ctrl.g24.executable-output.evaluate-lifecycle-preconditions.r69.v1',
  status,
...
describe('G24 R69 lifecycle evaluator structural artifact', () => {
  it('has one closed no-write output and no semantic success branch', () => {
    expect(dispatchEvaluateLifecyclePreconditionsStructuralR69(validInput())).toEqual(expected('verified_not_runnable'))
  })
  ...
  it('accepts every R63 transition only as structurally verified and not runnable', () => {
    ...
    for (const transition_id of transitions) {
      ...
      expect(dispatchEvaluateLifecyclePreconditionsStructuralR69(input)).toEqual(expected('verified_not_runnable'))
    }
  })
  ...
  it('holds unknown fields, wrong versions, invented predicate authority and malformed intent', () => {
    ...
    for (const input of attacks) {
      expect(dispatchEvaluateLifecyclePreconditionsStructuralR69(input)).toEqual(expected('hold'))
    }
  })
  ...
  it('enforces the inherited NFC, UTF-8 byte and invisible-control identifier policy', () => {
    ...
    for (const operation_id of accepted) {
      expect(dispatchEvaluateLifecyclePreconditionsStructuralR69({ ...validInput(), operation_id })).toEqual(expected('verified_not_runnable'))
    }
    ...
    for (const operation_id of rejected) {
      expect(dispatchEvaluateLifecyclePreconditionsStructuralR69({ ...validInput(), operation_id })).toEqual(expected('hold'))
    }
  })
  ...
  it('holds hostile accessors, cycles and non-plain prototypes without invoking accessors', () => {
    ...
    expect(dispatchEvaluateLifecyclePreconditionsStructuralR69(accessor)).toEqual(expected('hold'))
    ...
    expect(dispatchEvaluateLifecyclePreconditionsStructuralR69(cyclic)).toEqual(expected('hold'))

    const inherited = Object.assign(Object.create({ caller_authority: true }), validInput())
    expect(dispatchEvaluateLifecyclePreconditionsStructuralR69(inherited)).toEqual(expected('hold'))
  })

  it('is byte-stable across repeated calls and cannot accumulate writes', () => {
    const first = JSON.stringify(dispatchEvaluateLifecyclePreconditionsStructuralR69(validInput()))
    for (let index = 0; index < 10; index += 1) {
      expect(JSON.stringify(dispatchEvaluateLifecyclePreconditionsStructuralR69(validInput()))).toBe(first)
    }
  })
})
