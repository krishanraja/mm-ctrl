...
} from "../_shared/standard-change-owner-core.ts";
...
export default {
  fetch: withSupabase({ auth: "user" }, async (request, ctx) => {
    ...
    const rpc = ownerStandardChangeRpc(parsed);
    const { data, error } = await ctx.supabase.rpc(rpc.name, rpc.args);
    if (error) {
      const message = safeErrorMessage(error);
      const status = ownerStandardChangeErrorStatus(message);
      if (status === 500) console.error("review-standard-change failed", { action: parsed.action, error: message });
