...
import { buildStandardChangeCandidate, type CompiledStandardChange } from "../_shared/standard-change-core.ts";
...
const ALLOWED = new Set(["request_id", "compilation_id", "expected_compile_hash"]);
...
export default {
  fetch: withSupabase({ auth: "user" }, async (request, ctx) => {
    ...
    const compilationId = body.compilation_id;
    const expectedHash = body.expected_compile_hash;
    if (typeof requestId !== "string" || !REQUEST.test(requestId)) return response({ error: "invalid_request_id" }, 400);
    if (typeof compilationId !== "string" || !UUID.test(compilationId)) return response({ error: "invalid_compilation_id" }, 400);
    ...
    try {
      const { data: reservation, error: reserveError } = await ctx.supabase.rpc("reserve_standard_change_build", {
        p_request_id: requestId,
        p_request_fingerprint: fingerprint,
        p_compilation_id: compilationId,
        p_expected_compile_sha256: expectedHash,
      ...
      const first = await buildStandardChangeCandidate(
        reservation.compiled as CompiledStandardChange,
        String(reservation.compile_sha256),
        String(reservation.source_body),
      );
      const repeat = await buildStandardChangeCandidate(
        reservation.compiled as CompiledStandardChange,
        String(reservation.compile_sha256),
        String(reservation.source_body),
      );
      if (first.package_sha256 !== repeat.package_sha256 || first.runtime_sha256 !== repeat.runtime_sha256) {
        throw new Error("repeat_build_mismatch");
      }
      const { data: result, error: finalizeError } = await ctx.supabase.rpc("finalize_standard_change_build", {
        p_run_id: runId,
        p_compilation_id: compilationId,
        p_build: first,
        p_capability: capability,
      ...
      return response({ status: "built", result, idempotent: false });
    } catch (error) {
      ...
      if (message.includes("conflict") || message.includes("stage_busy") || message.includes("already_built") || message.includes("not_build_ready")) {
        return response({ error: "state_conflict" }, 409);
      }
      console.error("build-standard-change failed", { error: message });
      return response({ error: "build_failed" }, 500);
    }
  }),
};
