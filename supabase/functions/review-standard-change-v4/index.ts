...
} from '../_shared/standard-change-owner-queue-core.ts'
...
export default {
  fetch: withSupabase({ auth: 'user' }, async (request, ctx) => {
    ...
    let parsed
    try {
      parsed = parseQueuedOwnerStandardChangeRequest(raw)
    } catch (error) {
      return response({ error: safeErrorMessage(error) }, 400)
    }

    const rpc = queuedOwnerStandardChangeRpc(parsed)
    const { data, error } = await ctx.supabase.rpc(rpc.name, rpc.args)
    if (error) {
      const message = safeErrorMessage(error)
      const status = ownerStandardChangeErrorStatus(message)
      if (status === 500) {
        console.error('review-standard-change-v4 failed', { action: parsed.action, error: message })
      }
      return response({
        error: status === 404
          ? 'review_not_found'
