...
> Replaced by: `supabase/functions/ai-generate/training/critical-thinking-training.md` (the in-code copy) and the active `training_material` row described in `docs/current/release-state.md`
> Reason: copy of the training manual that ships with the ai-generate function; differs only in line endings and a few punctuation marks, and the stored training row is the runtime truth.
