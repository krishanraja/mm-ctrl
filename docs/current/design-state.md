...
**The one current evolution action:** design and founder-lock the predicate-authority contract. R70 is independently accepted only for structural executable loading: optional primitive JSON text, pre-parse raw-byte pins, exact UTF-8 round trips and trusted hold envelopes. The next contract must define the exact server-derived structured fact for each of the thirteen catalogue preconditions, dispatch-time freshness and provenance, and the limited role of signed assertions. No result-producing evaluation, runtime integration, database, UI, deployment or external action is authorized.
