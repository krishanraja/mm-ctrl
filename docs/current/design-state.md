...
**The one current evolution action:** preserve the accepted R66 metadata-only architecture, R67 closure and vetoed R68/R69 evidence while repairing the structural evaluator harness forward in R70. Remove caller authority objects from the production-shaped boundary, hard-pin machine bytes before parsing, make UTF-8 round trips exact and use only a trusted hold envelope. Resolve the predicate-authority choice only after this structural gate passes; no runtime integration is authorized, and no database, UI, deployment or external action is authorized.
