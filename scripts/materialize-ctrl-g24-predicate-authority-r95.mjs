...
const effectivePath = `${directory}/08-effective-contract.json`
...
function buildEffective(modules) {
  ...
  const authorityMaterial = {
    imports,
    trust_and_context_program: modules.trust_and_context_program,
    semantic_set_program: modules.semantic_set_program,
  ...
  return {
    schema_version: 'ctrl.g24.predicate-authority.r95.effective-contract.v1',
    authority_bundle_fingerprint: sha256(Buffer.from(canonical(authorityMaterial), 'utf8')),
    ...imports,
    trust_and_context_program: modules.trust_and_context_program,
    semantic_set_program: modules.semantic_set_program,
    interaction_program: modules.interaction_program,
...
function build() {
  ...
  const withoutFingerprint = {
    schema_version: 'ctrl.g24.predicate-authority.r95.manifest.v1',
    standing: 'bound_local_contract_candidate_no_runtime_authority',
...
function main() {
  ...
  if (!['--write', '--check'].includes(mode)) { console.error('usage: node scripts/materialize-ctrl-g24-predicate-authority-r95.mjs --write|--check'); process.exit(2) }
  const built = build()
  if (mode === '--write') {
  ...
  if (!readFileSync(join(root, effectivePath)).equals(built.effectiveBytes) || !readFileSync(join(root, manifestPath)).equals(built.manifestBytes)) { console.error('R95 materialized bytes are stale'); process.exit(1) }
  const manifest = JSON.parse(built.manifestBytes)
  console.log(`ok: ${manifest.modules.length} R95 source modules; ${manifest.bundle_fingerprint}`)
}
