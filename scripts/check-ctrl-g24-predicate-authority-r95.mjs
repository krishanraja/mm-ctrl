...
const directory = 'project-documentation/ctrl-evolution/g24-predicate-authority-r95'
...
function validateCurrentStatusProjection(value, authoritativeObligations = null) {
  ...
  if (!authoritativeObligations.every(obligation => exactKeys(obligation, correction.replacement_obligation_required) && typeof obligation.named_human_id === 'string' && permittedNamedHumans.includes(obligation.named_human_id) && safePositiveInteger(obligation.required_replacements))) return false
  const obligationHumans = value.replacement_obligations.map(obligation => obligation.named_human_id)
  const authoritativeHumans = authoritativeObligations.map(obligation => obligation.named_human_id), total = safeObligationTotal(value.replacement_obligations), authoritativeTotal = safeObligationTotal(authoritativeObligations)
  return unique(obligationHumans) && same(obligationHumans, byteSorted(obligationHumans)) && same(obligationHumans, value.responsible_humans) && unique(authoritativeHumans) && same(authoritativeHumans, byteSorted(authoritativeHumans)) && safePositiveInteger(total) && safePositiveInteger(authoritativeTotal) && total === value.required_replacements && total === authoritativeTotal && same(value.replacement_obligations, authoritativeObligations) && value.message === renderReplacementMessage(value.replacement_obligations)
}

const expectedTopKeys = {
  effective_resolution: ['schema_version', 'module_id', 'semantic_base', 'imported_exact_sections', 'r95_replacement_sections', 'inheritance_rule', 'unknown_or_conflicting_section', 'authority_still_closed'],
...
check('accepted final registry schemas closed', same(correction.accepted_final_registry_required, ['registry_id', 'records']) && Array.isArray(correction.accepted_final_record_required) && correction.accepted_final_record_required.length > 10 && Array.isArray(correction.accepted_final_challenge_required) && Array.isArray(correction.challenge_observation_required) && Array.isArray(correction.descendant_block_required) && Array.isArray(correction.revalidation_authority_entry_required) && Array.isArray(correction.revalidation_record_required) && Array.isArray(correction.dependency_baseline_required) && exactKeys(correction.current_status_projection_variants, ['replacement_required', 'review_required', 'current', 'not_found']) && same(correction.current_status_projection_variants.replacement_required, ['status', 'message', 'required_replacements', 'responsible_humans', 'replacement_obligations']) && ['review_required', 'current', 'not_found'].every(status => same(correction.current_status_projection_variants[status], ['status', 'message'])) && same(correction.replacement_obligation_required, ['named_human_id', 'required_replacements']) && Array.isArray(correction.current_status_authority_rules) && correction.current_status_authority_rules.length === 6 && same(permittedNamedHumans, ['krish', 'leader-1']) && Array.isArray(trust.answer_state_revalidation_head_required) && Array.isArray(correction.answer_state_repair_rules) && correction.answer_state_repair_rules.length >= 10)
...
check('manifest schema and authority metadata exact', manifest.schema_version === 'ctrl.g24.predicate-authority.r95.manifest.v1' && manifest.standing === 'bound_local_contract_candidate_no_runtime_authority' && manifest.decision_id === 'DEC-20260916-g24-predicate-authority-r75' && canonical(manifest.semantic_base) === canonical(resolution.semantic_base) && manifest.resolution_rule === resolution.inheritance_rule)
...
function buildFinalReceipt(transitionId, predicateProof) {
  ...
  const authorityProofs = humans.map(human => ({ named_human_id: human, authentication_event_ref: human === 'krish' ? 'auth-r95-krish' : 'auth-r95-leader', authority_event_ref: `final-authority-${transitionId}-${human}` }))
...
if (process.env.R95_EMIT_FINAL_UNSIGNED === '1') {
  ...
  console.log(`R95_UNSIGNED_FINALS=${JSON.stringify(emitted)}`)
  process.exit(0)
}
...
const acceptedFinalService = (() => {
  ...
  const challengeAnswerHead = (prior, next, transition) => {
    for (const record of records) {
      ...
      const challenge = { challenge_id: challengeId, accepted_final_id: record.accepted_final_id, accepted_record_fingerprint: record.record_fingerprint, baseline_ref: baseline.baseline_id, route_ref: prior.route_ref, question_id: prior.question_id, trigger_transition_ref: transition.event_id, changed_head_fingerprint: prior.state_fingerprint, resulting_head_fingerprint: next.state_fingerprint, subject_id: record.subject_id, case_id: record.case_id, purpose_id: record.purpose_id, permitted_audience: record.permitted_audience, standing: 'challenged', current_steering_eligibility: false, status_text: statusText, challenged_at: transition.authorized_at, challenge_fingerprint: '' }; challenge.challenge_fingerprint = fingerprint(challenge, 'challenge_fingerprint')
  ...
  const currentStatus = acceptedFinalId => {
    if (typeof acceptedFinalId !== 'string' || acceptedFinalId.length === 0 || !records.some(record => record.accepted_final_id === acceptedFinalId)) return { status: 'not_found', message: 'This decision is unavailable.' }
    const open = unresolvedChallenges(acceptedFinalId)
    if (open.length) {
      const state = currentHeadState(open, false), missingHeads = state.heads.filter(head => head.standing !== 'current')
      if (missingHeads.length) {
        const counts = new Map()
        for (const head of missingHeads) {
          const human = specimens.answer_state_transition_events[head.transition_event_ref]?.named_human_id
          if (typeof human === 'string' && human.length) counts.set(human, (counts.get(human) ?? 0) + 1)
        }
        const replacementObligations = byteSorted([...counts.keys()]).map(namedHumanId => ({ named_human_id: namedHumanId, required_replacements: counts.get(namedHumanId) }))
        const status = { status: 'replacement_required', message: renderReplacementMessage(replacementObligations), required_replacements: missingHeads.length, responsible_humans: replacementObligations.map(obligation => obligation.named_human_id), replacement_obligations: replacementObligations }
        return validateCurrentStatusProjection(status, replacementObligations) ? status : { status: 'not_found', message: 'This decision is unavailable.' }
      }
...
if (process.env.R95_EMIT_MULTI_CHANGE_UNSIGNED === '1') {
  sessionAuthorityService.resetForConformance(); answerAuthorityService.resetForConformance(); acceptedFinalService.resetForConformance()
  const built = buildPredicateProof('accept_intensive_proof'), final = buildFinalReceipt('accept_intensive_proof', built.proof)
  ...
  console.log(`R95_UNSIGNED_MULTI_CHANGE=${JSON.stringify(unsigned)}`)
  process.exit(0)
}
if (process.env.R95_EMIT_MIXED_CHANGE_UNSIGNED === '1') {
  sessionAuthorityService.resetForConformance(); answerAuthorityService.resetForConformance(); acceptedFinalService.resetForConformance()
  const built = buildPredicateProof('accept_intensive_proof'), final = buildFinalReceipt('accept_intensive_proof', built.proof)
  ...
  console.log(`R95_UNSIGNED_MIXED_CHANGE=${JSON.stringify(unsigned)}`)
  process.exit(0)
}
if (process.env.R95_EMIT_DOUBLE_WITHDRAW_UNSIGNED === '1') {
  sessionAuthorityService.resetForConformance(); answerAuthorityService.resetForConformance(); acceptedFinalService.resetForConformance()
  const built = buildPredicateProof('accept_intensive_proof'), final = buildFinalReceipt('accept_intensive_proof', built.proof)
  ...
  console.log(`R95_UNSIGNED_DOUBLE_WITHDRAW=${JSON.stringify(unsigned)}`)
  process.exit(0)
}
...
}
{
  ...
  check('replacement projection accepts one exact context-bound discriminated schema', validateCurrentStatusProjection(valid, validObligations))
  check('replacement projection rejects missing extra wrong-type duplicate unsorted mismatched and false-copy attacks', [missing, extra, wrongType, duplicateOwner, badSum, unsorted, falseCopy].every(value => !validateCurrentStatusProjection(value, validObligations)))
  check('replacement projection rejects unknown owners and a registered but contextually wrong owner', !validateCurrentStatusProjection(unknownOwner, unknownOwner.replacement_obligations) && !validateCurrentStatusProjection(twoUnknownOwners, twoUnknownOwners.replacement_obligations) && !validateCurrentStatusProjection(wrongRegisteredOwner, [{ named_human_id: 'krish', required_replacements: 1 }]))
  check('replacement projection rejects unsafe counts unsafe sums and totals beyond authoritative missing heads', !validateCurrentStatusProjection(unsafeCount, unsafeCount.replacement_obligations) && !validateCurrentStatusProjection(unsafeSum, unsafeSum.replacement_obligations) && !validateCurrentStatusProjection(exceedsAuthorityObligations, [{ named_human_id: 'krish', required_replacements: 3 }]))
...
if (failures.length) {
  console.error(`R95 independent conformance failed (${failures.length})`)
  for (const failure of failures) console.error(`- ${failure}`)
  process.exit(1)
}
console.log(`ok: R95 end-to-end bound contract; ${imports.transition_catalogue.length}/13 predicate and final transition paths; ${vectorGroups.reduce((sum, group) => sum + vectors[group].length, 0)} executable vectors; ${totalityProbeCount} generated totality probes`)
