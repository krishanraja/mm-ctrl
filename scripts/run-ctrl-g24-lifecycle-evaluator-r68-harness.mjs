...
const DEFAULT_R68_PATH = 'project-documentation/ctrl-evolution/g24-lifecycle-precondition-evaluator-r68.json'
...
const holdOutput = r68 => ({
  schema_version: r68.dispatch_output_schema.schema_version,
  status: 'hold',
...
function ownedInputSnapshot(input) {
  const seen = new WeakSet()
  let nodes = 0
  let stringUnits = 0
  const visit = (value, depth) => {
    nodes += 1
    assert(nodes <= 256 && depth <= 8, 'input:resource_bound')
    if (value === null || typeof value === 'boolean') return value
    if (typeof value === 'string') {
      stringUnits += value.length
      assert(value.length <= 16384 && stringUnits <= 32768, 'input:string_bound')
      return value
    }
    if (typeof value === 'number') {
      assert(Number.isFinite(value) && !Object.is(value, -0), 'input:number')
      return value
    }
    assert(value && typeof value === 'object' && !seen.has(value), 'input:type_or_cycle')
    seen.add(value)
    if (Array.isArray(value)) {
      assert(value.length <= 128, 'input:array_bound')
      const keys = Reflect.ownKeys(value)
      assert(keys.length === value.length + 1 && keys.includes('length'), 'input:array_shape')
      const output = []
      for (let index = 0; index < value.length; index += 1) {
        const descriptor = Object.getOwnPropertyDescriptor(value, String(index))
        assert(descriptor && !descriptor.get && !descriptor.set && descriptor.enumerable, 'input:array_descriptor')
        Object.defineProperty(output, String(index), { value: visit(descriptor.value, depth + 1), enumerable: true, writable: false, configurable: false })
      }
      return Object.freeze(output)
    }
    const prototype = Object.getPrototypeOf(value)
    assert(prototype === Object.prototype || prototype === null, 'input:prototype')
    const keys = Reflect.ownKeys(value)
    assert(keys.length <= 64 && keys.every(key => typeof key === 'string'), 'input:object_keys')
    const output = Object.create(null)
    for (const key of keys) {
      const descriptor = Object.getOwnPropertyDescriptor(value, key)
      assert(descriptor && !descriptor.get && !descriptor.set && descriptor.enumerable, 'input:object_descriptor')
...
export function verifyR66LifecycleSelection(contract, r68, recordOverride) {
  ...
  assert(r68.executable_artifact.descriptor_content_ref === descriptor.descriptor_content_ref && r68.executable_artifact.descriptor_bytes_sha256 === descriptor.canonical_descriptor_bytes_sha256, 'selection:R68_descriptor_binding')

  const binding = r68.r66_selection_binding
  assert(same(binding, {
...
export function inspectExecutableSource(sourceText, entrypoint) {
  const file = ts.createSourceFile('g24-lifecycle-precondition-evaluator.r68.mjs', sourceText, ts.ScriptTarget.ESNext, true, ts.ScriptKind.JS)
...
function verifyArtifact(r68, executableBytes, artifactOverride) {
  const expected = r68.executable_artifact
  const artifact = artifactOverride ?? expected
  const bytesSha = sha256(executableBytes)
  assert(artifact.schema_version === 'ctrl.g24.evaluator-executable-artifact.r68.v1', 'artifact:schema')
  ...
  assert(artifact.entrypoint === 'dispatchEvaluateLifecyclePreconditionsStructuralR68', 'artifact:entrypoint')
  ...
  assert(lock.schema_version === 'ctrl.g24.evaluator-source-lock.r68.v1' && lock.domain_ascii === 'CTRL-G24-EVALUATOR-SOURCE-LOCK-R68', 'artifact:lock_schema_domain')
...
const result = await moduleValue[request.entrypoint](request.input)
process.stdout.write(JSON.stringify(result))
`

export function runIsolatedModule(executableBytes, entrypoint, input, options = {}) {
  const maxInputBytes = options.maxInputBytes ?? 65536
  const maxOutputBytes = options.maxOutputBytes ?? 65536
  const timeoutMs = options.timeoutMs ?? 1500
  const safeInput = ownedInputSnapshot(input)
  const request = Buffer.from(JSON.stringify({ source_b64: executableBytes.toString('base64'), entrypoint, input: safeInput }), 'utf8')
  assert(request.length <= maxInputBytes + executableBytes.length * 2, 'isolate:input_bound')
...
function validateClosedDispatch(r68, output) {
  const schema = r68.dispatch_output_schema
...
export function runR68StructuralGate({ r66, r68, executableBytes, dispatchInput, selectionRecord, artifact, isolateOptions } = {}) {
  const contract = r66 ?? JSON.parse(readFileSync(join(root, DEFAULT_R66_PATH), 'utf8'))
  const gate = r68 ?? JSON.parse(readFileSync(join(root, DEFAULT_R68_PATH), 'utf8'))
  const bytes = executableBytes ?? readFileSync(join(root, gate.executable_artifact.source_path))
  const input = dispatchInput ?? gate.sample_dispatch_input
  try {
    verifyR66LifecycleSelection(contract, gate, selectionRecord)
    const verifiedArtifact = verifyArtifact(gate, bytes, artifact)
    const options = {
    ...
    const first = validateClosedDispatch(gate, runIsolatedModule(bytes, verifiedArtifact.entrypoint, input, options))
    const second = validateClosedDispatch(gate, runIsolatedModule(bytes, verifiedArtifact.entrypoint, input, options))
...
if (process.argv[1] && process.argv[1].endsWith('run-ctrl-g24-lifecycle-evaluator-r68-harness.mjs')) {
  const result = runR68StructuralGate()
