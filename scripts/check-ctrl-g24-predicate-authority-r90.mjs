...
const directory = 'project-documentation/ctrl-evolution/g24-predicate-authority-r90'
...
const expectedTopKeys = {
  effective_resolution: ['schema_version', 'module_id', 'semantic_base', 'imported_exact_sections', 'r90_replacement_sections', 'inheritance_rule', 'unknown_or_conflicting_section', 'authority_still_closed'],
...
check('accepted final registry schemas closed', same(correction.accepted_final_registry_required, ['registry_id', 'records']) && Array.isArray(correction.accepted_final_record_required) && correction.accepted_final_record_required.length > 10 && Array.isArray(correction.accepted_final_challenge_required) && Array.isArray(correction.descendant_block_required) && Array.isArray(correction.revalidation_record_required) && Array.isArray(correction.dependency_baseline_required) && Array.isArray(trust.answer_state_revalidation_head_required) && Array.isArray(correction.answer_state_repair_rules) && correction.answer_state_repair_rules.length >= 10)
...
check('manifest schema and authority metadata exact', manifest.schema_version === 'ctrl.g24.predicate-authority.r90.manifest.v1' && manifest.standing === 'bound_local_contract_candidate_no_runtime_authority' && manifest.decision_id === 'DEC-20260916-g24-predicate-authority-r75' && canonical(manifest.semantic_base) === canonical(resolution.semantic_base) && manifest.resolution_rule === resolution.inheritance_rule)
...
function buildFinalReceipt(transitionId, predicateProof) {
  ...
  const authorityProofs = humans.map(human => ({ named_human_id: human, authentication_event_ref: human === 'krish' ? 'auth-r90-repair-krish' : 'auth-r90-repair-leader', authority_event_ref: `final-authority-${transitionId}-${human}` }))
...
if (process.env.R90_EMIT_FINAL_UNSIGNED === '1') {
  ...
  console.log(`R90_UNSIGNED_FINALS=${JSON.stringify(emitted)}`)
  process.exit(0)
}
...
const acceptedFinalService = (() => {
  let records = [], challenges = [], blocks = [], repairs = [], revalidations = [], baselines = []
  ...
  const commit = (receipt, predicateProof, evidence, finalRender, authorityEvents) => {
    ...
    const baseline = { baseline_id: `baseline-${id}-accepted`, accepted_final_id: id, predecessor_baseline_ref: null, source_ref: id, answer_heads: predicateProof.answer_state_heads.map(headDescriptor).sort((a, b) => Buffer.from(`${a.route_ref}|${a.question_id}`).compare(Buffer.from(`${b.route_ref}|${b.question_id}`))), standing: 'current', created_at: receipt.server_commit_time, record_fingerprint: '' }
  ...
  const currentHeadState = (openChallenges, requireCurrent = true) => {
    ...
    return { valid: eligible, heads: heads.map(headDescriptor).sort((a, b) => Buffer.from(`${a.route_ref}|${a.question_id}`).compare(Buffer.from(`${b.route_ref}|${b.question_id}`))) }
  }
  ...
  const validateRevalidationEvent = (event, record, openChallenges) => {
    ...
    const transition = transitionById[record.transition_id], allowedHumans = transition.final_authority.actor_roles.map(role => roleHuman[role]), currentState = currentHeadState(openChallenges, true), challengeIds = byteSorted(openChallenges.map(challenge => challenge.challenge_id))
  ...
  const buildUnsignedRevalidationEvents = (acceptedFinalId, requireCurrent) => {
    ...
    const transition = transitionById[record.transition_id], humans = byteSorted([...new Set(transition.final_authority.actor_roles.map(role => roleHuman[role]))]), challengeIds = byteSorted(open.map(challenge => challenge.challenge_id))
  ...
  const revalidate = events => {
    ...
    const resolution = { revalidation_id: `revalidated-${hashValue({ accepted_final_id: record.accepted_final_id, challenge_set_seal: hashValue(challengeIds) }).slice(0, 32)}`, accepted_final_id: record.accepted_final_id, accepted_request_identity: record.request_identity, event_refs: eventRefs, event_fingerprints: eventFingerprints, challenge_ids: challengeIds, challenge_set_seal: hashValue(challengeIds), block_ids: blockIds, current_answer_heads: currentState.heads, standing: 'resolved', status_text: 'You reviewed the change with Krish. Work can move again using your current answer.', resolved_at: events[0].authorized_at, record_fingerprint: '' }; resolution.record_fingerprint = fingerprint(resolution, 'record_fingerprint')
    ...
    const priorBaseline = activeBaseline(record.accepted_final_id), baseline = { baseline_id: `baseline-${resolution.revalidation_id}`, accepted_final_id: record.accepted_final_id, predecessor_baseline_ref: priorBaseline.baseline_id, source_ref: resolution.revalidation_id, answer_heads: clone(currentState.heads), standing: 'current', created_at: resolution.resolved_at, record_fingerprint: '' }
    baseline.record_fingerprint = fingerprint(baseline, 'record_fingerprint')
  ...
  const currentStatus = acceptedFinalId => {
    ...
    if (open.length) {
      ...
      if (withdrawal && state.heads.find(head => head.route_ref === withdrawal.route_ref && head.question_id === withdrawal.question_id)?.standing === 'current') return 'Your new answer is ready. Work stays paused until you review it with Krish.'
      return open.at(-1).status_text
    }
    return [...revalidations].reverse().find(value => value.accepted_final_id === acceptedFinalId)?.status_text ?? 'This decision is current.'
  }
  return Object.freeze({ resolve, canCommit, commit, challengeAnswerHead, isBlocked, revalidate, unsignedRevalidationEvents, unsafeUnsignedRevalidationEventsForConformance, currentStatus, records: () => clone(records), challenges: () => clone(challenges), blocks: () => clone(blocks), repairs: () => clone(repairs), revalidations: () => clone(revalidations), baselines: () => clone(baselines), resetForConformance: () => { records = []; challenges = []; blocks = []; repairs = []; revalidations = []; baselines = [] } })
})()
...
}
{
  ...
  if (head) {
    ...
    check('single authoritative status says both replacement and review are required', acceptedFinalService.currentStatus(acceptedFinalService.records()[0].accepted_final_id) === 'Your earlier decision stays on record. Nothing else will move until you choose a new answer and review it with Krish.')
    const reanswer = Object.values(specimens.answer_state_transition_events).find(event => event.route_ref === head.route_ref && event.question_id === head.question_id && event.action === 'reanswer')
    ...
    check('single authoritative status now says review remains', acceptedFinalService.currentStatus(acceptedFinalService.records()[0].accepted_final_id) === 'Your new answer is ready. Work stays paused until you review it with Krish.')
    ...
    check('single authoritative status truthfully reports reopening', acceptedFinalService.currentStatus(acceptedFinalService.records()[0].accepted_final_id) === 'You reviewed the change with Krish. Work can move again using your current answer.')
  }
}
{
  sessionAuthorityService.resetForConformance(); answerAuthorityService.resetForConformance(); acceptedFinalService.resetForConformance()
...
if (failures.length) {
  console.error(`R90 independent conformance failed (${failures.length})`)
  for (const failure of failures) console.error(`- ${failure}`)
  process.exit(1)
}
console.log(`ok: R90 end-to-end bound contract; ${imports.transition_catalogue.length}/13 predicate and final transition paths; ${vectorGroups.reduce((sum, group) => sum + vectors[group].length, 0)} executable vectors; ${totalityProbeCount} generated totality probes`)
