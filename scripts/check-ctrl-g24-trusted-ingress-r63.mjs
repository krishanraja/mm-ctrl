...
import { materializedR62Output } from './materialize-ctrl-g24-trusted-ingress-r62.mjs'
...
const machinePath = 'project-documentation/ctrl-evolution/g24-trusted-ingress-contract-r63.json'
...
function validateManifest(contract) {
  ...
  const sourcePaths = r63SemanticAuthorityPaths
  ...
  const expectedSnapshotSha = hash({ domain_ascii: 'CTRL-G24-R63-FINAL-SEMANTIC-SOURCE-SNAPSHOT', exact_paths: sourcePaths, values: sourceSnapshot })
  ...
  assert(hc.manifest_hash_version === 'ctrl.g24.runtime-semantic-manifest-hash.r63.v1', 'manifest:hash_version')
  ...
  const withoutSeal = { schema_version: 'ctrl.g24.runtime-semantic-authority-manifest.r63.v1', source_snapshot_sha256: expectedSnapshotSha, exact_paths: sourcePaths, rows: manifestRows, exact_expected_count: manifestRows.length, manifest_graph_sha256: hash({ domain_ascii: hc.graph_domain_ascii, manifest_hash_version: hc.manifest_hash_version, manifest_rows: manifestRows }) }
...
function validate(contract) {
  assert(contract.supersedes.commit === '73ebd17abbd5abd8b1ea8ddc78487287c2a641fb' && contract.supersedes.tree === '938cfb61c07ab0ce29d25091c651e8882f461f75', 'parent:git')
  assert(contract.supersedes.human_blob === 'b0d09c1f7b7e77b0670c7f3e877ea902a2857a61' && contract.supersedes.machine_blob === 'fcab9032998d78027c7393ae2c6bae848d58da33' && contract.supersedes.qa_blob === '830ac37878a175bb462e83fe11b2eab8c440c6cf', 'parent:core_blobs')
  assert(contract.supersedes.checker_blob === '48836b1fbf123bf6851065fabb0ee426190dd0f1' && contract.supersedes.materializer_blob === 'e2f5ab4a53eae907a5eaee77e3b1ca0ea0f590ed' && contract.supersedes.founder_checker_blob === '2546d35741440495c2ecd26dcc619e2d3b332dca', 'parent:tool_blobs')
  assert(sha(materializedR62Output) === '6ef89029be815e37dd5f2ea5871253dfdd583949c4eb217bf07d479a7bdbc3a8', 'parent:machine_sha256')
  assert(contract.materialization.generator === 'scripts/materialize-ctrl-g24-trusted-ingress-r63.mjs' && contract.materialization.frozen_input.sha256 === sha(materializedR62Output), 'materialization:identity')
...
assert(actualBytes === materializedR63Output, 'machine:exact_materialization')
validate(materializedR63)

let attacks = 0
function reject(name, mutate) {
  attacks += 1
  const candidate = structuredClone(materializedR63)
  mutate(candidate)
  let rejected = false
...
console.log(`ok: R63 exact; ${attacks} attacks; 1/1 actual R13 satisfied row for the one open_preparation catalogue precondition; 1/1 lifecycle proof member fingerprint; evaluate result and apply intent carry no caller-supplied precondition_set_seal; transition stage server-reserves owner lineage then computes the exact owner-bound set seal; canonical evidence bytes length and closed decoded schema bound; 19 reversible plus 2 singleton-only full ordering sites; ${materializedR63.authority_runtime_semantic_reference_field_registry.exact_expected_occurrence_count} semantic refs; ${materializedR63.authority_runtime_semantic_manifest.exact_expected_count} manifest rows; frozen R62 preserved`)
