...
const directory = 'project-documentation/ctrl-evolution/g24-predicate-authority-r91'
...
const expectedTopKeys = {
  effective_resolution: ['schema_version', 'module_id', 'semantic_base', 'imported_exact_sections', 'r91_replacement_sections', 'inheritance_rule', 'unknown_or_conflicting_section', 'authority_still_closed'],
...
check('manifest schema and authority metadata exact', manifest.schema_version === 'ctrl.g24.predicate-authority.r91.manifest.v1' && manifest.standing === 'bound_local_contract_candidate_no_runtime_authority' && manifest.decision_id === 'DEC-20260916-g24-predicate-authority-r75' && canonical(manifest.semantic_base) === canonical(resolution.semantic_base) && manifest.resolution_rule === resolution.inheritance_rule)
...
function buildFinalReceipt(transitionId, predicateProof) {
  ...
  const authorityProofs = humans.map(human => ({ named_human_id: human, authentication_event_ref: human === 'krish' ? 'auth-r91-krish' : 'auth-r91-leader', authority_event_ref: `final-authority-${transitionId}-${human}` }))
...
if (process.env.R91_EMIT_FINAL_UNSIGNED === '1') {
  ...
  console.log(`R91_UNSIGNED_FINALS=${JSON.stringify(emitted)}`)
  process.exit(0)
}
...
const acceptedFinalService = (() => {
  ...
  const headDescriptor = record => ({ route_ref: record.route_ref, question_id: record.question_id, answer_state_id: record.answer_state_id, state_version: record.state_version, issuer_sequence: record.issuer_sequence, state_fingerprint: record.state_fingerprint, standing: record.standing })
  ...
  const revalidationEventId = (acceptedFinalId, challengeSetSeal, namedHumanId) => `revalidation-${acceptedFinalId}-${challengeSetSeal.slice(0, 12)}-${namedHumanId}`
  ...
  const validateRevalidationEvent = (event, record, openChallenges) => {
    ...
    const transition = transitionById[record.transition_id], allowedHumans = transition.final_authority.actor_roles.map(role => roleHuman[role]), currentState = currentHeadState(openChallenges, true), challengeIds = byteSorted(openChallenges.map(challenge => challenge.challenge_id)), observationIds = byteSorted(observations.filter(value => challengeIds.includes(value.challenge_id)).map(value => value.observation_id))
  ...
  const revalidate = events => {
    ...
    const challengeIds = byteSorted(open.map(challenge => challenge.challenge_id)), blockIds = byteSorted(blocks.filter(block => challengeIds.includes(block.challenge_id)).map(block => block.block_id))
    ...
    const resolution = { revalidation_id: `revalidated-${hashValue({ accepted_final_id: record.accepted_final_id, challenge_set_seal: hashValue(challengeIds) }).slice(0, 32)}`, accepted_final_id: record.accepted_final_id, accepted_request_identity: record.request_identity, authority_events: entriesForRetry, challenge_ids: challengeIds, challenge_set_seal: hashValue(challengeIds), challenge_observation_refs: challengeObservationRefs, challenge_observation_seal: challengeObservationSeal, block_ids: blockIds, current_answer_heads: mergedHeads, standing: 'resolved', status_text: 'You reviewed the change with Krish. Work can move again using your current answer.', resolved_at: events[0].authorized_at, record_fingerprint: '' }; resolution.record_fingerprint = fingerprint(resolution, 'record_fingerprint')
  ...
  const currentStatus = acceptedFinalId => {
    ...
    if (open.length) {
      const state = currentHeadState(open, false), withdrawal = open.find(challenge => specimens.answer_state_transition_events[challenge.trigger_transition_ref]?.action === 'withdraw')
      if (withdrawal && state.heads.find(head => head.route_ref === withdrawal.route_ref && head.question_id === withdrawal.question_id)?.standing === 'current') return { status: 'review_required', message: 'Your new answer is ready. Work stays paused until you review it with Krish.' }
      return { status: 'review_required', message: open.at(-1).status_text }
    }
...
if (process.env.R91_EMIT_MULTI_CHANGE_UNSIGNED === '1') {
  sessionAuthorityService.resetForConformance(); answerAuthorityService.resetForConformance(); acceptedFinalService.resetForConformance()
  const built = buildPredicateProof('accept_intensive_proof'), final = buildFinalReceipt('accept_intensive_proof', built.proof)
  ...
  console.log(`R91_UNSIGNED_MULTI_CHANGE=${JSON.stringify(unsigned)}`)
  process.exit(0)
}
...
}
{
  ...
  if (head) {
    ...
    check('single authoritative status says both replacement and review are required', same(acceptedFinalService.currentStatus(acceptedFinalService.records()[0].accepted_final_id), { status: 'review_required', message: 'Your earlier decision stays on record. Nothing else will move until you choose a new answer and review it with Krish.' }))
    const reanswer = Object.values(specimens.answer_state_transition_events).find(event => event.route_ref === head.route_ref && event.question_id === head.question_id && event.action === 'reanswer')
    ...
    check('single authoritative status now says review remains', same(acceptedFinalService.currentStatus(acceptedFinalService.records()[0].accepted_final_id), { status: 'review_required', message: 'Your new answer is ready. Work stays paused until you review it with Krish.' }))
...
}
{
  ...
  const multiEvents = Object.values(specimens.answer_state_revalidation_multi_change_events), successor = buildPredicateProof('continue_after_intensive_proof')
...
if (failures.length) {
  console.error(`R91 independent conformance failed (${failures.length})`)
  for (const failure of failures) console.error(`- ${failure}`)
  process.exit(1)
}
console.log(`ok: R91 end-to-end bound contract; ${imports.transition_catalogue.length}/13 predicate and final transition paths; ${vectorGroups.reduce((sum, group) => sum + vectors[group].length, 0)} executable vectors; ${totalityProbeCount} generated totality probes`)
