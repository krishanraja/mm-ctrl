...
import { materializedR60, materializedR60Output } from './materialize-ctrl-g24-trusted-ingress-r60.mjs'
...
function validateOrdering(contract, spec, value, context, label) {
  ...
  if (rule === 'matching_evidence_refs') {
    ...
    assert(Array.isArray(refs) && refs.length === value.length && Array.isArray(resolvedPairs) && resolvedPairs.length === refs.length, `${label}:ordering_companion_alignment`)
    assert(new Set(resolvedPairs.map(pair => pair?.evidence_ref)).size === resolvedPairs.length, `${label}:ordering_duplicate_resolved_ref`)
    for (let index = 0; index < refs.length; index += 1) { const pair = resolvedPairs[index]; assert(pair && pair.evidence_ref === refs[index] && pair.evidence_fingerprint === value[index] && /^[0-9a-f]{64}$/.test(pair.evidence_fingerprint), `${label}:ordering_evidence_pair_mismatch:${index}`) }
...
function chooseVariant(schema, value = {}) {
  ...
  const matches = Object.entries(schema.variants).filter(([, candidate]) => Object.entries(candidate.properties ?? {}).every(([field, spec]) => {
    const allowed = allowedValues(materializedR60, spec)
...
function validateSpec(contract, spec, value, label, context = {}) {
  ...
  if (spec.type === 'controlling_watermark_change') { const variant = contract.type_registry.controlling_watermark_change.variants?.[value?.kind_class]; assert(variant, `${label}:watermark_variant`); validateSchema(contract, variant, value, `${label}<${value.kind_class}>`); return }
  ...
  if (spec.type === 'discriminated_union') { const variant = spec.variants?.[value?.[spec.discriminator]]; assert(variant, `${label}:union_variant`); validateSchema(contract, variant, value, `${label}<${value[spec.discriminator]}>`); return }
...
function validateSchema(contract, schema, payload, label) {
  ...
  for (const [field, spec] of Object.entries(schema.properties)) if (Object.hasOwn(payload, field)) validateSpec(contract, spec, payload[field], `${label}:${field}`, { ...payload })
...
function recomputeLinkedTarget(contract, authority, decoded, label) {
  ...
  assert(shared && shared.schema_version === 'ctrl.g24.complete-joint-equality-fixture.r60.v1', `${label}:complete_joint_dereference`)
...
function specValues(spec) { return allowedValues(materializedR60, spec) }
...
function validateSelector(contract, selector) {
  ...
  assert(selector.schema_version === 'ctrl.g24.native-source-schema-intersection-selector.r60.v1' && selector.exact_case_count === expected.length, `selector_count:${selector.selector_id}`)
...
function validateManifest(contract) {
  ...
  const snapshot = Object.fromEntries(paths.map(path => [path, ownedSnapshotR44(get(contract, path))])), snapshotSha = hash({ domain_ascii: 'CTRL-G24-R60-FINAL-SEMANTIC-SOURCE-SNAPSHOT', exact_paths: paths, values: snapshot })
...
function validateOrderingAuthority(contract) {
  ...
  assert(occurrences.length === 21 && registry.exact_occurrence_count === 21 && registry.exact_handler_count === 6 && registry.exact_positive_fixture_count === 21 && registry.exact_negative_fixture_count === 21, 'ordering_inventory_counts')
  ...
  assert(same(registry.matching_evidence_pair_authority, { schema_version: 'ctrl.g24.resolved-evidence-pair-ordering.r60.v1', pair_exact_keys: ['evidence_ref', 'evidence_fingerprint'], evidence_ref_unique: true, evidence_ref_must_equal_same_position_source_ref: true, evidence_fingerprint_must_equal_resolved_row_fingerprint: true, missing_duplicate_substituted_or_swapped_pair: 'reject_materialization_and_hold_without_write' }), 'ordering_pair_authority')
  const expected = occurrences.map((occurrence, index) => {
    ...
    else if (rule === 'matching_evidence_refs') { const refs = [`evidence_${String(index).padStart(2, '0')}_a`, `evidence_${String(index).padStart(2, '0')}_b`], fingerprints = refs.map(ref => sha(`R60:${occurrence.source_path}:${ref}`)), pairs = refs.map((evidence_ref, pairIndex) => ({ evidence_ref, evidence_fingerprint: fingerprints[pairIndex] })); positiveValue = fingerprints; positiveContext = { evidence_refs: refs, resolved_evidence_pairs: pairs }; negativeValue = [...fingerprints].reverse(); negativeContext = structuredClone(positiveContext) }
    else assert(false, `ordering_fixture_unknown:${rule}`)
    validateOrdering(contract, { ordered_by: rule }, positiveValue, positiveContext, `${occurrence.source_path}:positive`)
    let rejected = false
    try { validateOrdering(contract, { ordered_by: rule }, negativeValue, negativeContext, `${occurrence.source_path}:negative`) } catch { rejected = true }
    assert(rejected, `ordering_negative_not_rejected:${occurrence.source_path}`)
  ...
  assert(same(registry.executable_occurrence_fixtures, expected), 'ordering_executable_fixture_exact_derivation')
  return occurrences.length
}
function validate(contract) {
  assert(contract.schema_version === 'ctrl.g24.trusted-ingress.r60.effective.v1' && contract.schema_change_manifest.runtime_database_ui_deployment_or_external_action === 'closed', 'r60_boundary')
...
assert(readFileSync(join(root, 'project-documentation/ctrl-evolution/g24-trusted-ingress-contract-r59.json'), 'utf8') === materializedR54Output, 'frozen_R59_exact')
assert(readFileSync(join(root, outputPath), 'utf8') === materializedR60Output, 'exact_R60_materialization')
const baseline = validate(materializedR60)
let attacks = 0
function reject(name, fn) { let rejected = false; try { fn() } catch { rejected = true } assert(rejected, `attack_not_rejected:${name}`); attacks += 1 }
reject('identifier_empty', () => validateSpec(materializedR60, { type: 'identifier' }, '', 'identifier_empty'))
reject('identifier_257_utf8_bytes', () => validateSpec(materializedR60, { type: 'identifier' }, 'x'.repeat(257), 'identifier_257'))
reject('pattern_32_hex', () => validateSpec(materializedR60, { type: 'identifier', pattern: '^[0-9a-f]{32}$' }, 'not-32-hex', 'pattern_32_hex'))
reject('base64url_noncanonical_A', () => validateSpec(materializedR60, { type: 'base64url_without_padding' }, 'A', 'base64url_A'))
validateSpec(materializedR60, { type: 'nullable', value_schema: { type: 'identifier', min_utf8_bytes: 1, max_utf8_bytes: 8 } }, 'valid', 'nullable_positive')
reject('nullable_nonnull_recursive_invalid', () => validateSpec(materializedR60, { type: 'nullable', value_schema: { type: 'identifier', pattern: '^[0-9a-f]{32}$' } }, 'valid', 'nullable_recursive'))
reject('generic_string_rejects_number', () => validateSpec(materializedR60, { type: 'string' }, 42, 'generic_string_number'))
reject('identifier_or_unavailable_rejects_number', () => validateSpec(materializedR60, { type: 'identifier_or_UNAVAILABLE_literal' }, 42, 'identifier_or_unavailable_number'))
reject('unicode_scalar_rejects_lone_surrogate', () => validateSpec(materializedR60, { type: 'string', valid_unicode_scalar_only: true }, '\ud800', 'lone_surrogate'))
validateSchema(materializedR60, { properties: { required_value: { type: 'identifier' }, optional_value: { type: 'identifier' } }, required: ['required_value'], optional: ['optional_value'], exact_keys: ['required_value', 'optional_value'], additional_properties: false }, { required_value: 'present' }, 'optional_absent_positive')
reject('optional_schema_extra_key', () => validateSchema(materializedR60, { properties: { required_value: { type: 'identifier' }, optional_value: { type: 'identifier' } }, required: ['required_value'], optional: ['optional_value'], exact_keys: ['required_value', 'optional_value'], additional_properties: false }, { required_value: 'present', extra: 'forged' }, 'optional_extra'))
reject('operation_discriminated_empty_intent', () => validateSpec(materializedR60, { type: 'operation_discriminated_object' }, {}, 'operation_intent_empty', { operation_class: 'use_release' }))
reject('watermark_change_empty_object', () => validateSpec(materializedR60, { type: 'controlling_watermark_change' }, {}, 'watermark_empty'))
reject('discriminated_value_wrong_shape', () => validateSpec(materializedR60, materializedR60.proof_value_schemas.answer_value_by_kind, 'not-an-array', 'discriminated_ranking', { answer_kind: 'ranking' }))
for (const timestamp of ['2023-02-29T00:00:00.000Z', '2031-02-29T00:00:00.000Z', '2031-04-31T00:00:00.000Z']) reject(`impossible_timestamp:${timestamp}`, () => validateSpec(materializedR60, { type: 'canonical_timestamp' }, timestamp, 'impossible_timestamp'))
...
validateSchema(materializedR60, conditionalProbe, { operation_class: 'use_release', result_ref: sha('valid') }, 'conditional_shape_only')
assert(Object.values(materializedR60.authority_operation_conditional_rule_predicate_registry.rows).find(row => row.exact_rule === conditionalProbe.conditional_rules[0]).evaluation_scope === 'requires_resolved_external_artifact_or_transaction_context', 'terminal_consumption_relation_honestly_unproved')
const terminalIdentityFixtures = Object.values(materializedR60.authority_operation_complete_persisted_identity_authorities).filter(row => row.row_schema_ref === 'authoritative_row_schemas.release_authority_terminal_consumptions').map(row => row.schema_valid_linked_row_fixture)
...
for (const fixture of terminalIdentityFixtures) {
  const decoded = decodeFixture(materializedR60, fixture, 'terminal_valid_until_absent')
  assert(!Object.hasOwn(decoded.payload, 'valid_until'), 'terminal_valid_until_must_be_absent')
}
reject('terminal_valid_until_present', () => { const payload = decodeFixture(materializedR60, terminalIdentityFixtures[0], 'terminal_valid_until_attack_source').payload; payload.valid_until = payload.consumed_at; decodeFixture(materializedR60, resealFixture(terminalIdentityFixtures[0], payload), 'terminal_valid_until_attack') })
...
const index = materializedR60.authority_operation_artifact_fingerprint_derivation_authority.sole_active_identity_index
const identityOf = role => index.find(row => row.native_identity_role === role)
function authorityAttack(name, row, mutate) { reject(name, () => { const authority = structuredClone(get(materializedR60, row.exact_authority_ref)); mutate(authority); validateIdentity(materializedR60, row, authority) }) }
...
for (const row of index.filter(item => item.native_identity_role === 'declared_row_version')) reject(`row_version_self_reference:${row.identity_kind}`, () => { const authority = structuredClone(get(materializedR60, row.exact_authority_ref)); authority.executable_native_derivation_evidence.selected_native_derivation.exact_preimage.ordered_complete_mutable_authority_fields.push({ field: row.identity_field, value: authority.executable_native_derivation_evidence.expected_identity }); validateIdentity(materializedR60, row, authority) })
const sessionHold = index.find(item => item.identity_field === 'session_hold_evidence_fingerprint_or_unavailable')
reject('session_hold_companion_splice', () => { const authority = structuredClone(get(materializedR60, sessionHold.exact_authority_ref)); authority.executable_native_derivation_evidence.joint_source_target_fixture.target_native_fingerprint = sha('splice'); validateIdentity(materializedR60, sessionHold, authority) })
reject('fixture_malformed_length', () => { const row = identityOf('canonical_row_bytes_content_address'), authority = structuredClone(get(materializedR60, row.exact_authority_ref)); authority.schema_valid_linked_row_fixture.canonical_row_bytes_b64url = authority.schema_valid_linked_row_fixture.canonical_row_bytes_b64url.slice(2); validateIdentity(materializedR60, row, authority) })
reject('fixture_sha_mismatch', () => { const row = identityOf('schema_declared_fingerprint'), authority = structuredClone(get(materializedR60, row.exact_authority_ref)); authority.schema_valid_linked_row_fixture.canonical_row_bytes_sha256 = sha('wrong'); validateIdentity(materializedR60, row, authority) })
reject('fixture_ref_mismatch', () => { const row = index.find(item => item.native_identity_role === 'declared_content_address' && item.identity_field === 'artifact_ref'), authority = structuredClone(get(materializedR60, row.exact_authority_ref)); authority.executable_native_derivation_evidence.expected_identity = sha('wrong'); validateIdentity(materializedR60, row, authority) })

const fixtureRecords = []
for (const row of index) {
  const authority = get(materializedR60, row.exact_authority_ref), joint = authority.executable_native_derivation_evidence.joint_source_target_fixture
...
const interventionDecoded = decodeFixture(materializedR60, interventionRecord.fixture, 'intervention_positive')
reject('intervention_question_null_contract', () => { const payload = structuredClone(interventionDecoded.payload); payload.question_contract = null; payload.question_contract_fingerprint = null; decodeFixture(materializedR60, resealFixture(interventionRecord.fixture, payload), 'intervention_question_null') })
reject('intervention_payload_length_zero', () => { const payload = structuredClone(interventionDecoded.payload); payload.payload_byte_length = 0; decodeFixture(materializedR60, resealFixture(interventionRecord.fixture, payload), 'intervention_length_zero') })
...
function resealInterventionQuestion(payload, encodedQuestion = null) {
  payload.question_contract.question_contract_fingerprint = declaredTestHash(materializedR60.fingerprint_schemas.question_contract, payload.question_contract)
...
decodeFixture(materializedR60, resealInterventionQuestion(paddedDisplayPayload), 'question_padded_display_preserves_exact_bytes')
reject('question_blank_display', () => { const payload = structuredClone(interventionDecoded.payload); payload.question_contract.visible_wording = '   '; decodeFixture(materializedR60, resealInterventionQuestion(payload), 'question_blank_display') })
reject('intervention_question_arbitrary_schema_version', () => { const payload = structuredClone(interventionDecoded.payload); payload.payload_schema_version = 'invented.schema.v99'; payload.atom_content_fingerprint = declaredTestHash(materializedR60.authoritative_semantic_fingerprint_schemas.intervention_atoms, payload); decodeFixture(materializedR60, resealFixture(interventionRecord.fixture, payload), 'intervention_question_arbitrary_schema_version') })
reject('intervention_question_cross_kind_schema_version', () => { const payload = structuredClone(interventionDecoded.payload); payload.payload_schema_version = materializedR60.authority_operation_intervention_inner_payload_schemas.session.schema_version; payload.atom_content_fingerprint = declaredTestHash(materializedR60.authoritative_semantic_fingerprint_schemas.intervention_atoms, payload); decodeFixture(materializedR60, resealFixture(interventionRecord.fixture, payload), 'intervention_question_cross_kind_schema_version') })
const sessionInterventionAuthority = materializedR60.authority_operation_complete_persisted_identity_authorities.identity_0043
...
const sessionInterventionDecoded = decodeFixture(materializedR60, sessionInterventionRecord.fixture, 'intervention_session_positive')
assert(sessionInterventionDecoded.payload.atom_kind === 'session' && sessionInterventionDecoded.payload.payload_schema_version === materializedR60.authority_operation_intervention_inner_payload_schemas.session.schema_version, 'intervention_session_selected_version')
reject('intervention_session_arbitrary_schema_version', () => { const payload = structuredClone(sessionInterventionDecoded.payload); payload.payload_schema_version = 'invented.session.schema.v99'; payload.atom_content_fingerprint = declaredTestHash(materializedR60.authoritative_semantic_fingerprint_schemas.intervention_atoms, payload); decodeFixture(materializedR60, resealFixture(sessionInterventionRecord.fixture, payload), 'intervention_session_arbitrary_schema_version') })
reject('intervention_session_cross_kind_schema_version', () => { const payload = structuredClone(sessionInterventionDecoded.payload); payload.payload_schema_version = materializedR60.proof_value_schemas.question_contract.schema_version; payload.atom_content_fingerprint = declaredTestHash(materializedR60.authoritative_semantic_fingerprint_schemas.intervention_atoms, payload); decodeFixture(materializedR60, resealFixture(sessionInterventionRecord.fixture, payload), 'intervention_session_cross_kind_schema_version') })

const questionHonestExitSpec = materializedR60.proof_value_schemas.question_contract.properties.honest_exits
validateSpec(materializedR60, questionHonestExitSpec, ['defer', 'premise_wrong', 'refuse', 'unknown'], 'question_honest_exits_positive')
reject('question_honest_exits_exact_member_removed', () => validateSpec(materializedR60, questionHonestExitSpec, ['defer', 'premise_wrong', 'refuse'], 'question_honest_exits_member_removed'))
reject('question_honest_exits_duplicate', () => validateSpec(materializedR60, questionHonestExitSpec, ['defer', 'premise_wrong', 'refuse', 'unknown', 'unknown'], 'question_honest_exits_duplicate'))
reject('ordered_identifier_array_scalar', () => validateSpec(materializedR60, { type: 'ordered_identifier_array' }, 42, 'ordered_identifier_array_scalar'))
reject('ordered_identifier_array_reverse', () => validateSpec(materializedR60, { type: 'ordered_identifier_array' }, ['b', 'a'], 'ordered_identifier_array_reverse'))
reject('unicode_sorted_unique_identifier_array_scalar', () => validateSpec(materializedR60, { type: 'unicode_sorted_unique_identifier_array' }, 42, 'unicode_sorted_identifier_array_scalar'))
reject('unicode_sorted_unique_identifier_array_duplicate', () => validateSpec(materializedR60, { type: 'unicode_sorted_unique_identifier_array' }, ['a', 'a'], 'unicode_sorted_identifier_array_duplicate'))
reject('ordering_unsigned_utf8', () => validateOrdering(materializedR60, { ordered_by: 'unsigned_utf8' }, ['b', 'a'], {}, 'ordering_unsigned_utf8'))
reject('ordering_effect_key', () => validateOrdering(materializedR60, { ordered_by: 'unsigned_utf8_effect_key' }, [{ effect_key: 'b' }, { effect_key: 'a' }], {}, 'ordering_effect_key'))
reject('ordering_structured_watermark', () => validateOrdering(materializedR60, { ordered_by: 'kind_class_then_base_kind_or_control_id_unsigned_utf8' }, [{ kind_class: 'applicable_control', control_id: 'b' }, { kind_class: 'applicable_control', control_id: 'a' }], {}, 'ordering_structured_watermark'))
reject('ordering_route_enum', () => validateOrdering(materializedR60, { ordered_by: 'reuse_enrich_ask_session' }, [{ route: 'enrich' }, { route: 'reuse' }], {}, 'ordering_route_enum'))
reject('ordering_outbox_failure_enum', () => validateSpec(materializedR60, materializedR60.outbox.invocation_aborted_before_provider_schema.properties.failed_nonlease_final_recheck_codes, ['payload_changed', 'operation_authority_changed'], 'ordering_outbox_failure_enum'))
const evidenceRefs = ['evidence_a', 'evidence_b'], evidenceFingerprints = evidenceRefs.map(ref => sha(`resolved:${ref}`)), resolvedEvidencePairs = evidenceRefs.map((evidence_ref, index) => ({ evidence_ref, evidence_fingerprint: evidenceFingerprints[index] }))
validateOrdering(materializedR60, { ordered_by: 'matching_evidence_refs' }, evidenceFingerprints, { evidence_refs: evidenceRefs, resolved_evidence_pairs: resolvedEvidencePairs }, 'ordering_matching_evidence_positive')
reject('ordering_matching_evidence_fingerprint_swap', () => validateOrdering(materializedR60, { ordered_by: 'matching_evidence_refs' }, [...evidenceFingerprints].reverse(), { evidence_refs: evidenceRefs, resolved_evidence_pairs: resolvedEvidencePairs }, 'ordering_matching_evidence_fingerprint_swap'))
reject('ordering_matching_evidence_missing_pair', () => validateOrdering(materializedR60, { ordered_by: 'matching_evidence_refs' }, evidenceFingerprints, { evidence_refs: evidenceRefs, resolved_evidence_pairs: resolvedEvidencePairs.slice(0, 1) }, 'ordering_matching_evidence_missing_pair'))
reject('ordering_matching_evidence_ref_substitute', () => validateOrdering(materializedR60, { ordered_by: 'matching_evidence_refs' }, evidenceFingerprints, { evidence_refs: evidenceRefs, resolved_evidence_pairs: [{ ...resolvedEvidencePairs[0], evidence_ref: 'evidence_substitute' }, resolvedEvidencePairs[1]] }, 'ordering_matching_evidence_ref_substitute'))
reject('ordering_matching_evidence_duplicate_ref', () => validateOrdering(materializedR60, { ordered_by: 'matching_evidence_refs' }, evidenceFingerprints, { evidence_refs: evidenceRefs, resolved_evidence_pairs: [resolvedEvidencePairs[0], { ...resolvedEvidencePairs[1], evidence_ref: resolvedEvidencePairs[0].evidence_ref }] }, 'ordering_matching_evidence_duplicate_ref'))
reject('ordering_unknown_authority', () => validateOrdering(materializedR60, { ordered_by: 'invented_order' }, ['a'], {}, 'ordering_unknown_authority'))
...
reject('outbox_failed_code_reverse_resealed', () => { const decoded = decodeFixture(materializedR60, outboxAbortRecord.fixture, 'outbox_abort_attack_source'), payload = structuredClone(decoded.payload), schema = materializedR60.outbox.invocation_aborted_before_provider_schema; payload.failed_nonlease_final_recheck_codes = ['payload_changed', 'operation_authority_changed']; payload.abort_reason = 'payload_changed'; payload.abort_fingerprint = declaredTestHash(get(materializedR60, schema.fingerprint_ref), payload); decodeFixture(materializedR60, resealFixture(outboxAbortRecord.fixture, payload), 'outbox_failed_code_reverse') })
const bootstrapWrapper = materializedR60.authority_operation_replay_restart_fixtures.fixtures.map(fixture => fixture.artifact_store_by_role?.proof?.stored_row_value).find(row => row?.canonical_schema_ref === 'case_session_root_bootstrap_proof_schema')
...
validateSchema(materializedR60, materializedR60.case_session_root_bootstrap_proof_schema, bootstrapPayload, 'bootstrap_positive')
reject('bootstrap_signer_alias', () => { const payload = structuredClone(bootstrapPayload); payload.signer_2_ref = payload.signer_1_ref; validateSchema(materializedR60, materializedR60.case_session_root_bootstrap_proof_schema, payload, 'bootstrap_alias') })
const enumRefOccurrences = []
for (const record of fixtureRecords) {
  const decoded = decodeFixture(materializedR60, record.fixture, `enum_ref_inventory:${record.row.identity_kind}:${record.kind}`)
...
for (const [probeIndex, probe] of enumRefOccurrences.slice(0, 8).entries()) reject(`enum_ref_${probeIndex + 1}`, () => {
  const payload = structuredClone(probe.decoded.payload); payload[probe.field] = `invented_${probe.spec.enum_ref}`; decodeFixture(materializedR60, resealFixture(probe.record.fixture, payload), `enum_ref_attack:${probeIndex}`)
})
const literalOccurrences = []
const minItemOccurrences = []
for (const record of fixtureRecords) {
  const decoded = decodeFixture(materializedR60, record.fixture, `attack_inventory:${record.row.identity_kind}:${record.kind}`)
...
for (const [probeIndex, probe] of literalOccurrences.slice(0, 148).entries()) reject(`literal_or_const_${probeIndex + 1}`, () => {
  const payload = structuredClone(probe.decoded.payload); payload[probe.field] = wrongLiteral(probe.spec, payload[probe.field]); decodeFixture(materializedR60, resealFixture(probe.record.fixture, payload), `literal_attack:${probeIndex}`)
})
...
for (const [probeIndex, probe] of minItemOccurrences.entries()) reject(`min_items_${probeIndex + 1}`, () => {
  const payload = structuredClone(probe.decoded.payload); payload[probe.field] = []; decodeFixture(materializedR60, resealFixture(probe.record.fixture, payload), `min_items_attack:${probeIndex}`)
})
...
const ordinaryDecoded = decodeFixture(materializedR60, ordinaryFixture.fixture, 'ordinary_target_probe')
const targetProbes = [
...
for (const probe of fixtureRecords.filter(record => record.kind === 'target')) {
  const decoded = decodeFixture(materializedR60, probe.fixture, `target_literal_inventory:${probe.row.identity_kind}`)
...
for (const [probeIndex, probe] of targetProbes.entries()) reject(`target_literal_${probeIndex + 1}`, () => {
  const payload = structuredClone(probe.decoded.payload); payload[probe.field] = probe.value; decodeFixture(materializedR60, resealFixture(probe.record.fixture, payload), `target_literal_attack:${probeIndex}`)
})

const jointRows = index.filter(row => get(materializedR60, row.exact_authority_ref).executable_native_derivation_evidence.joint_source_target_fixture)
...
for (const identityId of ['identity_0929', 'identity_0958', 'identity_0973']) {
  ...
  reject(`${identityId}_shared_fixture_dereference`, () => { const candidate = structuredClone(materializedR60), authority = get(candidate, row.exact_authority_ref), shared = get(candidate, authority.executable_native_derivation_evidence.complete_joint_fixture_ref); shared.target_native_fingerprint = sha(`splice:${identityId}`); validateIdentity(candidate, row) })
  reject(`${identityId}_specialized_mapping_splice`, () => { const authority = structuredClone(get(materializedR60, row.exact_authority_ref)); authority.executable_native_derivation_evidence.authority_companion_target_mapping.target_value = sha(`splice:${identityId}:mapping`); validateIdentity(materializedR60, row, authority) })
}
reject('cross_authority_complete_joint_splice', () => { const candidate = structuredClone(materializedR60), first = index.find(item => item.exact_authority_ref.endsWith('.identity_0929')), second = index.find(item => item.exact_authority_ref.endsWith('.identity_0958')), authority = get(candidate, first.exact_authority_ref), foreign = get(candidate, second.exact_authority_ref).executable_native_derivation_evidence.complete_joint_fixture_ref; authority.executable_native_derivation_evidence.complete_joint_fixture_ref = foreign; authority.executable_native_derivation_evidence.joint_source_target_fixture.complete_joint_fixture_ref = foreign; validateIdentity(candidate, first) })
assert(Object.values(materializedR60.authority_operation_conditional_rule_predicate_registry.rows).some(row => row.rule_id === 'rule_0077' && row.evaluation_scope === 'requires_resolved_external_artifact_or_transaction_context'), 'committed_use_release_terminal_ref_not_counted_as_proved')
const ordinaryComplete = Object.entries(materializedR60.authority_operation_complete_joint_equality_fixtures.rows).find(([, row]) => row.equality_source_schema_ref === 'authority_operation_hold_store.row_union' && row.equality_source_schema_variant === 'ordinary_single_proof' && row.equality_source_field === 'result_ref' && row.declared_companion_bytes_fields.some(item => item.field === 'result_bytes_sha256') && row.declared_companion_fingerprint_fields.some(item => item.field === 'result_fingerprint'))
assert(ordinaryComplete, 'ordinary_complete_result_fixture')
reject('ordinary_result_companions_cannot_be_vacuous', () => { const candidate = structuredClone(materializedR60); const row = candidate.authority_operation_complete_joint_equality_fixtures.rows[ordinaryComplete[0]]; row.declared_companion_bytes_fields = []; row.declared_companion_fingerprint_fields = []; row.declared_companion_bytes_count = 0; row.declared_companion_fingerprint_count = 0; validateCompleteJointFixtures(candidate) })
reject('ordinary_result_same_target_bytes_splice', () => { const candidate = structuredClone(materializedR60), row = candidate.authority_operation_complete_joint_equality_fixtures.rows[ordinaryComplete[0]], decoded = decodeFixture(candidate, row.source_fixture, 'ordinary_splice_source'), payload = structuredClone(decoded.payload); payload.result_bytes_sha256 = sha('different_target'); row.source_fixture = resealFixture(row.source_fixture, payload); validateCompleteJointFixtures(candidate) })
reject('ordinary_result_same_target_fingerprint_splice', () => { const candidate = structuredClone(materializedR60), row = candidate.authority_operation_complete_joint_equality_fixtures.rows[ordinaryComplete[0]], decoded = decodeFixture(candidate, row.source_fixture, 'ordinary_splice_fp'), payload = structuredClone(decoded.payload); payload.result_fingerprint = sha('different_target'); row.source_fixture = resealFixture(row.source_fixture, payload); validateCompleteJointFixtures(candidate) })
...
const selectorIds = Object.keys(materializedR60.authority_operation_internal_reference_target_selectors.rows)
function selectorAttack(name, selectorId, mutate) { reject(name, () => { const selector = structuredClone(materializedR60.authority_operation_internal_reference_target_selectors.rows[selectorId]); mutate(selector); validateSelector(materializedR60, selector) }) }
...
reject('conditional_exclusion_substituted_identity', () => { const candidate = structuredClone(materializedR60), coverage = candidate.authority_operation_conditional_fixture_coverage; coverage.semantic_or_live_unproved_identity_exclusions[0].identity_id = 'identity_invented'; validateConditionalAuthority(candidate) })
const nestedQuestionExclusion = materializedR60.authority_operation_conditional_fixture_coverage.semantic_or_live_unproved_identity_exclusions.find(row => row.identity_id === 'identity_0042')
...
reject('manifest_identity_mutation', () => { const contract = structuredClone(materializedR60); contract.authority_operation_native_identity_applicability_report.exact_native_identity_total += 1; validateManifest(contract) })

console.log(`ok: R60 exact; ${attacks} attacks; ${baseline.identities.total}/1229 native identities; ${jointRows.length} linked identities share ${materializedR60.authority_operation_complete_joint_equality_fixtures.exact_row_count} complete equality fixtures; ${baseline.conditional.local}+${baseline.conditional.contexts} executed conditional rules and ${baseline.conditional.unproved} honest semantic/live exclusions; ${baseline.selectors} selectors over ${baseline.contexts} source-schema-valid contexts; ${baseline.refs} semantic refs; ${baseline.manifest} manifest rows; frozen R59 preserved`)
