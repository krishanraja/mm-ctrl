...
const directory = 'project-documentation/ctrl-evolution/g24-predicate-authority-r88'
...
const expectedTopKeys = {
  effective_resolution: ['schema_version', 'module_id', 'semantic_base', 'imported_exact_sections', 'r88_replacement_sections', 'inheritance_rule', 'unknown_or_conflicting_section', 'authority_still_closed'],
...
check('accepted final registry schemas closed', same(correction.accepted_final_registry_required, ['registry_id', 'records']) && Array.isArray(correction.accepted_final_record_required) && correction.accepted_final_record_required.length > 10 && Array.isArray(correction.answer_state_repair_rules) && correction.answer_state_repair_rules.length >= 4)
...
function validateAnswerStateTransitionEvent(event, record, predecessor) {
  ...
  const previousMatches = predecessor === null
    ...
    : event.prior_state_id === predecessor.answer_state_id && event.prior_state_version === predecessor.state_version && event.prior_state_fingerprint === predecessor.state_fingerprint && event.issuer_sequence === predecessor.issuer_sequence + 1 && ['supersede', 'correct', 'withdraw'].includes(event.action)
  const answerTime = answer.answered_at ?? answer.confirmed_at
  const actionMatches = event.action === 'admit' ? event.standing === 'current' : event.action === 'withdraw' ? event.standing === 'withdrawn' && predecessor !== null && event.answer_ref === predecessor.current_answer_ref : ['supersede', 'correct'].includes(event.action) && event.standing === 'current'
  ...
  try {
    ...
    if (process.env.R88_DEBUG && !(previousMatches && actionMatches && recordMatches && signatureValid)) console.error('answer transition invalid', event.event_id, { previousMatches, actionMatches, recordMatches, signatureValid, answerTime, authorized_at: event.authorized_at })
...
const answerAuthorityService = (() => {
  ...
  const advanceForConformance = (routeRef, questionId, answerRef, action = 'correct') => {
    ...
    const headMeta = snapshot.history_heads.find(record => record.route_ref === routeRef && record.question_id === questionId), prior = store.records.find(record => record.answer_state_id === headMeta?.head_state_id)
    ...
    next.state_fingerprint = fingerprint(next, 'state_fingerprint'); store.records.push(next); store.store_version = `answer-state-store-v${next.issuer_sequence}`; store.store_fingerprint = fingerprint(store, 'store_fingerprint')
    const group = store.records.filter(record => record.route_ref === routeRef && record.question_id === questionId).sort((a, b) => a.issuer_sequence - b.issuer_sequence)
...
function validateFreeConfirmation(event, receipt, question, render, interpretation, interpretationRender) {
  const reject = reason => { if (process.env.R88_DEBUG) console.error('free confirmation reject', reason); return false }
  ...
  if (Object.values(bindingChecks).includes(false)) { if (process.env.R88_DEBUG) console.error(bindingChecks); return reject('binding') }
  const unsigned = clone(event); delete unsigned.issuer_signature_base64
...
function buildFinalReceipt(transitionId, predicateProof) {
  ...
  const authorityProofs = humans.map(human => ({ named_human_id: human, authentication_event_ref: human === 'krish' ? 'auth-r88-krish' : 'auth-r88-leader', authority_event_ref: `final-authority-${transitionId}-${human}` }))
...
if (process.env.R88_EMIT_FINAL_UNSIGNED === '1') {
  ...
  console.log(`R88_UNSIGNED_FINALS=${JSON.stringify(emitted)}`)
  process.exit(0)
}
function validateFinalAuthorityEvent(event, proof, receipt) {
  const reject = reason => { if (process.env.R88_DEBUG) console.error('final authority reject', reason, proof?.authority_event_ref); return false }
...
const acceptedFinalService = (() => {
  ...
  const challengeAnswerHead = (prior, next, transition) => {
    for (const record of records.filter(value => value.answer_state_head_fingerprints.includes(prior.state_fingerprint))) {
      if (repairs.some(receipt => receipt.dependency_use_ref === record.accepted_final_id && receipt.dependency_fingerprint === prior.state_fingerprint)) continue
      record.standing = 'challenged'; record.current_steering_eligibility = false; record.record_fingerprint = fingerprint(record, 'record_fingerprint')
      const receipt = { receipt_type: 'ctrl.answer-state-repair.v1', domain_separator: trust.exact_constants.correction_domain_separator, repair_id: `repair-${record.accepted_final_id}-${next.answer_state_id}`, graph_head: `answer-state-graph-${record.accepted_final_id}`, graph_epoch: next.issuer_sequence, dependency_set_seal: hashValue(record.answer_state_head_fingerprints), subject_id: record.subject_id, case_id: record.case_id, purpose_id: record.purpose_id, permitted_audience: record.permitted_audience, authority_bundle_fingerprint: authorityBundle, trigger_ref: transition.event_id, challenged_proof_ref: record.predicate_proof_fingerprint, dependency_id: `answer-state:${next.route_ref}|${next.question_id}`, dependency_fingerprint: prior.state_fingerprint, edge_kind: 'content_dependency', dependency_use_ref: record.accepted_final_id, affected_artifact_ref: record.final_authority_receipt_fingerprint, repair_action: 'require_human_review', history_preserved_ref: record.history_preserved_ref, descendant_block_ref: `blocked-descendants-${record.accepted_final_id}`, fresh_reopen_authority_ref: null, terminal_status: 'review_required', current_steering_eligibility: false, resulting_version_ref: next.state_fingerprint, issued_at: transition.authorized_at, receipt_fingerprint: '' }
      receipt.receipt_fingerprint = fingerprint(receipt, 'receipt_fingerprint'); repairs.push(receipt)
    }
  }
  return Object.freeze({ resolve, commit, challengeAnswerHead, records: () => clone(records), repairs: () => clone(repairs), resetForConformance: () => { records = []; repairs = [] } })
})()
...
}
{
  ...
  check('post-final signed correction creates a new valid proof candidate', replaceProofNormativeOptions(competing, selections, true) && validatePredicate(competing.proof, competing.evidence) === 'satisfied')
  checkEq('consumed old proof is challenged by current answer change', validatePredicate(accepted.proof, accepted.evidence), 'answer_state_invalid')
  ...
  const acceptedRecord = acceptedFinalService.records()[0], repairs = acceptedFinalService.repairs()
  check('accepted result remains historical but cannot currently steer', exactKeys(acceptedRecord, correction.accepted_final_record_required) && acceptedRecord.standing === 'challenged' && acceptedRecord.current_steering_eligibility === false && fingerprint(acceptedRecord, 'record_fingerprint') === acceptedRecord.record_fingerprint)
...
}
{
  ...
  if (head) {
    ...
    checkEq('withdrawn consumed answer invalidates current proof', validatePredicate(accepted.proof, accepted.evidence), 'answer_state_invalid')
    ...
    check('withdrawal emits one blocking repair path', acceptedFinalService.records()[0].standing === 'challenged' && acceptedFinalService.repairs().length === 1 && acceptedFinalService.repairs()[0].terminal_status === 'review_required')
  }
}
...
function validatePredicate(proof, evidence) {
  ...
  for (const result of proof.dependency_results) {
    ...
    if (expectedKind === 'profile') {
      ...
      if (profileResult !== 'valid') { if (process.env.R88_DEBUG) console.error('predicate profile invalid', result.dependency_id, profileResult); return 'evidence_invalid' }
...
if (failures.length) {
  console.error(`R88 independent conformance failed (${failures.length})`)
  for (const failure of failures) console.error(`- ${failure}`)
  process.exit(1)
}
console.log(`ok: R88 end-to-end bound contract; ${imports.transition_catalogue.length}/13 predicate and final transition paths; ${vectorGroups.reduce((sum, group) => sum + vectors[group].length, 0)} executable vectors; ${totalityProbeCount} generated totality probes`)
