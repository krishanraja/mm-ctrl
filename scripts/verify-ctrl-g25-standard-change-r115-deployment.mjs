...
const manifestPath = resolve(
  root,
  "project-documentation/ctrl-evolution/g25-standard-change-candidate-pipeline-r115-deployment-manifest.json",
);
...
const fail = (message) => {
  throw new Error(`r115_deployment_identity_failed:${message}`);
};
...
const selectInventory = (functions) =>
  manifest.functions
    .map((expected) => {
...
for (const expected of manifest.functions) {
  ...
  const scratch = mkdtempSync(join(tmpdir(), `ctrl-r115-${expected.slug}-`));
  try {
    runNpx(
      [
        "supabase",
    ...
    if (basename(scratch).startsWith(`ctrl-r115-${expected.slug}-`)) {
...
process.stdout.write(
  `${JSON.stringify({
    schema_version: "ctrl.g25.standard-change-deployment-identity.r115.v1",
    ...
    downloaded_sources: downloadedSources.sort((left, right) =>
