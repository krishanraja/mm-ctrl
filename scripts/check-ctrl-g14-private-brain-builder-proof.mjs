...
const url = process.argv[2] || 'http://127.0.0.1:4189/g14-private-brain-builder-proof-r1.html';
...
for (const viewport of [{ name: 'mobile', width: 390, height: 844 }, { name: 'desktop', width: 1440, height: 900 }]) {
  ...
  expect(`${viewport.name}: first frame leads with one intervention`, (await page.locator('#interventionTitle').textContent()).includes('Let her show you what she notices'));
  expect(`${viewport.name}: intervention directive is actionable`, (await page.locator('#interventionDirective').textContent()).includes('two deliberately flawed launch directions'));
  expect(`${viewport.name}: one primary preparation action is visible`, await page.getByRole('button', { name: /Prepare this move/ }).first().isVisible());
  ...
  await page.screenshot({ path: resolve(evidenceDir, `g14-r1-${viewport.name}-now.png`), fullPage: false });

  await page.getByRole('button', { name: 'Why this move?' }).click();
  const change = await page.locator('[data-screen="change"]').textContent();
  expect(`${viewport.name}: change receipt preserves old and new meaning`, change.includes('Weaker now') && change.includes('Supported now') && change.includes('fundamentally distrusts delegation') && change.includes('anticipate the quality signals'));
  expect(`${viewport.name}: change receipt exposes exact source quote`, change.includes("I don't need the team to imitate me"));
  expect(`${viewport.name}: change receipt keeps an unresolved question`, change.includes('Still unresolved') && change.includes('designed to test that, not assume it'));
  await page.waitForTimeout(500);
  await page.screenshot({ path: resolve(evidenceDir, `g14-r1-${viewport.name}-change.png`), fullPage: false });

  await page.getByRole('button', { name: /Use the sharper read/ }).click();
  ...
  await page.waitForTimeout(500);
  await page.screenshot({ path: resolve(evidenceDir, `g14-r1-${viewport.name}-prepare.png`), fullPage: false });
  ...
  expect(`${viewport.name}: private ask produces evidence-bounded guidance`, ask.includes('Protect the distinction') && ask.includes('no durable effect') && ask.includes('3 sources'));
  await page.getByRole('button', { name: 'Close private ask' }).click();

  await page.getByRole('button', { name: 'Back to the move' }).click();
  ...
  await page.waitForTimeout(500);
  await page.screenshot({ path: resolve(evidenceDir, `g14-r1-${viewport.name}-brain.png`), fullPage: false });
  ...
  await page.waitForTimeout(500);
  await page.screenshot({ path: resolve(evidenceDir, `g14-r1-${viewport.name}-customer.png`), fullPage: false });
...
if (failures) {
  console.error(`G14 R1 proof failed ${failures} check(s).`);
  process.exit(1);
}
console.log('G14 R1 Private Brain Builder proof passed.');
