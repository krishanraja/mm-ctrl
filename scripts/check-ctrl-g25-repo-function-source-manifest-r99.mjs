...
check(manifest.summary.local_function_count === 121, "local function inventory no longer includes the candidate standard-change pipeline");
check(manifest.summary.entrypoint_count === 121, "a local function entrypoint is missing");
...
check(manifest.summary.explicit_verify_jwt_true === 87, "expected eighty-seven explicit JWT routes");
...
check(contract.production_metadata.repo_only === 6, "forward repo-only function count drifted");
...
check(contract.open_work.repo_backed_route_authentication_proofs_required === 121, "route-proof queue drifted");
