...
import { materializedR58, materializedR58Output } from './materialize-ctrl-g24-trusted-ingress-r58.mjs'
...
function chooseVariant(schema, value = {}) {
  ...
  const matches = Object.entries(schema.variants).filter(([, candidate]) => Object.entries(candidate.properties ?? {}).every(([field, spec]) => {
    const allowed = allowedValues(materializedR58, spec)
...
function evaluateLocalPredicate(contract, row, payload, label) {
  ...
  if (kind === 'intervention_payload_bytes') { const bytes = Buffer.from(payload.payload_b64url, 'base64url'); ok(bytes.toString('base64url') === payload.payload_b64url && payload.payload_byte_length === bytes.length && payload.atom_content_fingerprint === declared(contract.authoritative_semantic_fingerprint_schemas.intervention_atoms, payload)) }
  else if (kind === 'intervention_question_payload') { let decoded; try { decoded = JSON.parse(Buffer.from(payload.payload_b64url, 'base64url').toString('utf8')) } catch {} const question = payload.atom_kind === 'question'; ok(question === (payload.question_contract !== null && payload.question_contract_fingerprint !== null)); if (question) ok(same(decoded, payload.question_contract) && payload.question_contract_fingerprint === payload.question_contract.question_contract_fingerprint && payload.question_contract_fingerprint === declared(contract.fingerprint_schemas.question_contract, payload.question_contract)) }
  else if (kind === 'intervention_session_nulls') { ok((payload.atom_kind === 'session') === (payload.question_contract === null && payload.question_contract_fingerprint === null)); if (payload.atom_kind === 'session') { let decoded; try { decoded = JSON.parse(Buffer.from(payload.payload_b64url, 'base64url').toString('utf8')) } catch {}; ok(decoded?.atom_kind === 'session' && decoded?.content === payload.content) } }
  ...
  else if (kind === 'effect_text') ok(typeof payload.visible_consequence === 'string' && payload.visible_consequence.trim().length > 0 && (payload.pending_human_owned_proposal === null || payload.pending_human_owned_proposal.trim().length > 0))
  ...
  else if (kind === 'question_effect_texts') ok(payload.answer_effects.every(effect => typeof effect.visible_consequence === 'string' && effect.visible_consequence.trim().length > 0 && (effect.pending_human_owned_proposal === null || effect.pending_human_owned_proposal.trim().length > 0)))
...
function recomputeLinkedTarget(contract, authority, decoded, label) {
  ...
  assert(shared && shared.schema_version === 'ctrl.g24.complete-joint-equality-fixture.r58.v1', `${label}:complete_joint_dereference`)
...
function specValues(spec) { return allowedValues(materializedR58, spec) }
...
function validateSelector(contract, selector) {
  ...
  assert(selector.schema_version === 'ctrl.g24.native-source-schema-intersection-selector.r58.v1' && selector.exact_case_count === expected.length, `selector_count:${selector.selector_id}`)
...
function validateManifest(contract) {
  ...
  const snapshot = Object.fromEntries(paths.map(path => [path, ownedSnapshotR44(get(contract, path))])), snapshotSha = hash({ domain_ascii: 'CTRL-G24-R58-FINAL-SEMANTIC-SOURCE-SNAPSHOT', exact_paths: paths, values: snapshot })
...
function validate(contract) {
  assert(contract.schema_version === 'ctrl.g24.trusted-ingress.r58.effective.v1' && contract.schema_change_manifest.runtime_database_ui_deployment_or_external_action === 'closed', 'r58_boundary')
  ...
  return { identities, ...selectors, complete, conditional, ...manifest }
}

assert(readFileSync(join(root, 'project-documentation/ctrl-evolution/g24-trusted-ingress-contract-r57.json'), 'utf8') === materializedR54Output, 'frozen_R57_exact')
assert(readFileSync(join(root, outputPath), 'utf8') === materializedR58Output, 'exact_R58_materialization')
const baseline = validate(materializedR58)
let attacks = 0
function reject(name, fn) { let rejected = false; try { fn() } catch { rejected = true } assert(rejected, `attack_not_rejected:${name}`); attacks += 1 }
reject('identifier_empty', () => validateSpec(materializedR58, { type: 'identifier' }, '', 'identifier_empty'))
reject('identifier_257_utf8_bytes', () => validateSpec(materializedR58, { type: 'identifier' }, 'x'.repeat(257), 'identifier_257'))
reject('pattern_32_hex', () => validateSpec(materializedR58, { type: 'identifier', pattern: '^[0-9a-f]{32}$' }, 'not-32-hex', 'pattern_32_hex'))
reject('base64url_noncanonical_A', () => validateSpec(materializedR58, { type: 'base64url_without_padding' }, 'A', 'base64url_A'))
validateSpec(materializedR58, { type: 'nullable', value_schema: { type: 'identifier', min_utf8_bytes: 1, max_utf8_bytes: 8 } }, 'valid', 'nullable_positive')
reject('nullable_nonnull_recursive_invalid', () => validateSpec(materializedR58, { type: 'nullable', value_schema: { type: 'identifier', pattern: '^[0-9a-f]{32}$' } }, 'valid', 'nullable_recursive'))
reject('generic_string_rejects_number', () => validateSpec(materializedR58, { type: 'string' }, 42, 'generic_string_number'))
reject('identifier_or_unavailable_rejects_number', () => validateSpec(materializedR58, { type: 'identifier_or_UNAVAILABLE_literal' }, 42, 'identifier_or_unavailable_number'))
reject('unicode_scalar_rejects_lone_surrogate', () => validateSpec(materializedR58, { type: 'string', valid_unicode_scalar_only: true }, '\ud800', 'lone_surrogate'))
validateSchema(materializedR58, { properties: { required_value: { type: 'identifier' }, optional_value: { type: 'identifier' } }, required: ['required_value'], optional: ['optional_value'], exact_keys: ['required_value', 'optional_value'], additional_properties: false }, { required_value: 'present' }, 'optional_absent_positive')
reject('optional_schema_extra_key', () => validateSchema(materializedR58, { properties: { required_value: { type: 'identifier' }, optional_value: { type: 'identifier' } }, required: ['required_value'], optional: ['optional_value'], exact_keys: ['required_value', 'optional_value'], additional_properties: false }, { required_value: 'present', extra: 'forged' }, 'optional_extra'))
reject('operation_discriminated_empty_intent', () => validateSpec(materializedR58, { type: 'operation_discriminated_object' }, {}, 'operation_intent_empty', { operation_class: 'use_release' }))
reject('watermark_change_empty_object', () => validateSpec(materializedR58, { type: 'controlling_watermark_change' }, {}, 'watermark_empty'))
reject('discriminated_value_wrong_shape', () => validateSpec(materializedR58, materializedR58.proof_value_schemas.answer_value_by_kind, 'not-an-array', 'discriminated_ranking', { answer_kind: 'ranking' }))
for (const timestamp of ['2023-02-29T00:00:00.000Z', '2031-02-29T00:00:00.000Z', '2031-04-31T00:00:00.000Z']) reject(`impossible_timestamp:${timestamp}`, () => validateSpec(materializedR58, { type: 'canonical_timestamp' }, timestamp, 'impossible_timestamp'))
...
validateSchema(materializedR58, conditionalProbe, { operation_class: 'use_release', result_ref: sha('valid') }, 'conditional_shape_only')
assert(Object.values(materializedR58.authority_operation_conditional_rule_predicate_registry.rows).find(row => row.exact_rule === conditionalProbe.conditional_rules[0]).evaluation_scope === 'requires_resolved_external_artifact_or_transaction_context', 'terminal_consumption_relation_honestly_unproved')
const terminalIdentityFixtures = Object.values(materializedR58.authority_operation_complete_persisted_identity_authorities).filter(row => row.row_schema_ref === 'authoritative_row_schemas.release_authority_terminal_consumptions').map(row => row.schema_valid_linked_row_fixture)
...
for (const fixture of terminalIdentityFixtures) {
  const decoded = decodeFixture(materializedR58, fixture, 'terminal_valid_until_absent')
  assert(!Object.hasOwn(decoded.payload, 'valid_until'), 'terminal_valid_until_must_be_absent')
}
reject('terminal_valid_until_present', () => { const payload = decodeFixture(materializedR58, terminalIdentityFixtures[0], 'terminal_valid_until_attack_source').payload; payload.valid_until = payload.consumed_at; decodeFixture(materializedR58, resealFixture(terminalIdentityFixtures[0], payload), 'terminal_valid_until_attack') })
...
const index = materializedR58.authority_operation_artifact_fingerprint_derivation_authority.sole_active_identity_index
const identityOf = role => index.find(row => row.native_identity_role === role)
function authorityAttack(name, row, mutate) { reject(name, () => { const authority = structuredClone(get(materializedR58, row.exact_authority_ref)); mutate(authority); validateIdentity(materializedR58, row, authority) }) }
...
for (const row of index.filter(item => item.native_identity_role === 'declared_row_version')) reject(`row_version_self_reference:${row.identity_kind}`, () => { const authority = structuredClone(get(materializedR58, row.exact_authority_ref)); authority.executable_native_derivation_evidence.selected_native_derivation.exact_preimage.ordered_complete_mutable_authority_fields.push({ field: row.identity_field, value: authority.executable_native_derivation_evidence.expected_identity }); validateIdentity(materializedR58, row, authority) })
const sessionHold = index.find(item => item.identity_field === 'session_hold_evidence_fingerprint_or_unavailable')
reject('session_hold_companion_splice', () => { const authority = structuredClone(get(materializedR58, sessionHold.exact_authority_ref)); authority.executable_native_derivation_evidence.joint_source_target_fixture.target_native_fingerprint = sha('splice'); validateIdentity(materializedR58, sessionHold, authority) })
reject('fixture_malformed_length', () => { const row = identityOf('canonical_row_bytes_content_address'), authority = structuredClone(get(materializedR58, row.exact_authority_ref)); authority.schema_valid_linked_row_fixture.canonical_row_bytes_b64url = authority.schema_valid_linked_row_fixture.canonical_row_bytes_b64url.slice(2); validateIdentity(materializedR58, row, authority) })
reject('fixture_sha_mismatch', () => { const row = identityOf('schema_declared_fingerprint'), authority = structuredClone(get(materializedR58, row.exact_authority_ref)); authority.schema_valid_linked_row_fixture.canonical_row_bytes_sha256 = sha('wrong'); validateIdentity(materializedR58, row, authority) })
reject('fixture_ref_mismatch', () => { const row = index.find(item => item.native_identity_role === 'declared_content_address' && item.identity_field === 'artifact_ref'), authority = structuredClone(get(materializedR58, row.exact_authority_ref)); authority.executable_native_derivation_evidence.expected_identity = sha('wrong'); validateIdentity(materializedR58, row, authority) })

const fixtureRecords = []
for (const row of index) {
  const authority = get(materializedR58, row.exact_authority_ref), joint = authority.executable_native_derivation_evidence.joint_source_target_fixture
...
const interventionDecoded = decodeFixture(materializedR58, interventionRecord.fixture, 'intervention_positive')
reject('intervention_question_null_contract', () => { const payload = structuredClone(interventionDecoded.payload); payload.question_contract = null; payload.question_contract_fingerprint = null; decodeFixture(materializedR58, resealFixture(interventionRecord.fixture, payload), 'intervention_question_null') })
reject('intervention_payload_length_zero', () => { const payload = structuredClone(interventionDecoded.payload); payload.payload_byte_length = 0; decodeFixture(materializedR58, resealFixture(interventionRecord.fixture, payload), 'intervention_length_zero') })
...
validateSchema(materializedR58, materializedR58.case_session_root_bootstrap_proof_schema, bootstrapPayload, 'bootstrap_positive')
reject('bootstrap_signer_alias', () => { const payload = structuredClone(bootstrapPayload); payload.signer_2_ref = payload.signer_1_ref; validateSchema(materializedR58, materializedR58.case_session_root_bootstrap_proof_schema, payload, 'bootstrap_alias') })
const enumRefOccurrences = []
for (const record of fixtureRecords) {
  const decoded = decodeFixture(materializedR58, record.fixture, `enum_ref_inventory:${record.row.identity_kind}:${record.kind}`)
...
for (const [probeIndex, probe] of enumRefOccurrences.slice(0, 8).entries()) reject(`enum_ref_${probeIndex + 1}`, () => {
  const payload = structuredClone(probe.decoded.payload); payload[probe.field] = `invented_${probe.spec.enum_ref}`; decodeFixture(materializedR58, resealFixture(probe.record.fixture, payload), `enum_ref_attack:${probeIndex}`)
})
const literalOccurrences = []
const minItemOccurrences = []
for (const record of fixtureRecords) {
  const decoded = decodeFixture(materializedR58, record.fixture, `attack_inventory:${record.row.identity_kind}:${record.kind}`)
...
for (const [probeIndex, probe] of literalOccurrences.slice(0, 148).entries()) reject(`literal_or_const_${probeIndex + 1}`, () => {
  const payload = structuredClone(probe.decoded.payload); payload[probe.field] = wrongLiteral(probe.spec, payload[probe.field]); decodeFixture(materializedR58, resealFixture(probe.record.fixture, payload), `literal_attack:${probeIndex}`)
})
...
for (const [probeIndex, probe] of minItemOccurrences.entries()) reject(`min_items_${probeIndex + 1}`, () => {
  const payload = structuredClone(probe.decoded.payload); payload[probe.field] = []; decodeFixture(materializedR58, resealFixture(probe.record.fixture, payload), `min_items_attack:${probeIndex}`)
})
...
const ordinaryDecoded = decodeFixture(materializedR58, ordinaryFixture.fixture, 'ordinary_target_probe')
const targetProbes = [
...
for (const probe of fixtureRecords.filter(record => record.kind === 'target')) {
  const decoded = decodeFixture(materializedR58, probe.fixture, `target_literal_inventory:${probe.row.identity_kind}`)
...
for (const [probeIndex, probe] of targetProbes.entries()) reject(`target_literal_${probeIndex + 1}`, () => {
  const payload = structuredClone(probe.decoded.payload); payload[probe.field] = probe.value; decodeFixture(materializedR58, resealFixture(probe.record.fixture, payload), `target_literal_attack:${probeIndex}`)
})

const jointRows = index.filter(row => get(materializedR58, row.exact_authority_ref).executable_native_derivation_evidence.joint_source_target_fixture)
...
for (const identityId of ['identity_0929', 'identity_0958', 'identity_0973']) {
  ...
  reject(`${identityId}_shared_fixture_dereference`, () => { const candidate = structuredClone(materializedR58), authority = get(candidate, row.exact_authority_ref), shared = get(candidate, authority.executable_native_derivation_evidence.complete_joint_fixture_ref); shared.target_native_fingerprint = sha(`splice:${identityId}`); validateIdentity(candidate, row) })
  reject(`${identityId}_specialized_mapping_splice`, () => { const authority = structuredClone(get(materializedR58, row.exact_authority_ref)); authority.executable_native_derivation_evidence.authority_companion_target_mapping.target_value = sha(`splice:${identityId}:mapping`); validateIdentity(materializedR58, row, authority) })
}
reject('cross_authority_complete_joint_splice', () => { const candidate = structuredClone(materializedR58), first = index.find(item => item.exact_authority_ref.endsWith('.identity_0929')), second = index.find(item => item.exact_authority_ref.endsWith('.identity_0958')), authority = get(candidate, first.exact_authority_ref), foreign = get(candidate, second.exact_authority_ref).executable_native_derivation_evidence.complete_joint_fixture_ref; authority.executable_native_derivation_evidence.complete_joint_fixture_ref = foreign; authority.executable_native_derivation_evidence.joint_source_target_fixture.complete_joint_fixture_ref = foreign; validateIdentity(candidate, first) })
assert(Object.values(materializedR58.authority_operation_conditional_rule_predicate_registry.rows).some(row => row.rule_id === 'rule_0077' && row.evaluation_scope === 'requires_resolved_external_artifact_or_transaction_context'), 'committed_use_release_terminal_ref_not_counted_as_proved')
const ordinaryComplete = Object.entries(materializedR58.authority_operation_complete_joint_equality_fixtures.rows).find(([, row]) => row.equality_source_schema_ref === 'authority_operation_hold_store.row_union' && row.equality_source_schema_variant === 'ordinary_single_proof' && row.equality_source_field === 'result_ref' && row.declared_companion_bytes_fields.some(item => item.field === 'result_bytes_sha256') && row.declared_companion_fingerprint_fields.some(item => item.field === 'result_fingerprint'))
assert(ordinaryComplete, 'ordinary_complete_result_fixture')
reject('ordinary_result_companions_cannot_be_vacuous', () => { const candidate = structuredClone(materializedR58); const row = candidate.authority_operation_complete_joint_equality_fixtures.rows[ordinaryComplete[0]]; row.declared_companion_bytes_fields = []; row.declared_companion_fingerprint_fields = []; row.declared_companion_bytes_count = 0; row.declared_companion_fingerprint_count = 0; validateCompleteJointFixtures(candidate) })
reject('ordinary_result_same_target_bytes_splice', () => { const candidate = structuredClone(materializedR58), row = candidate.authority_operation_complete_joint_equality_fixtures.rows[ordinaryComplete[0]], decoded = decodeFixture(candidate, row.source_fixture, 'ordinary_splice_source'), payload = structuredClone(decoded.payload); payload.result_bytes_sha256 = sha('different_target'); row.source_fixture = resealFixture(row.source_fixture, payload); validateCompleteJointFixtures(candidate) })
reject('ordinary_result_same_target_fingerprint_splice', () => { const candidate = structuredClone(materializedR58), row = candidate.authority_operation_complete_joint_equality_fixtures.rows[ordinaryComplete[0]], decoded = decodeFixture(candidate, row.source_fixture, 'ordinary_splice_fp'), payload = structuredClone(decoded.payload); payload.result_fingerprint = sha('different_target'); row.source_fixture = resealFixture(row.source_fixture, payload); validateCompleteJointFixtures(candidate) })
...
const selectorIds = Object.keys(materializedR58.authority_operation_internal_reference_target_selectors.rows)
function selectorAttack(name, selectorId, mutate) { reject(name, () => { const selector = structuredClone(materializedR58.authority_operation_internal_reference_target_selectors.rows[selectorId]); mutate(selector); validateSelector(materializedR58, selector) }) }
...
reject('manifest_identity_mutation', () => { const contract = structuredClone(materializedR58); contract.authority_operation_native_identity_applicability_report.exact_native_identity_total += 1; validateManifest(contract) })

console.log(`ok: R58 exact; ${attacks} attacks; ${baseline.identities.total}/1229 native identities; ${jointRows.length} linked identities share ${materializedR58.authority_operation_complete_joint_equality_fixtures.exact_row_count} complete equality fixtures; ${baseline.conditional.local}+${baseline.conditional.contexts} executed conditional rules and ${baseline.conditional.unproved} honest semantic/live exclusions; ${baseline.selectors} selectors over ${baseline.contexts} source-schema-valid contexts; ${baseline.refs} semantic refs; ${baseline.manifest} manifest rows; frozen R57 preserved`)
