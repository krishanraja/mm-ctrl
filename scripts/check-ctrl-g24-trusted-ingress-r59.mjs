...
import { materializedR59, materializedR59Output } from './materialize-ctrl-g24-trusted-ingress-r59.mjs'
...
function chooseVariant(schema, value = {}) {
  ...
  const matches = Object.entries(schema.variants).filter(([, candidate]) => Object.entries(candidate.properties ?? {}).every(([field, spec]) => {
    const allowed = allowedValues(materializedR59, spec)
...
function evaluateLocalPredicate(contract, row, payload, label) {
  ...
  if (kind === 'intervention_payload_bytes') { const bytes = Buffer.from(payload.payload_b64url, 'base64url'); let decoded; try { decoded = JSON.parse(bytes.toString('utf8')) } catch {}; ok(bytes.toString('base64url') === payload.payload_b64url && payload.payload_byte_length === bytes.length && decoded && bytes.toString('utf8') === canonicalR44(decoded)); if (payload.atom_kind === 'question') validateSchema(contract, contract.proof_value_schemas.question_contract, decoded, `${label}:decoded_question`); else if (payload.atom_kind === 'session') validateSchema(contract, contract.authority_operation_intervention_inner_payload_schemas.session, decoded, `${label}:decoded_session`); else ok(false); ok(payload.atom_content_fingerprint === declared(contract.authoritative_semantic_fingerprint_schemas.intervention_atoms, payload)) }
  ...
  else if (kind === 'question_text') ok(['visible_wording', 'rendered_control_payload', 'material_effect_disclosure', 'visible_changed_consequence', 'visible_unknown_consequence'].every(field => typeof payload[field] === 'string' && payload[field].length > 0 && payload[field] === payload[field].trim()))
...
function recomputeLinkedTarget(contract, authority, decoded, label) {
  ...
  assert(shared && shared.schema_version === 'ctrl.g24.complete-joint-equality-fixture.r59.v1', `${label}:complete_joint_dereference`)
...
function specValues(spec) { return allowedValues(materializedR59, spec) }
...
function validateSelector(contract, selector) {
  ...
  assert(selector.schema_version === 'ctrl.g24.native-source-schema-intersection-selector.r59.v1' && selector.exact_case_count === expected.length, `selector_count:${selector.selector_id}`)
...
function validateManifest(contract) {
  ...
  const snapshot = Object.fromEntries(paths.map(path => [path, ownedSnapshotR44(get(contract, path))])), snapshotSha = hash({ domain_ascii: 'CTRL-G24-R59-FINAL-SEMANTIC-SOURCE-SNAPSHOT', exact_paths: paths, values: snapshot })
...
function validateConditionalAuthority(contract) {
  ...
  for (const [identityId, authority] of Object.entries(contract.authority_operation_complete_persisted_identity_authorities)) {
    const schema = schemaAt(contract, authority.schema_valid_linked_row_fixture.schema_ref, authority.schema_valid_linked_row_fixture.schema_variant)
    const rulesForFixture = schema?.conditional_rules ?? []
    ...
    else expectedExclusions.push({ identity_id: identityId, schema_ref: authority.row_schema_ref, schema_variant: authority.row_schema_variant, exact_unproved_rules: unproved, disposition: 'excluded_from_full_conditional_semantics_claim_until_exact_resolved_external_context_is_materialized' })
  }
...
function validateOrderingAuthority(contract) {
  ...
  assert(occurrences.length === 21 && registry.exact_occurrence_count === 21 && registry.exact_handler_count === 6, 'ordering_inventory_counts')
...
function validate(contract) {
  assert(contract.schema_version === 'ctrl.g24.trusted-ingress.r59.effective.v1' && contract.schema_change_manifest.runtime_database_ui_deployment_or_external_action === 'closed', 'r59_boundary')
...
assert(readFileSync(join(root, 'project-documentation/ctrl-evolution/g24-trusted-ingress-contract-r58.json'), 'utf8') === materializedR54Output, 'frozen_R58_exact')
assert(readFileSync(join(root, outputPath), 'utf8') === materializedR59Output, 'exact_R59_materialization')
const baseline = validate(materializedR59)
let attacks = 0
function reject(name, fn) { let rejected = false; try { fn() } catch { rejected = true } assert(rejected, `attack_not_rejected:${name}`); attacks += 1 }
reject('identifier_empty', () => validateSpec(materializedR59, { type: 'identifier' }, '', 'identifier_empty'))
reject('identifier_257_utf8_bytes', () => validateSpec(materializedR59, { type: 'identifier' }, 'x'.repeat(257), 'identifier_257'))
reject('pattern_32_hex', () => validateSpec(materializedR59, { type: 'identifier', pattern: '^[0-9a-f]{32}$' }, 'not-32-hex', 'pattern_32_hex'))
reject('base64url_noncanonical_A', () => validateSpec(materializedR59, { type: 'base64url_without_padding' }, 'A', 'base64url_A'))
validateSpec(materializedR59, { type: 'nullable', value_schema: { type: 'identifier', min_utf8_bytes: 1, max_utf8_bytes: 8 } }, 'valid', 'nullable_positive')
reject('nullable_nonnull_recursive_invalid', () => validateSpec(materializedR59, { type: 'nullable', value_schema: { type: 'identifier', pattern: '^[0-9a-f]{32}$' } }, 'valid', 'nullable_recursive'))
reject('generic_string_rejects_number', () => validateSpec(materializedR59, { type: 'string' }, 42, 'generic_string_number'))
reject('identifier_or_unavailable_rejects_number', () => validateSpec(materializedR59, { type: 'identifier_or_UNAVAILABLE_literal' }, 42, 'identifier_or_unavailable_number'))
reject('unicode_scalar_rejects_lone_surrogate', () => validateSpec(materializedR59, { type: 'string', valid_unicode_scalar_only: true }, '\ud800', 'lone_surrogate'))
validateSchema(materializedR59, { properties: { required_value: { type: 'identifier' }, optional_value: { type: 'identifier' } }, required: ['required_value'], optional: ['optional_value'], exact_keys: ['required_value', 'optional_value'], additional_properties: false }, { required_value: 'present' }, 'optional_absent_positive')
reject('optional_schema_extra_key', () => validateSchema(materializedR59, { properties: { required_value: { type: 'identifier' }, optional_value: { type: 'identifier' } }, required: ['required_value'], optional: ['optional_value'], exact_keys: ['required_value', 'optional_value'], additional_properties: false }, { required_value: 'present', extra: 'forged' }, 'optional_extra'))
reject('operation_discriminated_empty_intent', () => validateSpec(materializedR59, { type: 'operation_discriminated_object' }, {}, 'operation_intent_empty', { operation_class: 'use_release' }))
reject('watermark_change_empty_object', () => validateSpec(materializedR59, { type: 'controlling_watermark_change' }, {}, 'watermark_empty'))
reject('discriminated_value_wrong_shape', () => validateSpec(materializedR59, materializedR59.proof_value_schemas.answer_value_by_kind, 'not-an-array', 'discriminated_ranking', { answer_kind: 'ranking' }))
for (const timestamp of ['2023-02-29T00:00:00.000Z', '2031-02-29T00:00:00.000Z', '2031-04-31T00:00:00.000Z']) reject(`impossible_timestamp:${timestamp}`, () => validateSpec(materializedR59, { type: 'canonical_timestamp' }, timestamp, 'impossible_timestamp'))
...
validateSchema(materializedR59, conditionalProbe, { operation_class: 'use_release', result_ref: sha('valid') }, 'conditional_shape_only')
assert(Object.values(materializedR59.authority_operation_conditional_rule_predicate_registry.rows).find(row => row.exact_rule === conditionalProbe.conditional_rules[0]).evaluation_scope === 'requires_resolved_external_artifact_or_transaction_context', 'terminal_consumption_relation_honestly_unproved')
const terminalIdentityFixtures = Object.values(materializedR59.authority_operation_complete_persisted_identity_authorities).filter(row => row.row_schema_ref === 'authoritative_row_schemas.release_authority_terminal_consumptions').map(row => row.schema_valid_linked_row_fixture)
...
for (const fixture of terminalIdentityFixtures) {
  const decoded = decodeFixture(materializedR59, fixture, 'terminal_valid_until_absent')
  assert(!Object.hasOwn(decoded.payload, 'valid_until'), 'terminal_valid_until_must_be_absent')
}
reject('terminal_valid_until_present', () => { const payload = decodeFixture(materializedR59, terminalIdentityFixtures[0], 'terminal_valid_until_attack_source').payload; payload.valid_until = payload.consumed_at; decodeFixture(materializedR59, resealFixture(terminalIdentityFixtures[0], payload), 'terminal_valid_until_attack') })
...
const index = materializedR59.authority_operation_artifact_fingerprint_derivation_authority.sole_active_identity_index
const identityOf = role => index.find(row => row.native_identity_role === role)
function authorityAttack(name, row, mutate) { reject(name, () => { const authority = structuredClone(get(materializedR59, row.exact_authority_ref)); mutate(authority); validateIdentity(materializedR59, row, authority) }) }
...
for (const row of index.filter(item => item.native_identity_role === 'declared_row_version')) reject(`row_version_self_reference:${row.identity_kind}`, () => { const authority = structuredClone(get(materializedR59, row.exact_authority_ref)); authority.executable_native_derivation_evidence.selected_native_derivation.exact_preimage.ordered_complete_mutable_authority_fields.push({ field: row.identity_field, value: authority.executable_native_derivation_evidence.expected_identity }); validateIdentity(materializedR59, row, authority) })
const sessionHold = index.find(item => item.identity_field === 'session_hold_evidence_fingerprint_or_unavailable')
reject('session_hold_companion_splice', () => { const authority = structuredClone(get(materializedR59, sessionHold.exact_authority_ref)); authority.executable_native_derivation_evidence.joint_source_target_fixture.target_native_fingerprint = sha('splice'); validateIdentity(materializedR59, sessionHold, authority) })
reject('fixture_malformed_length', () => { const row = identityOf('canonical_row_bytes_content_address'), authority = structuredClone(get(materializedR59, row.exact_authority_ref)); authority.schema_valid_linked_row_fixture.canonical_row_bytes_b64url = authority.schema_valid_linked_row_fixture.canonical_row_bytes_b64url.slice(2); validateIdentity(materializedR59, row, authority) })
reject('fixture_sha_mismatch', () => { const row = identityOf('schema_declared_fingerprint'), authority = structuredClone(get(materializedR59, row.exact_authority_ref)); authority.schema_valid_linked_row_fixture.canonical_row_bytes_sha256 = sha('wrong'); validateIdentity(materializedR59, row, authority) })
reject('fixture_ref_mismatch', () => { const row = index.find(item => item.native_identity_role === 'declared_content_address' && item.identity_field === 'artifact_ref'), authority = structuredClone(get(materializedR59, row.exact_authority_ref)); authority.executable_native_derivation_evidence.expected_identity = sha('wrong'); validateIdentity(materializedR59, row, authority) })

const fixtureRecords = []
for (const row of index) {
  const authority = get(materializedR59, row.exact_authority_ref), joint = authority.executable_native_derivation_evidence.joint_source_target_fixture
...
const interventionDecoded = decodeFixture(materializedR59, interventionRecord.fixture, 'intervention_positive')
reject('intervention_question_null_contract', () => { const payload = structuredClone(interventionDecoded.payload); payload.question_contract = null; payload.question_contract_fingerprint = null; decodeFixture(materializedR59, resealFixture(interventionRecord.fixture, payload), 'intervention_question_null') })
reject('intervention_payload_length_zero', () => { const payload = structuredClone(interventionDecoded.payload); payload.payload_byte_length = 0; decodeFixture(materializedR59, resealFixture(interventionRecord.fixture, payload), 'intervention_length_zero') })
...
function resealInterventionQuestion(payload, encodedQuestion = null) {
  payload.question_contract.question_contract_fingerprint = declaredTestHash(materializedR59.fingerprint_schemas.question_contract, payload.question_contract)
  ...
  payload.atom_content_fingerprint = declaredTestHash(materializedR59.authoritative_semantic_fingerprint_schemas.intervention_atoms, payload)
  return resealFixture(interventionRecord.fixture, payload)
}
reject('intervention_question_forged_shared_fingerprint', () => { const payload = structuredClone(interventionDecoded.payload), forged = sha('forged_question'); payload.question_contract.question_contract_fingerprint = forged; payload.question_contract_fingerprint = forged; const bytes = Buffer.from(canonicalR44(payload.question_contract), 'utf8'); payload.payload_b64url = bytes.toString('base64url'); payload.payload_byte_length = bytes.length; payload.atom_content_fingerprint = declaredTestHash(materializedR59.authoritative_semantic_fingerprint_schemas.intervention_atoms, payload); validateSchema(materializedR59, materializedR59.authoritative_row_schemas.intervention_atoms, payload, 'intervention_forged_question') })
reject('intervention_session_inner_outer_content_mismatch', () => { const payload = structuredClone(interventionDecoded.payload); payload.atom_kind = 'session'; payload.question_contract = null; payload.question_contract_fingerprint = null; const bytes = Buffer.from(canonicalR44({ atom_kind: 'session', content: 'different inner content' }), 'utf8'); payload.payload_b64url = bytes.toString('base64url'); payload.payload_byte_length = bytes.length; payload.atom_content_fingerprint = declaredTestHash(materializedR59.authoritative_semantic_fingerprint_schemas.intervention_atoms, payload); validateSchema(materializedR59, materializedR59.authoritative_row_schemas.intervention_atoms, payload, 'intervention_session_mismatch') })
reject('intervention_question_pretty_printed_inner_bytes', () => { const payload = structuredClone(interventionDecoded.payload); decodeFixture(materializedR59, resealInterventionQuestion(payload, JSON.stringify(payload.question_contract, null, 2)), 'intervention_pretty_inner') })
reject('intervention_question_inner_key_injection', () => { const payload = structuredClone(interventionDecoded.payload), inner = { ...payload.question_contract, injected: true }; const bytes = Buffer.from(canonicalR44(inner), 'utf8'); payload.payload_b64url = bytes.toString('base64url'); payload.payload_byte_length = bytes.length; payload.atom_content_fingerprint = declaredTestHash(materializedR59.authoritative_semantic_fingerprint_schemas.intervention_atoms, payload); decodeFixture(materializedR59, resealFixture(interventionRecord.fixture, payload), 'intervention_inner_key_injection') })
reject('intervention_session_inner_key_injection', () => { const payload = structuredClone(interventionDecoded.payload), inner = { atom_kind: 'session', content: payload.content, injected: true }; payload.atom_kind = 'session'; payload.question_contract = null; payload.question_contract_fingerprint = null; const bytes = Buffer.from(canonicalR44(inner), 'utf8'); payload.payload_b64url = bytes.toString('base64url'); payload.payload_byte_length = bytes.length; payload.atom_content_fingerprint = declaredTestHash(materializedR59.authoritative_semantic_fingerprint_schemas.intervention_atoms, payload); decodeFixture(materializedR59, resealFixture(interventionRecord.fixture, payload), 'intervention_session_key_injection') })
reject('question_answer_effects_reverse_resealed', () => { const payload = structuredClone(interventionDecoded.payload); payload.question_contract.answer_effects.reverse(); decodeFixture(materializedR59, resealInterventionQuestion(payload), 'question_answer_effects_reverse') })
reject('question_visible_consequence_padded', () => { const payload = structuredClone(interventionDecoded.payload); payload.question_contract.answer_effects[0].visible_consequence = ` ${payload.question_contract.answer_effects[0].visible_consequence}`; decodeFixture(materializedR59, resealInterventionQuestion(payload), 'question_visible_consequence_padded') })
reject('question_pending_proposal_padded', () => { const payload = structuredClone(interventionDecoded.payload); payload.question_contract.answer_effects[0].pending_human_owned_proposal = ' padded proposal '; payload.question_contract.answer_effects[0].case_effect = 'stage_pending_proposal'; decodeFixture(materializedR59, resealInterventionQuestion(payload), 'question_pending_proposal_padded') })

const questionHonestExitSpec = materializedR59.proof_value_schemas.question_contract.properties.honest_exits
validateSpec(materializedR59, questionHonestExitSpec, ['defer', 'premise_wrong', 'refuse', 'unknown'], 'question_honest_exits_positive')
reject('question_honest_exits_exact_member_removed', () => validateSpec(materializedR59, questionHonestExitSpec, ['defer', 'premise_wrong', 'refuse'], 'question_honest_exits_member_removed'))
reject('question_honest_exits_duplicate', () => validateSpec(materializedR59, questionHonestExitSpec, ['defer', 'premise_wrong', 'refuse', 'unknown', 'unknown'], 'question_honest_exits_duplicate'))
reject('ordered_identifier_array_scalar', () => validateSpec(materializedR59, { type: 'ordered_identifier_array' }, 42, 'ordered_identifier_array_scalar'))
reject('ordered_identifier_array_reverse', () => validateSpec(materializedR59, { type: 'ordered_identifier_array' }, ['b', 'a'], 'ordered_identifier_array_reverse'))
reject('unicode_sorted_unique_identifier_array_scalar', () => validateSpec(materializedR59, { type: 'unicode_sorted_unique_identifier_array' }, 42, 'unicode_sorted_identifier_array_scalar'))
reject('unicode_sorted_unique_identifier_array_duplicate', () => validateSpec(materializedR59, { type: 'unicode_sorted_unique_identifier_array' }, ['a', 'a'], 'unicode_sorted_identifier_array_duplicate'))
reject('ordering_unsigned_utf8', () => validateOrdering(materializedR59, { ordered_by: 'unsigned_utf8' }, ['b', 'a'], {}, 'ordering_unsigned_utf8'))
reject('ordering_effect_key', () => validateOrdering(materializedR59, { ordered_by: 'unsigned_utf8_effect_key' }, [{ effect_key: 'b' }, { effect_key: 'a' }], {}, 'ordering_effect_key'))
reject('ordering_structured_watermark', () => validateOrdering(materializedR59, { ordered_by: 'kind_class_then_base_kind_or_control_id_unsigned_utf8' }, [{ kind_class: 'applicable_control', control_id: 'b' }, { kind_class: 'applicable_control', control_id: 'a' }], {}, 'ordering_structured_watermark'))
reject('ordering_route_enum', () => validateOrdering(materializedR59, { ordered_by: 'reuse_enrich_ask_session' }, [{ route: 'enrich' }, { route: 'reuse' }], {}, 'ordering_route_enum'))
reject('ordering_outbox_failure_enum', () => validateSpec(materializedR59, materializedR59.outbox.invocation_aborted_before_provider_schema.properties.failed_nonlease_final_recheck_codes, ['payload_changed', 'operation_authority_changed'], 'ordering_outbox_failure_enum'))
reject('ordering_matching_evidence_refs', () => validateOrdering(materializedR59, { ordered_by: 'matching_evidence_refs' }, [sha('first'), sha('second')], { evidence_refs: ['b', 'a'] }, 'ordering_matching_evidence_refs'))
reject('ordering_unknown_authority', () => validateOrdering(materializedR59, { ordered_by: 'invented_order' }, ['a'], {}, 'ordering_unknown_authority'))
...
reject('outbox_failed_code_reverse_resealed', () => { const decoded = decodeFixture(materializedR59, outboxAbortRecord.fixture, 'outbox_abort_attack_source'), payload = structuredClone(decoded.payload), schema = materializedR59.outbox.invocation_aborted_before_provider_schema; payload.failed_nonlease_final_recheck_codes = ['payload_changed', 'operation_authority_changed']; payload.abort_reason = 'payload_changed'; payload.abort_fingerprint = declaredTestHash(get(materializedR59, schema.fingerprint_ref), payload); decodeFixture(materializedR59, resealFixture(outboxAbortRecord.fixture, payload), 'outbox_failed_code_reverse') })
const bootstrapWrapper = materializedR59.authority_operation_replay_restart_fixtures.fixtures.map(fixture => fixture.artifact_store_by_role?.proof?.stored_row_value).find(row => row?.canonical_schema_ref === 'case_session_root_bootstrap_proof_schema')
...
validateSchema(materializedR59, materializedR59.case_session_root_bootstrap_proof_schema, bootstrapPayload, 'bootstrap_positive')
reject('bootstrap_signer_alias', () => { const payload = structuredClone(bootstrapPayload); payload.signer_2_ref = payload.signer_1_ref; validateSchema(materializedR59, materializedR59.case_session_root_bootstrap_proof_schema, payload, 'bootstrap_alias') })
const enumRefOccurrences = []
for (const record of fixtureRecords) {
  const decoded = decodeFixture(materializedR59, record.fixture, `enum_ref_inventory:${record.row.identity_kind}:${record.kind}`)
...
for (const [probeIndex, probe] of enumRefOccurrences.slice(0, 8).entries()) reject(`enum_ref_${probeIndex + 1}`, () => {
  const payload = structuredClone(probe.decoded.payload); payload[probe.field] = `invented_${probe.spec.enum_ref}`; decodeFixture(materializedR59, resealFixture(probe.record.fixture, payload), `enum_ref_attack:${probeIndex}`)
})
const literalOccurrences = []
const minItemOccurrences = []
for (const record of fixtureRecords) {
  const decoded = decodeFixture(materializedR59, record.fixture, `attack_inventory:${record.row.identity_kind}:${record.kind}`)
...
for (const [probeIndex, probe] of literalOccurrences.slice(0, 148).entries()) reject(`literal_or_const_${probeIndex + 1}`, () => {
  const payload = structuredClone(probe.decoded.payload); payload[probe.field] = wrongLiteral(probe.spec, payload[probe.field]); decodeFixture(materializedR59, resealFixture(probe.record.fixture, payload), `literal_attack:${probeIndex}`)
})
...
for (const [probeIndex, probe] of minItemOccurrences.entries()) reject(`min_items_${probeIndex + 1}`, () => {
  const payload = structuredClone(probe.decoded.payload); payload[probe.field] = []; decodeFixture(materializedR59, resealFixture(probe.record.fixture, payload), `min_items_attack:${probeIndex}`)
})
...
const ordinaryDecoded = decodeFixture(materializedR59, ordinaryFixture.fixture, 'ordinary_target_probe')
const targetProbes = [
...
for (const probe of fixtureRecords.filter(record => record.kind === 'target')) {
  const decoded = decodeFixture(materializedR59, probe.fixture, `target_literal_inventory:${probe.row.identity_kind}`)
...
for (const [probeIndex, probe] of targetProbes.entries()) reject(`target_literal_${probeIndex + 1}`, () => {
  const payload = structuredClone(probe.decoded.payload); payload[probe.field] = probe.value; decodeFixture(materializedR59, resealFixture(probe.record.fixture, payload), `target_literal_attack:${probeIndex}`)
})

const jointRows = index.filter(row => get(materializedR59, row.exact_authority_ref).executable_native_derivation_evidence.joint_source_target_fixture)
...
for (const identityId of ['identity_0929', 'identity_0958', 'identity_0973']) {
  ...
  reject(`${identityId}_shared_fixture_dereference`, () => { const candidate = structuredClone(materializedR59), authority = get(candidate, row.exact_authority_ref), shared = get(candidate, authority.executable_native_derivation_evidence.complete_joint_fixture_ref); shared.target_native_fingerprint = sha(`splice:${identityId}`); validateIdentity(candidate, row) })
  reject(`${identityId}_specialized_mapping_splice`, () => { const authority = structuredClone(get(materializedR59, row.exact_authority_ref)); authority.executable_native_derivation_evidence.authority_companion_target_mapping.target_value = sha(`splice:${identityId}:mapping`); validateIdentity(materializedR59, row, authority) })
}
reject('cross_authority_complete_joint_splice', () => { const candidate = structuredClone(materializedR59), first = index.find(item => item.exact_authority_ref.endsWith('.identity_0929')), second = index.find(item => item.exact_authority_ref.endsWith('.identity_0958')), authority = get(candidate, first.exact_authority_ref), foreign = get(candidate, second.exact_authority_ref).executable_native_derivation_evidence.complete_joint_fixture_ref; authority.executable_native_derivation_evidence.complete_joint_fixture_ref = foreign; authority.executable_native_derivation_evidence.joint_source_target_fixture.complete_joint_fixture_ref = foreign; validateIdentity(candidate, first) })
assert(Object.values(materializedR59.authority_operation_conditional_rule_predicate_registry.rows).some(row => row.rule_id === 'rule_0077' && row.evaluation_scope === 'requires_resolved_external_artifact_or_transaction_context'), 'committed_use_release_terminal_ref_not_counted_as_proved')
const ordinaryComplete = Object.entries(materializedR59.authority_operation_complete_joint_equality_fixtures.rows).find(([, row]) => row.equality_source_schema_ref === 'authority_operation_hold_store.row_union' && row.equality_source_schema_variant === 'ordinary_single_proof' && row.equality_source_field === 'result_ref' && row.declared_companion_bytes_fields.some(item => item.field === 'result_bytes_sha256') && row.declared_companion_fingerprint_fields.some(item => item.field === 'result_fingerprint'))
assert(ordinaryComplete, 'ordinary_complete_result_fixture')
reject('ordinary_result_companions_cannot_be_vacuous', () => { const candidate = structuredClone(materializedR59); const row = candidate.authority_operation_complete_joint_equality_fixtures.rows[ordinaryComplete[0]]; row.declared_companion_bytes_fields = []; row.declared_companion_fingerprint_fields = []; row.declared_companion_bytes_count = 0; row.declared_companion_fingerprint_count = 0; validateCompleteJointFixtures(candidate) })
reject('ordinary_result_same_target_bytes_splice', () => { const candidate = structuredClone(materializedR59), row = candidate.authority_operation_complete_joint_equality_fixtures.rows[ordinaryComplete[0]], decoded = decodeFixture(candidate, row.source_fixture, 'ordinary_splice_source'), payload = structuredClone(decoded.payload); payload.result_bytes_sha256 = sha('different_target'); row.source_fixture = resealFixture(row.source_fixture, payload); validateCompleteJointFixtures(candidate) })
reject('ordinary_result_same_target_fingerprint_splice', () => { const candidate = structuredClone(materializedR59), row = candidate.authority_operation_complete_joint_equality_fixtures.rows[ordinaryComplete[0]], decoded = decodeFixture(candidate, row.source_fixture, 'ordinary_splice_fp'), payload = structuredClone(decoded.payload); payload.result_fingerprint = sha('different_target'); row.source_fixture = resealFixture(row.source_fixture, payload); validateCompleteJointFixtures(candidate) })
...
const selectorIds = Object.keys(materializedR59.authority_operation_internal_reference_target_selectors.rows)
function selectorAttack(name, selectorId, mutate) { reject(name, () => { const selector = structuredClone(materializedR59.authority_operation_internal_reference_target_selectors.rows[selectorId]); mutate(selector); validateSelector(materializedR59, selector) }) }
...
reject('conditional_exclusion_substituted_identity', () => { const candidate = structuredClone(materializedR59), coverage = candidate.authority_operation_conditional_fixture_coverage; coverage.semantic_or_live_unproved_identity_exclusions[0].identity_id = 'identity_invented'; validateConditionalAuthority(candidate) })
reject('manifest_identity_mutation', () => { const contract = structuredClone(materializedR59); contract.authority_operation_native_identity_applicability_report.exact_native_identity_total += 1; validateManifest(contract) })

console.log(`ok: R59 exact; ${attacks} attacks; ${baseline.identities.total}/1229 native identities; ${jointRows.length} linked identities share ${materializedR59.authority_operation_complete_joint_equality_fixtures.exact_row_count} complete equality fixtures; ${baseline.conditional.local}+${baseline.conditional.contexts} executed conditional rules and ${baseline.conditional.unproved} honest semantic/live exclusions; ${baseline.selectors} selectors over ${baseline.contexts} source-schema-valid contexts; ${baseline.refs} semantic refs; ${baseline.manifest} manifest rows; frozen R58 preserved`)
