...
const directory = 'project-documentation/ctrl-evolution/g24-predicate-authority-r92'
...
const expectedTopKeys = {
  effective_resolution: ['schema_version', 'module_id', 'semantic_base', 'imported_exact_sections', 'r92_replacement_sections', 'inheritance_rule', 'unknown_or_conflicting_section', 'authority_still_closed'],
...
check('manifest schema and authority metadata exact', manifest.schema_version === 'ctrl.g24.predicate-authority.r92.manifest.v1' && manifest.standing === 'bound_local_contract_candidate_no_runtime_authority' && manifest.decision_id === 'DEC-20260916-g24-predicate-authority-r75' && canonical(manifest.semantic_base) === canonical(resolution.semantic_base) && manifest.resolution_rule === resolution.inheritance_rule)
...
const answerAuthorityService = (() => {
  ...
  const advanceForConformance = (routeRef, questionId, answerRef, action = 'correct') => {
    ...
    for (const listener of [...listeners]) { try { listener(clone(prior), clone(next), clone(transition)) } catch {} }
    return true
  }
  const onAdvance = listener => {
    if (typeof listener !== 'function') return () => false
    listeners.push(listener)
    return () => { const index = listeners.indexOf(listener); if (index < 0) return false; listeners.splice(index, 1); return true }
  }
...
function buildFinalReceipt(transitionId, predicateProof) {
  ...
  const authorityProofs = humans.map(human => ({ named_human_id: human, authentication_event_ref: human === 'krish' ? 'auth-r92-krish' : 'auth-r92-leader', authority_event_ref: `final-authority-${transitionId}-${human}` }))
...
if (process.env.R92_EMIT_FINAL_UNSIGNED === '1') {
  ...
  console.log(`R92_UNSIGNED_FINALS=${JSON.stringify(emitted)}`)
  process.exit(0)
}
...
const acceptedFinalService = (() => {
  ...
  const currentStatus = acceptedFinalId => {
    ...
    if (open.length) {
      const state = currentHeadState(open, false), withdrawals = open.filter(challenge => specimens.answer_state_transition_events[challenge.trigger_transition_ref]?.action === 'withdraw'), replacementMissing = withdrawals.some(challenge => state.heads.find(head => head.route_ref === challenge.route_ref && head.question_id === challenge.question_id)?.standing !== 'current')
      if (replacementMissing) return { status: 'replacement_required', message: 'One answer was withdrawn. Choose a new answer, then review all changes with Krish.' }
      if (withdrawals.length) return { status: 'review_required', message: 'Your new answer is ready. Work stays paused until you review all changes with Krish.' }
...
if (process.env.R92_EMIT_MULTI_CHANGE_UNSIGNED === '1') {
  sessionAuthorityService.resetForConformance(); answerAuthorityService.resetForConformance(); acceptedFinalService.resetForConformance()
  const built = buildPredicateProof('accept_intensive_proof'), final = buildFinalReceipt('accept_intensive_proof', built.proof)
  ...
  console.log(`R92_UNSIGNED_MULTI_CHANGE=${JSON.stringify(unsigned)}`)
  process.exit(0)
}
if (process.env.R92_EMIT_MIXED_CHANGE_UNSIGNED === '1') {
  sessionAuthorityService.resetForConformance(); answerAuthorityService.resetForConformance(); acceptedFinalService.resetForConformance()
  const built = buildPredicateProof('accept_intensive_proof'), final = buildFinalReceipt('accept_intensive_proof', built.proof)
  ...
  console.log(`R92_UNSIGNED_MIXED_CHANGE=${JSON.stringify(unsigned)}`)
  process.exit(0)
}
...
}
{
  ...
  if (head) {
    ...
    check('single authoritative status says both replacement and review are required', same(acceptedFinalService.currentStatus(acceptedFinalService.records()[0].accepted_final_id), { status: 'replacement_required', message: 'One answer was withdrawn. Choose a new answer, then review all changes with Krish.' }))
    const reanswer = Object.values(specimens.answer_state_transition_events).find(event => event.route_ref === head.route_ref && event.question_id === head.question_id && event.action === 'reanswer')
...
}
{
  ...
  const replacementRequired = { status: 'replacement_required', message: 'One answer was withdrawn. Choose a new answer, then review all changes with Krish.' }, reviewRequired = { status: 'review_required', message: 'Your new answer is ready. Work stays paused until you review all changes with Krish.' }
  check('mixed correction and withdrawal status is independent of append order', Boolean(forward && reverse) && same(forward.replacementStatus, replacementRequired) && same(reverse.replacementStatus, replacementRequired) && forward.prematureCount === 0 && reverse.prematureCount === 0 && same(forward.reviewStatus, reviewRequired) && same(reverse.reviewStatus, reviewRequired) && forward.unsignedCount === 2 && reverse.unsignedCount === 2)
...
if (failures.length) {
  console.error(`R92 independent conformance failed (${failures.length})`)
  for (const failure of failures) console.error(`- ${failure}`)
  process.exit(1)
}
console.log(`ok: R92 end-to-end bound contract; ${imports.transition_catalogue.length}/13 predicate and final transition paths; ${vectorGroups.reduce((sum, group) => sum + vectors[group].length, 0)} executable vectors; ${totalityProbeCount} generated totality probes`)
