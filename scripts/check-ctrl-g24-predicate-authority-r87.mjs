...
const directory = 'project-documentation/ctrl-evolution/g24-predicate-authority-r87'
...
const expectedTopKeys = {
  effective_resolution: ['schema_version', 'module_id', 'semantic_base', 'imported_exact_sections', 'r87_replacement_sections', 'inheritance_rule', 'unknown_or_conflicting_section', 'authority_still_closed'],
...
check('interaction nested programs closed', exactKeys(interaction.copy_limits, ['heading', 'question', 'answer_option_each', 'unknown_option', 'free_expression_option', 'visible_consequence', 'optional_note_label', 'primary_action_label', 'total_visible_characters', 'answer_options']) && exactKeys(interaction.default_surface, ['heading_count', 'question_count', 'answer_control_group_count', 'optional_note_count', 'primary_action_count', 'automatic_follow_up_count', 'technical_label_count']) && same(interaction.transition_effect_required, ['transition_id', 'from_state', 'to_state', 'authority', 'invalidation', 'receipt', 'plain_language_action']) && exactKeys(interaction.answer_state_store, interaction.answer_state_store_required) && interaction.answer_state_store.records.every(record => exactKeys(record, interaction.answer_state_record_required)) && interaction.judgement_contracts.every(contract => exactKeys(contract, ['contract_id', 'judgement_kind', 'dependency_ids', 'heading', 'question', 'visible_consequence', 'answers']) && contract.answers.every(answer => exactKeys(answer, ['option', 'effect_id', 'patch', 'transition_disposition']) && exactKeys(answer.patch, ['path', 'value']) && ['supports_transition', 'blocks_transition'].includes(answer.transition_disposition))))
...
check('manifest schema and authority metadata exact', manifest.schema_version === 'ctrl.g24.predicate-authority.r87.manifest.v1' && manifest.standing === 'bound_local_contract_candidate_no_runtime_authority' && manifest.decision_id === 'DEC-20260916-g24-predicate-authority-r75' && canonical(manifest.semantic_base) === canonical(resolution.semantic_base) && manifest.resolution_rule === resolution.inheritance_rule)
...
function validateAnswerStateRecord(record) {
  if (!finiteJson(record) || !exactKeys(record, interaction.answer_state_record_required) || typeof record.issuer_sequence !== 'number' || !Number.isInteger(record.issuer_sequence) || record.issuer_sequence < 1 || !['current', 'superseded', 'withdrawn'].includes(record.standing) || !Array.isArray(record.member_answer_refs) || !record.member_answer_refs.length || !record.member_answer_refs.every(value => typeof value === 'string') || !unique(record.member_answer_refs)) return false
  const event = specimens.answer_events[record.current_answer_ref] ?? specimens.free_expression_confirmations[record.current_answer_ref]
  return Boolean(event && event.question_id === record.question_id && event.question_version === record.question_version && hashValue(event) === record.current_answer_fingerprint && record.member_answer_refs.includes(record.current_answer_ref) && record.member_set_seal === hashValue(byteSorted(record.member_answer_refs)) && fingerprint(record, 'authoritative_head_fingerprint') === record.authoritative_head_fingerprint && (record.supersedes_answer_ref === null || typeof record.supersedes_answer_ref === 'string'))
}
function validateAnswerStateHistory(matching) {
  if (!Array.isArray(matching) || !matching.length || matching.some(record => !validateAnswerStateRecord(record))) return 'invalid'
  const current = matching.filter(record => record.standing === 'current')
  if (current.length > 1) return 'ambiguous'
  if (!current.length) return 'missing'
  const ordered = [...matching].sort((a, b) => a.issuer_sequence - b.issuer_sequence)
  if (!unique(ordered.map(record => record.answer_state_id)) || !unique(ordered.map(record => record.state_version)) || !unique(ordered.map(record => record.issuer_sequence))) return 'ambiguous'
  const { route_ref: routeRef, question_id: questionId, question_version: questionVersion } = ordered[0]
  if (ordered.some((record, index) => record.route_ref !== routeRef || record.question_id !== questionId || record.question_version !== questionVersion || record.issuer_sequence !== index + 1)) return 'invalid'
  let admitted = []
  for (const [index, record] of ordered.entries()) {
    admitted = byteSorted([...new Set([...admitted, record.current_answer_ref])])
    if (!same(record.member_answer_refs, byteSorted(record.member_answer_refs)) || !same(record.member_answer_refs, admitted)) return 'invalid'
    if (index === 0) {
      if (record.supersedes_answer_ref !== null) return 'invalid'
    } else if (record.supersedes_answer_ref !== ordered[index - 1].current_answer_ref) return 'invalid'
    if (index < ordered.length - 1 && record.standing === 'current') return 'ambiguous'
  }
  return current[0] === ordered.at(-1) ? 'valid' : 'ambiguous'
}
function resolveAnswerState(routeRef, questionId, answerRef, records = interaction.answer_state_store.records) {
  if (typeof routeRef !== 'string' || typeof questionId !== 'string' || typeof answerRef !== 'string' || !Array.isArray(records)) return { status: 'invalid', record: null }
  const matching = records.filter(record => record.route_ref === routeRef && record.question_id === questionId)
  const historyStatus = validateAnswerStateHistory(matching)
  if (historyStatus !== 'valid') return { status: historyStatus, record: null }
  const head = matching.find(record => record.standing === 'current')
  if (head.current_answer_ref !== answerRef) return { status: matching.some(record => record.current_answer_ref === answerRef || record.member_answer_refs.includes(answerRef)) ? 'stale' : 'missing', record: clone(head) }
  return { status: 'current', record: clone(head) }
}
const answerAuthorityService = (() => {
  let records = clone(interaction.answer_state_store.records)
  const resolve = (routeRef, questionId, answerRef) => resolveAnswerState(routeRef, questionId, answerRef, records)
  const selectForConformance = (routeRef, questionId, answerRef) => {
    const event = specimens.answer_events[answerRef] ?? specimens.free_expression_confirmations[answerRef], matching = records.filter(record => record.route_ref === routeRef && record.question_id === questionId)
    if (!event || !matching.length || event.question_id !== questionId) return false
    for (const record of matching) if (record.standing === 'current') { record.standing = 'superseded'; record.authoritative_head_fingerprint = fingerprint(record, 'authoritative_head_fingerprint') }
    const prior = matching.sort((a, b) => b.issuer_sequence - a.issuer_sequence)[0], members = byteSorted([...new Set([...prior.member_answer_refs, answerRef])])
    const next = { answer_state_id: `answer-state-${questionId}-v${prior.issuer_sequence + 1}`, route_ref: routeRef, question_id: questionId, question_version: event.question_version, state_version: `answer-state-v${prior.issuer_sequence + 1}`, issuer_sequence: prior.issuer_sequence + 1, current_answer_ref: answerRef, current_answer_fingerprint: hashValue(event), supersedes_answer_ref: prior.current_answer_ref, standing: 'current', member_answer_refs: members, member_set_seal: hashValue(members), authoritative_head_fingerprint: '' }
    next.authoritative_head_fingerprint = fingerprint(next, 'authoritative_head_fingerprint')
    records.push(next)
    return true
  }
  return Object.freeze({ resolve, selectForConformance, resetForConformance: () => { records = clone(interaction.answer_state_store.records) } })
})()
{
  const questionId = interaction.answer_state_store.records.find(record => record.standing === 'current').question_id
  const current = interaction.answer_state_store.records.find(record => record.question_id === questionId && record.standing === 'current'), previous = interaction.answer_state_store.records.find(record => record.question_id === questionId && record.standing === 'superseded')
  for (const vector of vectors.answer_state_vectors) {
    const records = clone(interaction.answer_state_store.records)
    const group = records.filter(record => record.question_id === questionId), head = group.find(record => record.standing === 'current')
    let answerRef = head.current_answer_ref
    if (['use_previous', 'use_previous_after_current'].includes(vector.mutation?.op)) answerRef = previous.current_answer_ref
    if (vector.mutation?.op === 'withdraw_current') { head.standing = 'withdrawn'; head.authoritative_head_fingerprint = fingerprint(head, 'authoritative_head_fingerprint') }
    if (vector.mutation?.op === 'duplicate_current') { const duplicate = clone(head); duplicate.answer_state_id = `${head.answer_state_id}-conflict`; duplicate.current_answer_ref = previous.current_answer_ref; duplicate.current_answer_fingerprint = hashValue(specimens.answer_events[previous.current_answer_ref]); duplicate.authoritative_head_fingerprint = fingerprint(duplicate, 'authoritative_head_fingerprint'); records.push(duplicate) }
    if (vector.mutation?.op === 'break_member_seal') head.member_set_seal = '0'.repeat(64)
    checkEq(`answer state vector ${vector.id}`, resolveAnswerState(current.route_ref, questionId, answerRef, records).status, vector.expected)
  }
  const mutate = mutation => {
    const records = clone(interaction.answer_state_store.records), group = records.filter(record => record.question_id === questionId).sort((a, b) => a.issuer_sequence - b.issuer_sequence)
    mutation(group)
    for (const record of group) record.authoritative_head_fingerprint = fingerprint(record, 'authoritative_head_fingerprint')
    return resolveAnswerState(current.route_ref, questionId, group.at(-1).current_answer_ref, records).status
  }
  checkEq('answer state rejects skipped issuer sequence', mutate(group => { group.at(-1).issuer_sequence += 1 }), 'invalid')
  checkEq('answer state rejects broken supersession chain', mutate(group => { group.at(-1).supersedes_answer_ref = 'answer-not-the-predecessor' }), 'invalid')
  checkEq('answer state rejects incomplete cumulative member set with a valid seal', mutate(group => { group.at(-1).member_answer_refs = [group.at(-1).current_answer_ref]; group.at(-1).member_set_seal = hashValue(group.at(-1).member_answer_refs) }), 'invalid')
  checkEq('answer state rejects non-canonical member ordering with a valid seal', mutate(group => { group.at(-1).member_answer_refs.reverse(); group.at(-1).member_set_seal = hashValue(byteSorted(group.at(-1).member_answer_refs)) }), 'invalid')
  checkEq('answer state rejects duplicate state version', mutate(group => { group.at(-1).state_version = group[0].state_version }), 'ambiguous')
}
function validateFreeConfirmation(event, receipt, question, render, interpretation, interpretationRender) {
  const reject = reason => { if (process.env.R87_DEBUG) console.error('free confirmation reject', reason); return false }
  ...
  if (Object.values(bindingChecks).includes(false)) { if (process.env.R87_DEBUG) console.error(bindingChecks); return reject('binding') }
  const unsigned = clone(event); delete unsigned.issuer_signature_base64
...
function validateFinalAuthorityEvent(event, proof, receipt) {
  const reject = reason => { if (process.env.R87_DEBUG) console.error('final authority reject', reason, proof?.authority_event_ref); return false }
...
function validatePredicate(proof, evidence) {
  ...
  for (const result of proof.dependency_results) {
    ...
    if (expectedKind === 'profile') {
      ...
      if (profileResult !== 'valid') { if (process.env.R87_DEBUG) console.error('predicate profile invalid', result.dependency_id, profileResult); return 'evidence_invalid' }
...
check('mandatory vector bodies pinned', hashValue(Object.fromEntries(vectorGroups.map(group => [group, vectors[group]]))) === '65ab6d9efa9f974c2e05e554c1dd1e1f3607a19fffed1ce67399614b6f664b18')
...
if (failures.length) {
  console.error(`R87 independent conformance failed (${failures.length})`)
  for (const failure of failures) console.error(`- ${failure}`)
  process.exit(1)
}
console.log(`ok: R87 end-to-end bound contract; ${imports.transition_catalogue.length}/13 predicate and final transition paths; ${vectorGroups.reduce((sum, group) => sum + vectors[group].length, 0)} executable vectors; ${totalityProbeCount} generated totality probes`)
