...
const directory = 'project-documentation/ctrl-evolution/g24-predicate-authority-r96'
...
const expectedTopKeys = {
  effective_resolution: ['schema_version', 'module_id', 'semantic_base', 'imported_exact_sections', 'r96_replacement_sections', 'inheritance_rule', 'unknown_or_conflicting_section', 'authority_still_closed'],
...
check('manifest schema and authority metadata exact', manifest.schema_version === 'ctrl.g24.predicate-authority.r96.manifest.v1' && manifest.standing === 'bound_local_contract_candidate_no_runtime_authority' && manifest.decision_id === 'DEC-20260916-g24-predicate-authority-r75' && canonical(manifest.semantic_base) === canonical(resolution.semantic_base) && manifest.resolution_rule === resolution.inheritance_rule)
...
function buildFinalReceipt(transitionId, predicateProof) {
  ...
  const authorityProofs = humans.map(human => ({ named_human_id: human, authentication_event_ref: human === 'krish' ? 'auth-r96-krish' : 'auth-r96-leader', authority_event_ref: `final-authority-${transitionId}-${human}` }))
...
if (process.env.R96_EMIT_FINAL_UNSIGNED === '1') {
  ...
  console.log(`R96_UNSIGNED_FINALS=${JSON.stringify(emitted)}`)
  process.exit(0)
}
...
const acceptedFinalService = (() => {
  ...
  const revalidationEventId = (acceptedFinalId, challengeSetSeal, observationSetSeal, currentHeadSeal, namedHumanId, authorizedAt) => `revalidation-${hashValue({ accepted_final_id: acceptedFinalId, challenge_set_seal: challengeSetSeal, observation_set_seal: observationSetSeal, current_head_seal: currentHeadSeal, named_human_id: namedHumanId, authorized_at: authorizedAt }).slice(0, 40)}`
  ...
  const validateRevalidationEvent = (event, record, openChallenges) => {
    ...
    const transition = transitionById[record.transition_id], allowedHumans = transition.final_authority.actor_roles.map(role => roleHuman[role]), currentState = currentHeadState(openChallenges, true), challengeIds = byteSorted(openChallenges.map(challenge => challenge.challenge_id)), boundObservations = observations.filter(value => challengeIds.includes(value.challenge_id)), observationIds = byteSorted(boundObservations.map(value => value.observation_id))
    if (!currentState.valid || !Array.isArray(event.current_answer_heads) || !event.current_answer_heads.every(head => exactKeys(head, trust.answer_state_revalidation_head_required))) return false
  ...
  const buildUnsignedRevalidationEvents = (acceptedFinalId, requireCurrent) => {
    ...
    const currentState = currentHeadState(open, requireCurrent)
    if (!currentState.valid) return []
    const transition = transitionById[record.transition_id], humans = byteSorted([...new Set(transition.final_authority.actor_roles.map(role => roleHuman[role]))]), challengeIds = byteSorted(open.map(challenge => challenge.challenge_id)), boundObservations = observations.filter(value => challengeIds.includes(value.challenge_id)), observationIds = byteSorted(boundObservations.map(value => value.observation_id))
    const challengeSeal = hashValue(challengeIds), observationSeal = hashValue(observationIds), currentHeadSeal = hashValue(currentState.heads)
  ...
  const revalidate = events => {
    ...
    const challengeIds = byteSorted(open.map(challenge => challenge.challenge_id)), blockIds = byteSorted(blocks.filter(block => challengeIds.includes(block.challenge_id)).map(block => block.block_id)), resolvedAtMs = Math.max(...events.map(event => Date.parse(event.authorized_at)))
    if (!events.every(event => canonicalInstant(event.authorized_at)) || !Number.isFinite(resolvedAtMs)) return false
    const currentState = currentHeadState(open, true)
    ...
    const resolution = { revalidation_id: `revalidated-${hashValue({ accepted_final_id: record.accepted_final_id, challenge_set_seal: hashValue(challengeIds), observation_set_seal: challengeObservationSeal, current_head_seal: hashValue(currentState.heads) }).slice(0, 32)}`, accepted_final_id: record.accepted_final_id, accepted_request_identity: record.request_identity, authority_events: entriesForRetry, challenge_ids: challengeIds, challenge_set_seal: hashValue(challengeIds), challenge_observation_refs: challengeObservationRefs, challenge_observation_seal: challengeObservationSeal, block_ids: blockIds, current_answer_heads: mergedHeads, standing: 'resolved', status_text: 'You reviewed the change with Krish. Work can move again using your current answer.', resolved_at: new Date(resolvedAtMs).toISOString(), record_fingerprint: '' }; resolution.record_fingerprint = fingerprint(resolution, 'record_fingerprint')
  ...
  const verifyOpenChallengeProvenance = (record, open, authority) => {
    if (!record || fingerprint(record, 'record_fingerprint') !== record.record_fingerprint) return false
    ...
    return open.every(challenge => {
      if (!exactKeys(challenge, correction.accepted_final_challenge_required) || fingerprint(challenge, 'challenge_fingerprint') !== challenge.challenge_fingerprint || challenge.accepted_final_id !== record.accepted_final_id || challenge.accepted_record_fingerprint !== record.record_fingerprint || !same(challenge.permitted_audience, record.permitted_audience) || !permitted.includes(challenge.responsible_human_id)) return false
      const trigger = specimens.answer_state_transition_events[challenge.trigger_transition_ref]
      if (!trigger || challenge.trigger_transition_fingerprint !== hashValue(trigger) || !verifyTransitionEdge(challenge.trigger_transition_ref, challenge.changed_head_fingerprint, challenge.resulting_head_fingerprint, challenge.responsible_human_id, authority)) return false
  ...
  const currentStatus = acceptedFinalId => {
    ...
    if (open.length) {
      const answerAuthority = answerAuthorityService.snapshotForConformance(), state = currentHeadState(open, false)
      if (!state.valid || !verifyOpenChallengeProvenance(record, open, answerAuthority)) return unavailable
...
if (process.env.R96_EMIT_MULTI_CHANGE_UNSIGNED === '1') {
  sessionAuthorityService.resetForConformance(); answerAuthorityService.resetForConformance(); acceptedFinalService.resetForConformance()
  const built = buildPredicateProof('accept_intensive_proof'), final = buildFinalReceipt('accept_intensive_proof', built.proof)
  ...
  console.log(`R96_UNSIGNED_MULTI_CHANGE=${JSON.stringify(unsigned)}`)
  process.exit(0)
}
if (process.env.R96_EMIT_MIXED_CHANGE_UNSIGNED === '1') {
  sessionAuthorityService.resetForConformance(); answerAuthorityService.resetForConformance(); acceptedFinalService.resetForConformance()
  const built = buildPredicateProof('accept_intensive_proof'), final = buildFinalReceipt('accept_intensive_proof', built.proof)
  ...
  console.log(`R96_UNSIGNED_MIXED_CHANGE=${JSON.stringify(unsigned)}`)
  process.exit(0)
}
if (process.env.R96_EMIT_DOUBLE_WITHDRAW_UNSIGNED === '1') {
  sessionAuthorityService.resetForConformance(); answerAuthorityService.resetForConformance(); acceptedFinalService.resetForConformance()
  const built = buildPredicateProof('accept_intensive_proof'), final = buildFinalReceipt('accept_intensive_proof', built.proof)
  ...
  console.log(`R96_UNSIGNED_DOUBLE_WITHDRAW=${JSON.stringify(unsigned)}`)
  process.exit(0)
}
...
}
{
  sessionAuthorityService.resetForConformance(); answerAuthorityService.resetForConformance(); acceptedFinalService.resetForConformance()
  const accepted = buildPredicateProof('accept_intensive_proof'), final = buildFinalReceipt('accept_intensive_proof', accepted.proof)
  checkEq('status provenance mutation final accepted', validateFinal(final.receipt, accepted.proof, accepted.evidence, final.render), 'valid')
  const head = accepted.proof.answer_state_heads[0]
  check('status provenance mutation creates a missing accepted head', answerAuthorityService.withdrawForConformance(head.route_ref, head.question_id, head.current_answer_ref))
  const acceptedId = acceptedFinalService.records()[0]?.accepted_final_id, challenge = acceptedFinalService.challenges()[0], transition = challenge ? specimens.answer_state_transition_events[challenge.trigger_transition_ref] : null, pristine = clone(transition), unavailable = { status: 'not_found', message: 'This decision is unavailable.' }
  const initial = acceptedFinalService.currentStatus(acceptedId)
  const mutations = [
  ...
  const everyMutationCloses = Boolean(transition && acceptedId) && mutations.every(mutate => {
    mutate(transition)
    const status = acceptedFinalService.currentStatus(acceptedId)
    Object.assign(transition, clone(pristine))
    return same(status, unavailable) && canonical(transition) === canonical(pristine)
  })
  check('current status revalidates exact signed transition bytes and fails closed after any provenance mutation', initial.status === 'replacement_required' && everyMutationCloses && acceptedFinalService.currentStatus(acceptedId).status === 'replacement_required')
}
{
  sessionAuthorityService.resetForConformance(); answerAuthorityService.resetForConformance(); acceptedFinalService.resetForConformance()
  const accepted = buildPredicateProof('accept_intensive_proof'), final = buildFinalReceipt('accept_intensive_proof', accepted.proof)
...
}
{
  const executeDoubleWithdrawal = withdrawalOrder => {
    ...
    return { zeroReplacedStatus, zeroReviewRequests, oneReplacedStatus, oneReviewRequests, twoReplacedStatus, twoReviewRequests, resolution: acceptedFinalService.revalidations()[0], baseline: acceptedFinalService.baselines().at(-1), currentStatus: acceptedFinalService.currentStatus(acceptedId) }
  }
...
if (failures.length) {
  console.error(`R96 independent conformance failed (${failures.length})`)
  for (const failure of failures) console.error(`- ${failure}`)
  process.exit(1)
}
console.log(`ok: R96 end-to-end bound contract; ${imports.transition_catalogue.length}/13 predicate and final transition paths; ${vectorGroups.reduce((sum, group) => sum + vectors[group].length, 0)} executable vectors; ${totalityProbeCount} generated totality probes`)
