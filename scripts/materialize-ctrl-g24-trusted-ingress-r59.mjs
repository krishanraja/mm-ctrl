...
import { materializedR58 as materializedR54, materializedR58Output as materializedR54Output, r58SemanticAuthorityPaths as r54SemanticAuthorityPaths } from './materialize-ctrl-g24-trusted-ingress-r58.mjs'
import { canonicalR44, ownedSnapshotR44 } from './materialize-ctrl-g24-trusted-ingress-r44.mjs'

const root = process.cwd()
const inputPath = 'project-documentation/ctrl-evolution/g24-trusted-ingress-contract-r58.json'
const outputPath = 'project-documentation/ctrl-evolution/g24-trusted-ingress-contract-r59.json'
const inputBytes = readFileSync(join(root, inputPath), 'utf8')
if (inputBytes !== materializedR54Output) throw new Error('R59_frozen_R58_input_mismatch')
...
function collectConditionalSites(value, path = '$') {
  ...
  for (const rule of value.conditional_rules ?? []) {
    if (typeof rule !== 'string' || !rule) throw new Error(`R59_conditional_rule_not_string:${path}`)
...
function validateOrdering(spec, value, context, path) {
  ...
  if (rule === 'matching_evidence_refs') {
    ...
    if (!Array.isArray(refs) || refs.length !== value.length) throw new Error(`R59_ordering_companion_alignment:${path}`)
    for (let index = 1; index < refs.length; index += 1) if (unsignedUtf8Compare(refs[index - 1], refs[index]) > 0) throw new Error(`R59_ordering_matching_refs:${path}`)
    return
  }
  ...
  if (!supported.has(rule)) throw new Error(`R59_unknown_ordering_authority:${path}:${rule}`)
  const keys = value.map(item => orderingKey(rule, item))
  if (keys.some(key => key === undefined || key === -1)) throw new Error(`R59_ordering_key_unresolved:${path}:${rule}`)
  for (let index = 1; index < keys.length; index += 1) {
    const comparison = typeof keys[index] === 'number' ? keys[index - 1] - keys[index] : unsignedUtf8Compare(keys[index - 1], keys[index])
    if (comparison > 0) throw new Error(`R59_ordering_violation:${path}:${rule}`)
  }
}

r54.schema_version = 'ctrl.g24.trusted-ingress.r59.effective.v1'
r54.status = ['founder_locked_direction', 'headless_kernel_independently_verified', 'trusted_ingress_r1_through_r58_vetoed', 'trusted_ingress_r59_fully_materialized', 'independent_attack_required', 'no_adapter_or_runtime_connection']
r54.supersedes = { commit: 'a517b02a295ad399779de92c77ca327c1b9215a5', tree: '80e01b6e59c12bb32507062106946b532dc4b0e9', human_blob: '10c7bd0594a72d28299c9a27ae558cb34c2dd026', machine_blob: '6585f8af2c3a6869a90e8d90139de9f6a89a0e81', qa_blob: '80114e4428408cc3b9748b14238ebb15afec18ea', checker_blob: '64e8d3808d45b4880a34e987b1f42c0ead340a41', materializer_blob: '1915c1411de0bd6760c950827d781d0e6358ea2a', founder_checker_blob: '459e9201e898a1edeec2f4bd28a87bde4614bdae', adjudication: 'veto' }
r54.materialization = { ...r54.materialization, schema_version: 'ctrl.g24.trusted-ingress-materialization.r59.v1', frozen_input: { path: inputPath, sha256: sha(inputBytes) }, output_path: outputPath, strict_finalization_dag: ['freeze_R58_input', 'close_ordering_authorities', 'close_array_semantics', 'canonicalize_intervention_inner_payloads', 'enforce_exact_trimmed_text', 'derive_complete_unproved_identity_exclusions', 'finalize_nonderived_authorities', 'snapshot_final_semantic_sources', 'derive_reference_owner_and_manifest', 'seal_output'], no_post_snapshot_source_write: true }
r54.authority_operation_intervention_inner_payload_schemas = {
  schema_version: 'ctrl.g24.intervention-inner-payload-schemas.r59.v1',
  question_schema_ref: 'proof_value_schemas.question_contract',
  session: {
    schema_version: 'ctrl.g24.intervention-session-inner-payload.r59.v1',
    type: 'object',
...
function values(spec) {
  ...
  if (spec.enum_ref) {
    ...
    if (!Array.isArray(resolved) || !resolved.length) throw new Error(`R59_enum_ref_missing_or_empty:${spec.enum_ref}`)
    return resolved
  }
...
function referencedSpec(spec) {
  ...
  if (!resolved) throw new Error(`R59_schema_ref_missing:${ref}`)
  return resolved
}
...
function valueForSpec(spec, seed, context = {}) {
  if (!spec || typeof spec !== 'object') return `r59_${seed}`
  ...
  if (pattern === '^[0-9a-f]{64}$') return sha(`R59:${seed}`)
  if (pattern === '^[0-9a-f]{32}$') return sha(`R59:${seed}`).slice(0, 32)
  if (spec.type === 'sha256') return sha(`R59:${seed}`)
  if (spec.type === 'sha256_or_exact_literal') return (spec.exact_literals ?? [])[0] ?? sha(`R59:${seed}`)
  if (spec.type === 'canonical_timestamp') return '2031-01-01T00:00:00.000Z'
  if (spec.type === 'base64url_without_padding') return Buffer.from(`R59:${seed}`, 'utf8').toString('base64url')
  ...
  return `r59_${String(seed).replace(/[^a-z0-9_]/gi, '_').slice(-96)}`.padEnd(minimum, 'x').slice(0, maximum)
}
...
function normalizePayloadForLocalRules(schema, payload, seed) {
  ...
  for (const [field, availability] of Object.entries(payload)) {
    ...
    if (availability === 'available') {
      payload[refField] = sha(`R59:${seed}:${refField}`)
      payload[shaField] = sha(`R59:${seed}:${shaField}`)
    } else {
  ...
  if (rules.has('result_ref_is_nonnull_iff_operation_class_is_use_release_and_equals_the_branch_terminal_consumption_ref')) payload.result_ref = payload.operation_class === 'use_release' ? (payload.result_ref ?? sha(`R59:${seed}:terminal_consumption_ref`)) : null
...
function validateSpec(spec, value, path, context = {}) {
  spec = effectiveSpec(spec)
  if (!spec || typeof spec !== 'object') throw new Error(`R59_invalid_schema_spec:${path}`)
  if (spec.type && !SUPPORTED_TYPES.has(spec.type)) throw new Error(`R59_unknown_type:${path}:${spec.type}`)
  if (!spec.properties && !spec.variants) for (const key of Object.keys(spec)) if (!LEAF_SPEC_KEYS.has(key)) throw new Error(`R59_unknown_schema_spec_key:${path}:${key}`)
  if (spec.type === 'nullable') {
    if (value === null) return
    if (!spec.value_schema) throw new Error(`R59_nullable_schema_missing:${path}`)
  ...
  if (spec.type === 'controlling_watermark_change') { const registry = r54.type_registry.controlling_watermark_change, variant = registry.variants?.[value?.kind_class]; if (!variant) throw new Error(`R59_watermark_variant:${path}`); validateSchema(variant, value, `${path}<${value.kind_class}>`, context); return }
  if (spec.type === 'discriminated_value') { const selected = spec.variants?.[context[spec.discriminator_source_field]]; if (!selected) throw new Error(`R59_discriminated_value:${path}`); validateSpec(selected, value, `${path}<${context[spec.discriminator_source_field]}>`, context); return }
  if (spec.type === 'discriminated_union') { const variant = spec.variants?.[value?.[spec.discriminator]]; if (!variant) throw new Error(`R59_union_variant:${path}`); validateSchema(variant, value, `${path}<${value[spec.discriminator]}>`, context); return }
  if (spec.type === 'operation_discriminated_object') { const operation = context.operation_class; const intent = r54.operation_specs?.[operation]?.intent; if (!intent) throw new Error(`R59_operation_intent:${path}:${operation}`); validateSpec(intent, value, `${path}<${operation}>`, context); return }
  const resolved = referencedSpec(spec)
  ...
  if (Object.hasOwn(spec, 'const') && value !== spec.const) throw new Error(`R59_const:${path}`)
  if (Object.hasOwn(spec, 'literal') && spec.type !== 'sha256_or_exact_literal' && value !== spec.literal) throw new Error(`R59_literal:${path}`)
  const allowed = values(spec)
  if (allowed.length && !allowed.includes(value)) throw new Error(`R59_enum:${path}`)
  ...
  if (acceptedJsonTypes.length && !acceptedJsonTypes.includes('schema_value')) {
    ...
    if (!acceptedJsonTypes.includes(actualJsonType) && !(actualJsonType === 'integer' && acceptedJsonTypes.includes('number'))) throw new Error(`R59_json_type:${path}:${actualJsonType}`)
  }
  if (spec.type === 'sha256' && !(typeof value === 'string' && /^[0-9a-f]{64}$/.test(value))) throw new Error(`R59_sha256:${path}`)
  if (spec.type === 'sha256_or_exact_literal' && !(typeof value === 'string' && (/^[0-9a-f]{64}$/.test(value) || [spec.literal, spec.const, ...(spec.exact_literals ?? [])].filter(item => item !== undefined).includes(value)))) throw new Error(`R59_sha256_or_literal:${path}`)
  if (['identifier', 'human_text', 'literal'].includes(spec.type) && typeof value !== 'string') throw new Error(`R59_string:${path}`)
  if (spec.type === 'identifier' && (value !== value.trim() || value.normalize('NFC') !== value || /[\u0000-\u001f\u007f-\u009f\u200b-\u200f\u202a-\u202e\u2060-\u206f\ufeff]/u.test(value))) throw new Error(`R59_identifier_normalization:${path}`)
  if (spec.type === 'canonical_timestamp' && !strictTimestamp(value)) throw new Error(`R59_time:${path}`)
  if (spec.type === 'base64url_without_padding' && !(typeof value === 'string' && /^[A-Za-z0-9_-]*$/.test(value) && !value.includes('=') && value.length % 4 !== 1 && Buffer.from(value, 'base64url').toString('base64url') === value)) throw new Error(`R59_base64url:${path}`)
  if (spec.type === 'boolean' && typeof value !== 'boolean') throw new Error(`R59_boolean:${path}`)
  if (['integer', 'positive_integer', 'safe_nonnegative_integer', 'nonnegative_integer'].includes(spec.type) && !(Number.isSafeInteger(value) && value >= (spec.type === 'positive_integer' ? 1 : 0))) throw new Error(`R59_integer:${path}`)
  if (spec.type === 'finite_nonnegative_number' && !(Number.isFinite(value) && value >= 0 && !Object.is(value, -0))) throw new Error(`R59_number:${path}`)
  if (spec.minimum !== undefined && value < spec.minimum) throw new Error(`R59_minimum:${path}`)
  if (spec.maximum !== undefined && value > spec.maximum) throw new Error(`R59_maximum:${path}`)
  if (spec.type === 'array') {
    if (!Array.isArray(value)) throw new Error(`R59_array:${path}`)
    if (spec.min_items !== undefined && value.length < spec.min_items) throw new Error(`R59_min_items:${path}`)
    if (spec.max_items !== undefined && value.length > spec.max_items) throw new Error(`R59_max_items:${path}`)
    value.forEach((item, index) => validateSpec(spec.items ?? { type: 'identifier' }, item, `${path}.${index}`, context))
    if (spec.unique && new Set(value.map(item => canonicalR44(item))).size !== value.length) throw new Error(`R59_unique:${path}`)
    if (spec.unique_by && new Set(value.map(item => item?.[spec.unique_by])).size !== value.length) throw new Error(`R59_unique_by:${path}`)
    if (spec.allowed_route_values && value.some(item => !spec.allowed_route_values.includes(item?.route))) throw new Error(`R59_route:${path}`)
    if (Array.isArray(spec.exact_members) && canonicalR44([...value].sort(cp)) !== canonicalR44([...spec.exact_members].sort(cp))) throw new Error(`R59_exact_members:${path}`)
    for (const constraint of spec.item_constraints ?? []) {
      if (constraint === 'string' && value.some(item => typeof item !== 'string')) throw new Error(`R59_item_string:${path}`)
      else if (constraint === 'exact_trimmed_nonempty' && value.some(item => typeof item !== 'string' || !item.trim() || item !== item.trim())) throw new Error(`R59_item_trimmed_nonempty:${path}`)
        ...
        if (value.some(item => reserved.has(item))) throw new Error(`R59_item_reserved:${path}`)
      } else if (!['string', 'exact_trimmed_nonempty', 'not_exact_reserved_default_or_offered_honest_exit'].includes(constraint)) throw new Error(`R59_unknown_item_constraint:${path}:${constraint}`)
    }
  ...
  if (['ordered_identifier_array', 'unicode_sorted_unique_identifier_array'].includes(spec.type)) {
    if (!Array.isArray(value)) throw new Error(`R59_ordered_identifier_array:${path}`)
    for (const [index, item] of value.entries()) validateSpec({ type: 'identifier' }, item, `${path}.${index}`, context)
    if (new Set(value).size !== value.length || canonicalR44([...value].sort(cp)) !== canonicalR44(value)) throw new Error(`R59_ordered_identifier_array_order:${path}`)
  }
  if ((spec.type === 'object' || spec.properties) && (value === null || typeof value !== 'object' || Array.isArray(value))) throw new Error(`R59_object:${path}`)
  if (spec.type === 'object' || spec.properties) validateSchema(spec, value, path, context)
  if (typeof value === 'string') {
    if (spec.valid_unicode_scalar_only && !validUnicodeScalars(value)) throw new Error(`R59_unicode_scalar:${path}`)
    const bytes = Buffer.byteLength(value, 'utf8')
    if (spec.min_utf8_bytes !== undefined && bytes < spec.min_utf8_bytes) throw new Error(`R59_min_utf8_bytes:${path}`)
    if ((spec.max_utf8_bytes ?? spec.max_bytes) !== undefined && bytes > (spec.max_utf8_bytes ?? spec.max_bytes)) throw new Error(`R59_max_utf8_bytes:${path}`)
    const pattern = spec.pattern ?? spec.regex
    if (pattern && !new RegExp(pattern, 'u').test(value)) throw new Error(`R59_pattern:${path}`)
    if (spec.max_decoded_bytes !== undefined && (!(typeof value === 'string' && /^[A-Za-z0-9_-]*$/.test(value) && value.length % 4 !== 1 && Buffer.from(value, 'base64url').toString('base64url') === value) || Buffer.from(value, 'base64url').length > spec.max_decoded_bytes)) throw new Error(`R59_max_decoded_bytes:${path}`)
  }
}
function requireRule(condition, path, kind) { if (!condition) throw new Error(`R59_conditional_${kind}:${path}`) }
function evaluateLocalRule(ruleRow, schema, payload, path) {
  ...
  if (kind === 'intervention_payload_bytes') {
    ...
    if (payload.atom_kind === 'question') validateSchema(r54.proof_value_schemas.question_contract, decoded, `${path}.decoded_question`)
    else if (payload.atom_kind === 'session') validateSchema(r54.authority_operation_intervention_inner_payload_schemas.session, decoded, `${path}.decoded_session`)
  ...
  else if (kind === 'question_text') requireRule(['visible_wording', 'rendered_control_payload', 'material_effect_disclosure', 'visible_changed_consequence', 'visible_unknown_consequence'].every(field => typeof payload[field] === 'string' && payload[field].length > 0 && payload[field] === payload[field].trim()), path, kind)
  ...
  else throw new Error(`R59_unknown_local_predicate:${kind}`)
}
function validateSchema(schema, payload, path, context = {}) {
  if (!schema?.properties || payload === null || typeof payload !== 'object' || Array.isArray(payload)) throw new Error(`R59_schema_object:${path}`)
  const actual = Object.keys(payload).sort(cp), allowed = (schema.exact_keys ?? Object.keys(schema.properties)).sort(cp)
  if (schema.additional_properties === false && actual.some(field => !allowed.includes(field))) throw new Error(`R59_exact_keys:${path}`)
  for (const field of schema.required ?? []) if (!Object.hasOwn(payload, field)) throw new Error(`R59_required:${path}.${field}`)
  ...
  if (schema.max_canonical_bytes !== undefined && Buffer.byteLength(canonicalR44(payload), 'utf8') > schema.max_canonical_bytes) throw new Error(`R59_max_canonical_bytes:${path}`)
  if (schema.max_bytes !== undefined && Buffer.byteLength(canonicalR44(payload), 'utf8') > schema.max_bytes) throw new Error(`R59_row_max_bytes:${path}`)
  for (const [field, value] of Object.entries(payload)) {
    if (field.endsWith('_length') && Object.hasOwn(payload, field.replace(/_length$/, '_b64url'))) {
      const raw = Buffer.from(payload[field.replace(/_length$/, '_b64url')], 'base64url'); if (value !== raw.length) throw new Error(`R59_length_equality:${path}.${field}`)
    }
  }
  for (const rule of schema.conditional_rules ?? []) {
    const row = ruleByText.get(rule)
    if (!row || row.unknown_or_unimplemented !== 'reject') throw new Error(`R59_conditional_rule_not_closed:${path}:${rule}`)
...
function fingerprint(schema, payload) {
  ...
  for (const field of authority.preimage_order) {
    ...
    else throw new Error(`R59_fingerprint_operand_missing:${schema.fingerprint_ref}:${field}`)
  }
...
function finalizeRow(schemaRef, variant, seed, context = {}) {
  ...
  if (!schema?.properties) throw new Error(`R59_fixture_schema_missing:${schemaRef}:${variant}`)
  ...
  if (rule === 'artifact_ref_equals_opaque_bytes_sha256') {
    const bytes = Buffer.from(`R59-OPAQUE:${seed}`, 'utf8'), digest = sha(bytes)
    ...
    if (!selectedSchema?.properties) throw new Error(`R59_canonical_payload_schema_missing:${selectedPayloadSchemaRef}`)
    ...
    payload.parsed_content_fingerprint = selectedFingerprint?.expected_fingerprint ?? hash({ domain_ascii: 'CTRL-G24-R59-PARSED-CONTENT', canonical_bytes_sha256: digest })
  ...
  if (schema.row_ref_derivation && Array.isArray(schema.row_ref_preimage_included_fields)) {
    ...
    if (!field) throw new Error(`R59_row_ref_field_missing:${schemaRef}:${variant}`)
...
function projectedContext(schema, context, strict = true) {
  ...
  for (const [field, dimension] of Object.entries(mappings)) if (schema.properties?.[field] && Object.hasOwn(context, dimension)) {
    ...
    if (allowed.length && !allowed.includes(context[dimension])) { if (strict) throw new Error(`R59_selector_dimension_not_accepted:${field}:${context[dimension]}`); continue }
...
function syntheticSelector(row, role) {
  ...
  const selectorId = `selector_r59_${row.identity_field}`, selectorRef = `authority_operation_joint_identity_selector_contexts.rows.${selectorId}`
  syntheticSelectorRows[selectorId] = { schema_version: 'ctrl.g24.joint-identity-selector-context.r59.v1', selector_id: selectorId, source_schema_ref: row.row_schema_ref, source_schema_variant: row.row_schema_variant, source_schema_version: sourceSchema.schema_version, source_field: row.identity_field, native_identity_role: role, exact_source_schema_valid_context: context, exact_target_case: target, exact_case_count: 1, every_applicable_dimension_equals_generated_source_fixture: true, reduced_projection_or_UNAVAILABLE_for_present_source_field: 'forbidden', caller_override_or_fallback: 'forbidden' }
  return { selector_ref: selectorRef, context, target }
}
function selectorChoice(row, role) {
  const equality = equalityFor(row, role)
  if (!equality) throw new Error(`R59_equality_missing:${row.identity_kind}:${role}`)
  ...
  if (!selector) throw new Error(`R59_selector_missing:${equality.selector_ref}`)
  ...
  if (!pairs.length) throw new Error(`R59_selector_context_missing:${row.identity_kind}`)
...
function persistedWrapperFor(targetRef) {
  ...
  if (selected.length !== 1) throw new Error(`R59_wrapper_resolution_not_exact_one:${targetRef}:${selected.length}`)
  return selected[0]
}
function concreteTarget(selected) {
  ...
  if (!schema?.properties) throw new Error(`R59_target_schema_missing:${target.target_schema_ref}:${target.target_schema_variant}`)
  if (target.target_native_identity_field !== '$canonical_row_bytes' && !schema.properties[target.target_native_identity_field]) throw new Error(`R59_target_identity_field_missing:${target.target_schema_ref}:${target.target_native_identity_field}`)
  return target
}
function targetIdentity(fixture, target) {
  ...
  if (value === undefined) throw new Error(`R59_target_native_identity_missing:${target.target_schema_ref}:${target.target_native_identity_field}`)
  return value
}
...
function jointLinkedFixture(row, role, seed) {
  ...
  const specialize = value => {
    ...
    copy.authority_companion_target_mapping = {
      schema_version: 'ctrl.g24.authority-companion-target-mapping.r59.v1',
  ...
  if (identity === undefined || defaultFingerprint === undefined) throw new Error(`R59_missing_target_identity_or_fingerprint:${row.identity_kind}`)
  ...
  completeJointRows[completeId] = { schema_version: 'ctrl.g24.complete-joint-equality-fixture.r59.v1', equality_source_schema_ref: equality.source_schema_ref, equality_source_schema_variant: equality.source_schema_variant, equality_source_field: equality.source_field, equality_selector_ref: equality.selector_ref, exact_selector_context: exactSourceDimensions, selected_selector_context: choice.context, target_schema_ref: result.target_schema_ref, target_schema_variant: result.target_schema_variant, target_identity_field: result.target_identity_field, target_fixture: result.target_fixture, source_fixture: result.source_fixture, target_native_identity: identity, target_native_bytes_sha256: defaultBytesSha, target_native_fingerprint: defaultFingerprint, declared_companion_bytes_fields: companionBytesFields, declared_companion_fingerprint_fields: companionFingerprintFields, declared_companion_bytes_count: companionBytesFields.length, declared_companion_fingerprint_count: companionFingerprintFields.length, exact_joint_equalities: result.exact_joint_equalities, every_applicable_selector_dimension_must_equal_source_fixture: true }
  completeJointCache.set(cacheKey, result)
  return specialize(result)
}

r54.authority_operation_persisted_identity_formula_library = {
  schema_version: 'ctrl.g24.persisted-identity-formula-library.r59.v1',
  role_precedence: ROLE_PRECEDENCE,
  canonical_row_bytes_content_address: { schema_version: 'ctrl.g24.persisted-identity-formula.canonical-row-bytes.r59.v1', formula_class: 'native_final_row_content_address', exact_formula: 'lowercase_hex_SHA256_of_exact_final_closed_schema_canonical_row_bytes' },
  declared_content_address: { schema_version: 'ctrl.g24.persisted-identity-formula.declared-content-address.r59.v1', formula_class: 'selected_schema_native_content_address', exact_formula: 'opaque_ref_equals_opaque_bytes_sha256_or_artifact_ref_equals_canonical_payload_bytes_sha256_or_exact_declared_row_ref_preimage_or_nonce_receipt_payload_sha256' },
  declared_row_version: { schema_version: 'ctrl.g24.persisted-identity-formula.declared-row-version.r59.v1', formula_class: 'selected_committed_target_native_row_version', exact_formula: 'selected_store_R49_domain_and_version_plus_ordered_complete_mutable_fields_excluding_row_version_and_native_fingerprint' },
  schema_declared_fingerprint: { schema_version: 'ctrl.g24.persisted-identity-formula.schema-fingerprint.r59.v1', formula_class: 'selected_schema_native_fingerprint', exact_formula: 'selected_fingerprint_authority_domain_order_types_and_declared_codec_over_actual_linked_row' },
  unique_key_component: { schema_version: 'ctrl.g24.persisted-identity-formula.unique-key-component.r59.v1', formula_class: 'native_schema_unique_key_component', exact_formula: 'exact_value_from_final_schema_valid_linked_row' },
  resolved_reference_identity: { schema_version: 'ctrl.g24.persisted-identity-formula.resolved-reference.r59.v1', formula_class: 'joint_source_target_native_identity', exact_formula: 'schema_valid_source_reference_equals_recomputed_identity_from_one_exact_selector_context_and_schema_valid_target_fixture' },
  companion_bytes_hash: { schema_version: 'ctrl.g24.persisted-identity-formula.companion-bytes.r59.v1', formula_class: 'joint_source_target_native_bytes', exact_formula: 'schema_valid_source_companion_bytes_equals_selected_target_native_bytes_sha256' },
  companion_fingerprint: { schema_version: 'ctrl.g24.persisted-identity-formula.companion-fingerprint.r59.v1', formula_class: 'joint_source_target_native_fingerprint', exact_formula: 'schema_valid_source_companion_equals_recomputed_selected_target_schema_native_fingerprint' },
}
...
for (const prior of priorIndex) {
  ...
  if (role === 'canonical_row_bytes_content_address') { expected = source.canonical_row_bytes_sha256; evidence = { execution: 'raw_sha256_final_canonical_row_bytes', exact_operand_b64url: source.canonical_row_bytes_b64url, expected_identity: expected } }
    ...
    if (!native) throw new Error(`R59_native_content_address_missing:${identityKind}`)
    ...
    if (!source.row_version) throw new Error(`R59_native_row_version_missing:${identityKind}`)
...
const orderingHandlers = {
  ...
  matching_evidence_refs: { operand: 'sibling.evidence_refs', comparator: 'same_length_and_index_alignment_to_unsigned_UTF8_ordered_evidence_refs' },
}
if (orderingOccurrences.length !== 21 || orderingOccurrences.some(row => !orderingHandlers[row.exact_ordering_authority])) throw new Error('R59_ordering_inventory_not_closed')
r54.authority_operation_ordering_rule_registry = { schema_version: 'ctrl.g24.ordering-rule-registry.r59.v1', exact_occurrence_count: orderingOccurrences.length, exact_handler_count: Object.keys(orderingHandlers).length, handlers: orderingHandlers, occurrences: orderingOccurrences, unknown_ordering_authority: 'reject_materialization_and_hold_without_write', every_occurrence_executes_selected_handler: true }
...
r54.schema_change_manifest.repair_roots = ['all_21_ordered_by_occurrences_resolve_one_closed_handler', 'generic_and_named_array_semantics', 'intervention_exact_canonical_inner_bytes_and_kind_schema', 'exact_trimmed_effect_and_proposal_text', 'independently_derived_complete_unproved_identity_exclusions', 'preserve_R58_honest_semantic_boundary_and_joint_evidence']
r54.required_negative_fixture_families = [...new Set([...r54.required_negative_fixture_families, 'ordered_by_each_family_and_actual_rows', 'array_exact_members_named_types_duplicates_and_order', 'intervention_noncanonical_inner_bytes_and_keyset', 'exact_trimmed_effect_and_proposal_text', 'conditional_exclusion_empty_drop_duplicate_substitute'])]
...
r54.authority_runtime_semantic_reference_field_registry = { ...r54.authority_runtime_semantic_reference_field_registry, schema_version: 'ctrl.g24.runtime-semantic-reference-field-registry.r59.v1' }
r54.authority_runtime_semantic_reference_owner_map = { ...r54.authority_runtime_semantic_reference_owner_map, schema_version: 'ctrl.g24.runtime-semantic-reference-owner-map.r59.v1' }
r54.authority_runtime_semantic_dependency_owner_map = { ...r54.authority_runtime_semantic_dependency_owner_map, schema_version: 'ctrl.g24.runtime-semantic-dependency-owner-map.r59.v1' }
r54.authority_runtime_semantic_manifest = { ...r54.authority_runtime_semantic_manifest, schema_version: 'ctrl.g24.runtime-semantic-authority-manifest.r59.v1' }
...
r54.authority_runtime_semantic_manifest_hash_contract = { ...r54.authority_runtime_semantic_manifest_hash_contract, schema_version: 'ctrl.g24.runtime-semantic-manifest-hash-contract.r59.v1', manifest_hash_version: 'ctrl.g24.runtime-semantic-manifest-hash.r59.v1', content_domain_ascii: 'CTRL-G24-R59-MANIFEST-CONTENT', dependency_domain_ascii: 'CTRL-G24-R59-MANIFEST-DEPENDENCY', graph_domain_ascii: 'CTRL-G24-R59-MANIFEST-GRAPH', envelope_domain_ascii: 'CTRL-G24-R59-MANIFEST-ENVELOPE' }
...
const snapshotSha = hash({ domain_ascii: 'CTRL-G24-R59-FINAL-SEMANTIC-SOURCE-SNAPSHOT', exact_paths: sourcePaths, values: sourceSnapshot })
...
r54.authority_runtime_semantic_reference_owner_map = { schema_version: 'ctrl.g24.runtime-semantic-reference-owner-map.r59.v1', source_snapshot_sha256: snapshotSha, exact_row_count: refRows.length, reference_registry_ref: 'authority_runtime_semantic_reference_field_registry', schema_declared_and_exact_path_value_bijection: true }
r54.authority_runtime_semantic_dependency_owner_map = { schema_version: 'ctrl.g24.runtime-semantic-dependency-owner-map.r59.v1', source_snapshot_sha256: snapshotSha, exact_paths: sourcePaths, rows: sourcePaths.map(path => ({ authority_path: path, typed_owner_paths: dependencies[path] })), reference_registry_ref: 'authority_runtime_semantic_reference_field_registry' }
...
const withoutSeal = { schema_version: 'ctrl.g24.runtime-semantic-authority-manifest.r59.v1', source_snapshot_sha256: snapshotSha, exact_paths: sourcePaths, rows: manifestRows, exact_expected_count: manifestRows.length, manifest_graph_sha256: hash({ domain_ascii: hc.graph_domain_ascii, manifest_hash_version: hc.manifest_hash_version, manifest_rows: manifestRows }) }
...
export const materializedR59 = r54
export const materializedR59Output = `${JSON.stringify(finalSnapshot, null, 2)}\n`
export const r59SemanticAuthorityPaths = sourcePaths
const invoked = process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)
if (invoked) {
  const mode = process.argv[2] ?? '--check'
  if (mode === '--write') { writeFileSync(join(root, outputPath), materializedR59Output); console.log(`wrote ${outputPath}`) }
  else if (mode === '--check') { if (readFileSync(join(root, outputPath), 'utf8') !== materializedR59Output) { console.error(`${outputPath} differs from materializer`); process.exit(1) } console.log(`ok: ${outputPath} is the exact fully materialized R59 effective contract`) }
