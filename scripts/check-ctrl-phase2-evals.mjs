...
const expectedDecisionIds = Array.from({ length: 54 }, (_, index) => `D-${String(index + 1).padStart(3, '0')}`);
...
function validateTrace(trace, raw, ledgerDecisions) {
  ...
  if (trace.authority !== 'founder_approved_via_D-052_through_D-054') fail('traceability authority must point to D-052 through D-054');
  if (trace.source_of_truth !== ledgerRel) fail('traceability source_of_truth must remain the canonical ledger snapshot');
  if (trace.decision_range !== 'D-001..D-054') fail('traceability decision_range must remain D-001..D-054');
  if (trace.next_design_gate !== 'D-054 closes the first material mock gate; production implementation remains gated while G12 defines the living Brain domain and user model.') {
    fail('traceability must preserve the G12 Brain-model implementation gate');
  }
  ...
  if (ids.length !== expectedDecisionIds.length || !ids.every((id, index) => id === expectedDecisionIds[index])) {
    fail('traceability entries must be ordered, unique and complete from D-001 through D-054');
  }
  ...
  if (ledgerDecisions.size !== expectedDecisionIds.length) fail('ledger decision set has drifted beyond D-001 through D-054 without a traceability update');
...
function runTraceNegativeProbes(trace, ledgerDecisions) {
  const probes = [
    ['missing D-054', (copy) => copy.entries.pop()],
...
if (pack) {
  ...
  if (!failures.length) {
    ...
    console.log(`ok: decision traceability ${trace.entries.length} locked decisions through D-054`);
