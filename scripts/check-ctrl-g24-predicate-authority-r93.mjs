...
const directory = 'project-documentation/ctrl-evolution/g24-predicate-authority-r93'
...
const expectedTopKeys = {
  effective_resolution: ['schema_version', 'module_id', 'semantic_base', 'imported_exact_sections', 'r93_replacement_sections', 'inheritance_rule', 'unknown_or_conflicting_section', 'authority_still_closed'],
...
check('accepted final registry schemas closed', same(correction.accepted_final_registry_required, ['registry_id', 'records']) && Array.isArray(correction.accepted_final_record_required) && correction.accepted_final_record_required.length > 10 && Array.isArray(correction.accepted_final_challenge_required) && Array.isArray(correction.challenge_observation_required) && Array.isArray(correction.descendant_block_required) && Array.isArray(correction.revalidation_authority_entry_required) && Array.isArray(correction.revalidation_record_required) && Array.isArray(correction.dependency_baseline_required) && same(correction.current_status_projection_required, ['status', 'message']) && Array.isArray(trust.answer_state_revalidation_head_required) && Array.isArray(correction.answer_state_repair_rules) && correction.answer_state_repair_rules.length >= 10)
...
check('manifest schema and authority metadata exact', manifest.schema_version === 'ctrl.g24.predicate-authority.r93.manifest.v1' && manifest.standing === 'bound_local_contract_candidate_no_runtime_authority' && manifest.decision_id === 'DEC-20260916-g24-predicate-authority-r75' && canonical(manifest.semantic_base) === canonical(resolution.semantic_base) && manifest.resolution_rule === resolution.inheritance_rule)
...
function buildFinalReceipt(transitionId, predicateProof) {
  ...
  const authorityProofs = humans.map(human => ({ named_human_id: human, authentication_event_ref: human === 'krish' ? 'auth-r93-krish' : 'auth-r93-leader', authority_event_ref: `final-authority-${transitionId}-${human}` }))
...
if (process.env.R93_EMIT_FINAL_UNSIGNED === '1') {
  ...
  console.log(`R93_UNSIGNED_FINALS=${JSON.stringify(emitted)}`)
  process.exit(0)
}
...
const acceptedFinalService = (() => {
  ...
  const currentStatus = acceptedFinalId => {
    ...
    if (open.length) {
      const state = currentHeadState(open, false), missingHeads = state.heads.filter(head => head.standing !== 'current'), responsibleHumans = byteSorted([...new Set(missingHeads.map(head => specimens.answer_state_transition_events[head.transition_event_ref]?.named_human_id).filter(value => typeof value === 'string' && value.length))])
      if (missingHeads.length) {
        const count = missingHeads.length, countText = count === 1 ? 'One' : count === 2 ? 'Two' : String(count), ownerLabels = responsibleHumans.map(value => value === 'krish' ? 'Krish' : value === 'leader-1' ? 'the leader' : 'the answer owner'), owners = ownerLabels.length === 1 ? ownerLabels[0] : ownerLabels.length === 2 ? `${ownerLabels[0]} and ${ownerLabels[1]}` : 'The answer owners'
        return { status: 'replacement_required', message: count === 1 ? `${countText} answer needs replacing. ${owners} must choose it, then review all changes with Krish.` : `${countText} answers need replacing. ${owners} must each choose one, then review all changes with Krish.`, required_replacements: count, responsible_humans: responsibleHumans }
      }
...
if (process.env.R93_EMIT_MULTI_CHANGE_UNSIGNED === '1') {
  sessionAuthorityService.resetForConformance(); answerAuthorityService.resetForConformance(); acceptedFinalService.resetForConformance()
  const built = buildPredicateProof('accept_intensive_proof'), final = buildFinalReceipt('accept_intensive_proof', built.proof)
  ...
  console.log(`R93_UNSIGNED_MULTI_CHANGE=${JSON.stringify(unsigned)}`)
  process.exit(0)
}
if (process.env.R93_EMIT_MIXED_CHANGE_UNSIGNED === '1') {
  sessionAuthorityService.resetForConformance(); answerAuthorityService.resetForConformance(); acceptedFinalService.resetForConformance()
  const built = buildPredicateProof('accept_intensive_proof'), final = buildFinalReceipt('accept_intensive_proof', built.proof)
  ...
  console.log(`R93_UNSIGNED_MIXED_CHANGE=${JSON.stringify(unsigned)}`)
  process.exit(0)
}
if (process.env.R93_EMIT_DOUBLE_WITHDRAW_UNSIGNED === '1') {
  sessionAuthorityService.resetForConformance(); answerAuthorityService.resetForConformance(); acceptedFinalService.resetForConformance()
  const built = buildPredicateProof('accept_intensive_proof'), final = buildFinalReceipt('accept_intensive_proof', built.proof)
  ...
  console.log(`R93_UNSIGNED_DOUBLE_WITHDRAW=${JSON.stringify(unsigned)}`)
  process.exit(0)
}
...
}
{
  ...
  if (head) {
    ...
    check('single authoritative status says both replacement and review are required', replacementStatus.status === 'replacement_required' && replacementStatus.required_replacements === 1 && same(replacementStatus.responsible_humans, [specimens.answer_state_transition_events[challenge.trigger_transition_ref].named_human_id]) && replacementStatus.message.startsWith('One answer needs replacing.'))
    const reanswer = Object.values(specimens.answer_state_transition_events).find(event => event.route_ref === head.route_ref && event.question_id === head.question_id && event.action === 'reanswer')
...
}
{
  ...
  check('null empty unknown and reset-stale decision ids are typed unavailable', same(acceptedFinalService.currentStatus(null), unavailable) && same(acceptedFinalService.currentStatus(''), unavailable) && same(acceptedFinalService.currentStatus('accepted-does-not-exist'), unavailable))
}
{
  sessionAuthorityService.resetForConformance(); answerAuthorityService.resetForConformance(); acceptedFinalService.resetForConformance()
  const accepted = buildPredicateProof('accept_intensive_proof'), final = buildFinalReceipt('accept_intensive_proof', accepted.proof)
...
}
{
  ...
  check('two withdrawals expose the exact remaining replacement count and owners in both append orders', Boolean(forward && reverse) && same(forward.zeroReplacedStatus, reverse.zeroReplacedStatus) && forward.zeroReplacedStatus.status === 'replacement_required' && forward.zeroReplacedStatus.required_replacements === 2 && same(forward.zeroReplacedStatus.responsible_humans, ['krish', 'leader-1']) && forward.zeroReplacedStatus.message === 'Two answers need replacing. Krish and the leader must each choose one, then review all changes with Krish.' && forward.zeroReviewRequests === 0 && reverse.zeroReviewRequests === 0)
  check('one of two replacements leaves one truthful owner-bound action and no review request', Boolean(forward && reverse) && forward.oneReplacedStatus.status === 'replacement_required' && forward.oneReplacedStatus.required_replacements === 1 && forward.oneReplacedStatus.responsible_humans.length === 1 && forward.oneReplacedStatus.message.startsWith('One answer needs replacing.') && forward.oneReviewRequests === 0 && reverse.oneReviewRequests === 0)
...
if (failures.length) {
  console.error(`R93 independent conformance failed (${failures.length})`)
  for (const failure of failures) console.error(`- ${failure}`)
  process.exit(1)
}
console.log(`ok: R93 end-to-end bound contract; ${imports.transition_catalogue.length}/13 predicate and final transition paths; ${vectorGroups.reduce((sum, group) => sum + vectors[group].length, 0)} executable vectors; ${totalityProbeCount} generated totality probes`)
