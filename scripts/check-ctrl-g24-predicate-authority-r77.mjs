...
check('canonical next action remains inside approved local contract gate', state.includes('**CURRENT_NEXT_ACTION:** Freeze the exact [R87 authoritative answer-state and atomic finality contract]') && state.includes('No semantic evaluator implementation'))
check('design route agrees', design.includes('freeze the exact R87 authoritative answer-state and atomic finality contract') && design.includes('No semantic evaluator implementation'))
