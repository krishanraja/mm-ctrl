...
check('canonical next action remains inside approved local contract gate', state.includes('**CURRENT_NEXT_ACTION:** Freeze the exact [R89 immutable accepted history and signed revalidation contract]') && state.includes('No semantic evaluator implementation'))
check('design route agrees', design.includes('freeze the exact R89 immutable accepted history and signed revalidation contract') && design.includes('No semantic evaluator implementation'))
