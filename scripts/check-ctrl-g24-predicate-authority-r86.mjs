...
const directory = 'project-documentation/ctrl-evolution/g24-predicate-authority-r86'
...
const expectedTopKeys = {
  effective_resolution: ['schema_version', 'module_id', 'semantic_base', 'imported_exact_sections', 'r86_replacement_sections', 'inheritance_rule', 'unknown_or_conflicting_section', 'authority_still_closed'],
...
check('manifest schema and authority metadata exact', manifest.schema_version === 'ctrl.g24.predicate-authority.r86.manifest.v1' && manifest.standing === 'bound_local_contract_candidate_no_runtime_authority' && manifest.decision_id === 'DEC-20260916-g24-predicate-authority-r75' && canonical(manifest.semantic_base) === canonical(resolution.semantic_base) && manifest.resolution_rule === resolution.inheritance_rule)
...
function validateFreeConfirmation(event, receipt, question, render, interpretation, interpretationRender) {
  const reject = reason => { if (process.env.R86_DEBUG) console.error('free confirmation reject', reason); return false }
  ...
  if (Object.values(bindingChecks).includes(false)) { if (process.env.R86_DEBUG) console.error(bindingChecks); return reject('binding') }
  const unsigned = clone(event); delete unsigned.issuer_signature_base64
...
const sessionAuthorityService = (() => {
  const issued = new Map(Object.values(specimens.route_receipts).map(receipt => [routeReceiptRef(receipt), clone(receipt)])), reservations = new Map(), proofBindings = new Map(), answerBindings = new Map()
  ...
  const bindToProof = (receipts, answerClaims, proofFingerprint) => {
    if (!Array.isArray(receipts) || !Array.isArray(answerClaims) || typeof proofFingerprint !== 'string') return false
    ...
    for (const claim of answerClaims) {
      if (!exactKeys(claim, ['route_ref', 'question_id', 'answer_ref']) || !resolve(claim.route_ref)) return false
      const key = `${claim.route_ref}|${claim.question_id}`, prior = answerBindings.get(key)
      if (prior && (prior.answer_ref !== claim.answer_ref || prior.proof_fingerprint !== proofFingerprint)) return false
    }
    for (const receipt of receipts) proofBindings.set(routeReceiptRef(receipt), proofFingerprint)
    for (const claim of answerClaims) answerBindings.set(`${claim.route_ref}|${claim.question_id}`, { answer_ref: claim.answer_ref, proof_fingerprint: proofFingerprint })
    return true
  }
  return Object.freeze({ route, resolve, resolveIssued, bindToProof, resetForConformance: () => { reservations.clear(); proofBindings.clear(); answerBindings.clear() } })
})()
...
function validateFinalAuthorityEvent(event, proof, receipt) {
  const reject = reason => { if (process.env.R86_DEBUG) console.error('final authority reject', reason, proof?.authority_event_ref); return false }
...
function buildPredicateProof(transitionId) {
  const conditionalContext = authoritativeConditionalContext(transitionId), expectedIds = requiredDependencyIds(transitionId), vectorBinding = bindingForTransition(transitionId), routeByHuman = transitionById[transitionId].normative_dependencies.length ? buildRouteReceipts(transitionId) : {}, predecessor = resolveClosedPredecessor(transitionId)
...
function replaceProofNormativeOptions(built, selections) {
...
function validatePredicate(proof, evidence) {
  ...
  for (const result of proof.dependency_results) {
    ...
    if (expectedKind === 'profile') {
      ...
      if (profileResult !== 'valid') { if (process.env.R86_DEBUG) console.error('predicate profile invalid', result.dependency_id, profileResult); return 'evidence_invalid' }
  ...
  const answerClaims = validatedNormativeItems.map(item => ({ route_ref: routeReceiptRef(item.route_receipt), question_id: item.question.question_id, answer_ref: item.value.answer_event_ref ?? item.value.free_expression_confirmation_ref }))
  if (result === 'satisfied' && !sessionAuthorityService.bindToProof(proof.route_receipts, answerClaims, proof.proof_fingerprint)) return 'route_invalid'
  return result
}
...
check('predicate validation never issues or resets route authority', !/buildRouteReceipts|\.route\(|resetForConformance/.test(validatePredicate.toString()))
{
  ...
  check('first proof claims routes and competing proof is rejected', validatePredicate(first.proof, first.evidence) === 'satisfied' && validatePredicate(competing.proof, competing.evidence) === 'route_invalid')
  sessionAuthorityService.resetForConformance()
  const firstReversed = buildPredicateProof('accept_intensive_proof'), competingReversed = clone(firstReversed)
  check('reversed competing proof fixture has a second valid decision', replaceProofNormativeOptions(competingReversed, selections))
  check('competing proof claims routes and first proof is rejected in reverse order', validatePredicate(competingReversed.proof, competingReversed.evidence) === 'satisfied' && validatePredicate(firstReversed.proof, firstReversed.evidence) === 'route_invalid')
}
...
check('mandatory vector bodies pinned', hashValue(Object.fromEntries(vectorGroups.map(group => [group, vectors[group]]))) === 'fd5d6def878395024290245d87866c6d3e4cc7a380f9c4ace1fa30af6bcf4c05')
...
if (failures.length) {
  console.error(`R86 independent conformance failed (${failures.length})`)
  for (const failure of failures) console.error(`- ${failure}`)
  process.exit(1)
}
console.log(`ok: R86 end-to-end bound contract; ${imports.transition_catalogue.length}/13 predicate and final transition paths; ${vectorGroups.reduce((sum, group) => sum + vectors[group].length, 0)} executable vectors; ${totalityProbeCount} generated totality probes`)
