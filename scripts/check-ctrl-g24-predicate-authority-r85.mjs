...
const directory = 'project-documentation/ctrl-evolution/g24-predicate-authority-r85'
...
const expectedTopKeys = {
  effective_resolution: ['schema_version', 'module_id', 'semantic_base', 'imported_exact_sections', 'r85_replacement_sections', 'inheritance_rule', 'unknown_or_conflicting_section', 'authority_still_closed'],
  ...
  interaction_program: ['schema_version', 'module_id', 'session_state_required', 'unresolved_gap_set_required', 'question_reservation_required', 'krish_session_receipt_required', 'question_required', 'render_receipt_required', 'final_decision_render_required', 'free_expression_interpretation_required', 'free_expression_render_required', 'canonical_case_context_required', 'copy_limits', 'technical_vocabulary_forbidden', 'routing_rules', 'question_rules', 'judgement_contracts', 'default_surface'],
...
check('manifest schema and authority metadata exact', manifest.schema_version === 'ctrl.g24.predicate-authority.r85.manifest.v1' && manifest.standing === 'bound_local_contract_candidate_no_runtime_authority' && manifest.decision_id === 'DEC-20260916-g24-predicate-authority-r75' && canonical(manifest.semantic_base) === canonical(resolution.semantic_base) && manifest.resolution_rule === resolution.inheritance_rule)
...
function validateFreeConfirmation(event, receipt, question, render, interpretation, interpretationRender) {
  const reject = reason => { if (process.env.R85_DEBUG) console.error('free confirmation reject', reason); return false }
  ...
  if (Object.values(bindingChecks).includes(false)) { if (process.env.R85_DEBUG) console.error(bindingChecks); return reject('binding') }
  const unsigned = clone(event); delete unsigned.issuer_signature_base64
...
const sessionAuthorityService = (() => {
  ...
  return Object.freeze({ route, resolve, bindToProof, resetForConformance: () => { reservations.clear(); proofBindings.clear() } })
})()
...
function validateFinalDecisionRender(render, proof) {
  if (!finiteJson(render) || !finiteJson(proof) || !exactKeys(render, interaction.final_decision_render_required) || !exactKeys(proof, semantic.predicate_proof_required) || fingerprint(render, 'receipt_fingerprint') !== render.receipt_fingerprint) return false
  const expected = buildFinalDecisionRender(proof)
  return canonical(render) === canonical(expected) && render.displayed_consequence_text.includes('Aperture House') && !render.displayed_consequence_text.includes(proof.transition_id)
}
...
function validateFinalAuthorityEvent(event, proof, receipt) {
  const reject = reason => { if (process.env.R85_DEBUG) console.error('final authority reject', reason, proof?.authority_event_ref); return false }
...
function validatePredicate(proof, evidence) {
  ...
  for (const result of proof.dependency_results) {
    ...
    if (expectedKind === 'profile') {
      ...
      if (profileResult !== 'valid') { if (process.env.R85_DEBUG) console.error('predicate profile invalid', result.dependency_id, profileResult); return 'evidence_invalid' }
  ...
  if (result === 'satisfied' && !sessionAuthorityService.bindToProof(proof.route_receipts, proof.proof_fingerprint)) return 'route_invalid'
  return result
}
...
check('mandatory vector bodies pinned', hashValue(Object.fromEntries(vectorGroups.map(group => [group, vectors[group]]))) === '52f4b69fff34a773351bff3181dbcbd17f68fe62c5a131da4739e2f3e1aee5e6')
...
if (failures.length) {
  console.error(`R85 independent conformance failed (${failures.length})`)
  for (const failure of failures) console.error(`- ${failure}`)
  process.exit(1)
}
console.log(`ok: R85 end-to-end bound contract; ${imports.transition_catalogue.length}/13 predicate and final transition paths; ${vectorGroups.reduce((sum, group) => sum + vectors[group].length, 0)} executable vectors; ${totalityProbeCount} generated totality probes`)
