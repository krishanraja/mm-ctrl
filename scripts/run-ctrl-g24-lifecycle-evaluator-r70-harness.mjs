...
const DEFAULT_R70_PATH = 'project-documentation/ctrl-evolution/g24-lifecycle-precondition-evaluator-r70.json'
const DEFAULT_SOURCE_PATH = 'supabase/functions/_shared/g24-lifecycle-precondition-evaluator.r70.mjs'
const PINNED_R66_SHA256 = 'b930cff4b346aad614cc0a576ab7160a7846a5f1e9f5cb27f864d286fd32a920'
const PINNED_R70_MACHINE_SHA256 = 'bc83f908bc9b4a2fee46917ab9501b6ae730c5fe419b7f3c1e2a55c59314cb80'
const PINNED_SOURCE_SHA256 = 'e7b70f3816b38fa3744c1a86e627e835b770765745e370e569a350ddb1df05c9'
const PINNED_SOURCE_BYTES = 7451
const OUTPUT_SCHEMA_VERSION = 'ctrl.g24.executable-output.evaluate-lifecycle-preconditions.r70.v1'
...
const readPinnedJson = (path, expectedSha256, label) => {
  const bytes = readFileSync(join(root, path))
  assert(sha256(bytes) === expectedSha256, `${label}:raw_bytes_sha256`)
  const text = bytes.toString('utf8')
  assert(Buffer.from(text, 'utf8').equals(bytes), `${label}:utf8_roundtrip`)
  try { return JSON.parse(text) } catch { throw new Error(`${label}:json`) }
}
...
export function verifyR66LifecycleSelection(contract, r70, recordOverride) {
  ...
  assert(r70.executable_artifact.descriptor_content_ref === descriptor.descriptor_content_ref && r70.executable_artifact.descriptor_bytes_sha256 === descriptor.canonical_descriptor_bytes_sha256, 'selection:R70_descriptor_binding')

  const binding = r70.r66_selection_binding
  assert(same(binding, {
...
export function inspectExecutableSource(sourceText, entrypoint) {
  const file = ts.createSourceFile('g24-lifecycle-precondition-evaluator.r70.mjs', sourceText, ts.ScriptTarget.ESNext, true, ts.ScriptKind.JS)
...
function verifyArtifact(r70, executableBytes) {
  const expected = r70.executable_artifact
  ...
  assert(artifact.schema_version === 'ctrl.g24.evaluator-executable-artifact.r70.v1', 'artifact:schema')
  assert(artifact.source_path === DEFAULT_SOURCE_PATH, 'artifact:path')
  ...
  assert(artifact.entrypoint === 'dispatchEvaluateLifecyclePreconditionsStructuralR70', 'artifact:entrypoint')
  ...
  assert(lock.schema_version === 'ctrl.g24.evaluator-source-lock.r70.v1' && lock.domain_ascii === 'CTRL-G24-EVALUATOR-SOURCE-LOCK-R70', 'artifact:lock_schema_domain')
...
export function runIsolatedModule(executableBytes, entrypoint, serializedInput, options = {}) {
  ...
  assert(inputBytes.toString('utf8') === serializedInput, 'isolate:input_utf8_roundtrip')
...
function validateClosedDispatch(r70, output) {
  const schema = r70.dispatch_output_schema
...
export function runR70StructuralGate(dispatchInputText) {
  try {
    assert(dispatchInputText === undefined || typeof dispatchInputText === 'string', 'gate:primitive_input_only')
    const contract = readPinnedJson(DEFAULT_R66_PATH, PINNED_R66_SHA256, 'contract')
    const gate = readPinnedJson(DEFAULT_R70_PATH, PINNED_R70_MACHINE_SHA256, 'gate')
    const bytes = readFileSync(join(root, DEFAULT_SOURCE_PATH))
    assert(sha256(bytes) === PINNED_SOURCE_SHA256 && bytes.length === PINNED_SOURCE_BYTES, 'source:raw_bytes_identity')
    verifyR66LifecycleSelection(contract, gate)
    const verifiedArtifact = verifyArtifact(gate, bytes)
    const inputText = dispatchInputText ?? canonical(gate.sample_dispatch_input)
    const options = { maxInputBytes: MAX_INPUT_BYTES, maxRequestBytes: MAX_REQUEST_BYTES, maxOutputBytes: MAX_OUTPUT_BYTES, timeoutMs: TIMEOUT_MS }
    ...
    return { loaded: false, dispatch: trustedHoldOutput() }
  }
}

if (process.argv[1] && process.argv[1].endsWith('run-ctrl-g24-lifecycle-evaluator-r70-harness.mjs')) {
  const result = runR70StructuralGate()
