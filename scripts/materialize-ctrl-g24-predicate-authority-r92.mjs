...
function buildEffective(modules) {
  ...
  return {
    schema_version: 'ctrl.g24.predicate-authority.r92.effective-contract.v1',
...
function build() {
  ...
  const withoutFingerprint = {
    schema_version: 'ctrl.g24.predicate-authority.r92.manifest.v1',
...
function main() {
  ...
  if (!['--write', '--check'].includes(mode)) { console.error('usage: node scripts/materialize-ctrl-g24-predicate-authority-r92.mjs --write|--check'); process.exit(2) }
  const built = build()
  if (mode === '--write') {
  ...
  if (!readFileSync(join(root, effectivePath)).equals(built.effectiveBytes) || !readFileSync(join(root, manifestPath)).equals(built.manifestBytes)) { console.error('R92 materialized bytes are stale'); process.exit(1) }
  const manifest = JSON.parse(built.manifestBytes)
  console.log(`ok: ${manifest.modules.length} R92 source modules; ${manifest.bundle_fingerprint}`)
}
