...
const inputPath = 'project-documentation/ctrl-evolution/g24-trusted-ingress-contract-r54.json'
...
if (inputBytes !== materializedR54Output) throw new Error('R55_frozen_R54_input_mismatch')
...
r54.schema_version = 'ctrl.g24.trusted-ingress.r55.effective.v1'
r54.status = ['founder_locked_direction', 'headless_kernel_independently_verified', 'trusted_ingress_r1_through_r54_vetoed', 'trusted_ingress_r55_fully_materialized', 'independent_attack_required', 'no_adapter_or_runtime_connection']
r54.supersedes = { commit: '522a6f04e010bec9eefe35f52f0016ef2f5b7662', tree: '7f87ecdb2bcc44420ccd9c7cc5dcb6cb2d40c4fb', human_blob: 'c2520342ead9f3803820f43ca79e8a303f43a6d7', machine_blob: '1565c5c95804fea869bdfd50c27bed8a79a28557', qa_blob: 'c1049bed766c47d2483d99342bd40be0fc4a60c0', checker_blob: '97aa7576866942ee257cc0bd6d595702b0135560', materializer_blob: 'd4b16ccd2c4c2864e7ce9506bc89f8dc363bb59a', founder_checker_blob: '824c4aa10dba795dcfdd022b133aa3e41b249d34', adjudication: 'veto' }
r54.materialization = { ...r54.materialization, schema_version: 'ctrl.g24.trusted-ingress-materialization.r55.v1', frozen_input: { path: inputPath, sha256: sha(inputBytes) }, output_path: outputPath, strict_finalization_dag: ['freeze_R54_input', 'derive_native_role_union_with_precedence', 'build_complete_schema_valid_joint_source_target_fixtures', 'resolve_native_identity_without_fallback', 'compute_target_then_source_identities', 'validate_source_selector_context', 'finalize_nonderived_authorities', 'snapshot_final_semantic_sources', 'derive_reference_owner_and_manifest', 'seal_output'], no_post_snapshot_source_write: true }

function values(spec) { if (!spec) return []; if (Object.hasOwn(spec, 'const')) return [spec.const]; if (Object.hasOwn(spec, 'literal')) return [spec.literal]; return spec.values ?? spec.enum ?? [] }
function referencedSpec(spec) {
  const ref = spec?.schema_ref ?? spec?.value_schema?.schema_ref
  if (!ref) return null
  const resolved = get(r54, ref)
  if (!resolved) throw new Error(`R55_schema_ref_missing:${ref}`)
  return resolved
}
...
function valueForSpec(spec, seed, context = {}) {
  if (!spec || typeof spec !== 'object') return `r55_${seed}`
  ...
  if (spec.type === 'sha256') return sha(`R55:${seed}`)
  if (spec.type === 'sha256_or_exact_literal') return (spec.exact_literals ?? [])[0] ?? sha(`R55:${seed}`)
  if (spec.type === 'canonical_timestamp') return '2031-01-01T00:00:00.000Z'
  if (spec.type === 'base64url_without_padding') return Buffer.from(`R55:${seed}`, 'utf8').toString('base64url')
  ...
  return `r55_${String(seed).replace(/[^a-z0-9_]/gi, '_').slice(-96)}`
}
...
function validateSpec(spec, value, path, context = {}) {
  ...
  if (Object.hasOwn(spec, 'const') && value !== spec.const) throw new Error(`R55_const:${path}`)
  if (Object.hasOwn(spec, 'literal') && spec.type !== 'sha256_or_exact_literal' && value !== spec.literal) throw new Error(`R55_literal:${path}`)
  const allowed = spec.values ?? spec.enum
  if (Array.isArray(allowed) && !allowed.includes(value)) throw new Error(`R55_enum:${path}`)
  if (spec.type === 'sha256' && !(typeof value === 'string' && /^[0-9a-f]{64}$/.test(value))) throw new Error(`R55_sha256:${path}`)
  if (spec.type === 'sha256_or_exact_literal' && !(typeof value === 'string' && (/^[0-9a-f]{64}$/.test(value) || [spec.literal, spec.const, ...(spec.exact_literals ?? [])].filter(item => item !== undefined).includes(value)))) throw new Error(`R55_sha256_or_literal:${path}`)
  if (['identifier', 'human_text', 'literal'].includes(spec.type) && typeof value !== 'string') throw new Error(`R55_string:${path}`)
  if (spec.type === 'canonical_timestamp' && !(typeof value === 'string' && /^\d{4}-\d\d-\d\dT\d\d:\d\d:\d\d\.\d{3}Z$/.test(value) && Number.isFinite(Date.parse(value)))) throw new Error(`R55_time:${path}`)
  if (spec.type === 'base64url_without_padding' && !(typeof value === 'string' && /^[A-Za-z0-9_-]*$/.test(value) && !value.includes('='))) throw new Error(`R55_base64url:${path}`)
  if (spec.type === 'boolean' && typeof value !== 'boolean') throw new Error(`R55_boolean:${path}`)
  if (['integer', 'positive_integer', 'safe_nonnegative_integer', 'nonnegative_integer'].includes(spec.type) && !(Number.isSafeInteger(value) && value >= (spec.type === 'positive_integer' ? 1 : 0))) throw new Error(`R55_integer:${path}`)
  if (spec.type === 'finite_nonnegative_number' && !(Number.isFinite(value) && value >= 0 && !Object.is(value, -0))) throw new Error(`R55_number:${path}`)
  if (spec.minimum !== undefined && value < spec.minimum) throw new Error(`R55_minimum:${path}`)
  if (spec.maximum !== undefined && value > spec.maximum) throw new Error(`R55_maximum:${path}`)
  if (spec.type === 'nullable' && value !== null) throw new Error(`R55_nullable:${path}`)
  if (spec.type === 'array') {
    if (!Array.isArray(value)) throw new Error(`R55_array:${path}`)
    if (spec.min_items !== undefined && value.length < spec.min_items) throw new Error(`R55_min_items:${path}`)
    if (spec.max_items !== undefined && value.length > spec.max_items) throw new Error(`R55_max_items:${path}`)
    value.forEach((item, index) => validateSpec(spec.items ?? { type: 'identifier' }, item, `${path}.${index}`, context))
    if (spec.unique && new Set(value.map(item => canonicalR44(item))).size !== value.length) throw new Error(`R55_unique:${path}`)
    if (spec.unique_by && new Set(value.map(item => item?.[spec.unique_by])).size !== value.length) throw new Error(`R55_unique_by:${path}`)
    if (spec.allowed_route_values && value.some(item => !spec.allowed_route_values.includes(item?.route))) throw new Error(`R55_route:${path}`)
  }
  if ((spec.type === 'object' || spec.properties) && (value === null || typeof value !== 'object' || Array.isArray(value))) throw new Error(`R55_object:${path}`)
  if (spec.type === 'object' || spec.properties) validateSchema(spec, value, path, context)
  if (typeof value === 'string') {
    const bytes = Buffer.byteLength(value, 'utf8')
    if (spec.max_bytes !== undefined && bytes > spec.max_bytes) throw new Error(`R55_max_bytes:${path}`)
    if (spec.max_decoded_bytes !== undefined && Buffer.from(value, 'base64url').length > spec.max_decoded_bytes) throw new Error(`R55_max_decoded_bytes:${path}`)
  }
}
function validateSchema(schema, payload, path, context = {}) {
  if (!schema?.properties || payload === null || typeof payload !== 'object' || Array.isArray(payload)) throw new Error(`R55_schema_object:${path}`)
  const actual = Object.keys(payload).sort(cp), allowed = (schema.exact_keys ?? Object.keys(schema.properties)).sort(cp)
  if (schema.additional_properties === false && canonicalR44(actual) !== canonicalR44(allowed)) throw new Error(`R55_exact_keys:${path}`)
  for (const field of schema.required ?? []) if (!Object.hasOwn(payload, field)) throw new Error(`R55_required:${path}.${field}`)
  ...
  if (schema.max_canonical_bytes !== undefined && Buffer.byteLength(canonicalR44(payload), 'utf8') > schema.max_canonical_bytes) throw new Error(`R55_max_canonical_bytes:${path}`)
  if (schema.max_bytes !== undefined && Buffer.byteLength(canonicalR44(payload), 'utf8') > schema.max_bytes) throw new Error(`R55_row_max_bytes:${path}`)
  for (const [field, value] of Object.entries(payload)) {
    if (field.endsWith('_length') && Object.hasOwn(payload, field.replace(/_length$/, '_b64url'))) {
      const raw = Buffer.from(payload[field.replace(/_length$/, '_b64url')], 'base64url'); if (value !== raw.length) throw new Error(`R55_length_equality:${path}.${field}`)
    }
  }
}
function fingerprint(schema, payload) {
  ...
  for (const field of authority.preimage_order) {
    ...
    else throw new Error(`R55_fingerprint_operand_missing:${schema.fingerprint_ref}:${field}`)
  }
...
function finalizeRow(schemaRef, variant, seed, context = {}) {
  ...
  if (!schema?.properties) throw new Error(`R55_fixture_schema_missing:${schemaRef}:${variant}`)
  ...
  if (rule === 'artifact_ref_equals_opaque_bytes_sha256') {
    const bytes = Buffer.from(`R55-OPAQUE:${seed}`, 'utf8'), digest = sha(bytes)
    ...
    if (!selectedSchema?.properties) throw new Error(`R55_canonical_payload_schema_missing:${selectedPayloadSchemaRef}`)
    ...
    payload.parsed_content_fingerprint = selectedFingerprint?.expected_fingerprint ?? hash({ domain_ascii: 'CTRL-G24-R55-PARSED-CONTENT', canonical_bytes_sha256: digest })
  ...
  if (schema.row_ref_derivation && Array.isArray(schema.row_ref_preimage_included_fields)) {
    ...
    if (!field) throw new Error(`R55_row_ref_field_missing:${schemaRef}:${variant}`)
...
function projectedContext(schema, context) {
  const out = {}
  ...
  for (const [field, dimension] of Object.entries(mappings)) if (schema.properties?.[field] && Object.hasOwn(context, dimension) && (!values(schema.properties[field]).length || values(schema.properties[field]).includes(context[dimension]))) out[field] = context[dimension]
  return out
}
function syntheticSelector(row, role) {
  const sourceSchema = schemaAt(row.row_schema_ref, row.row_schema_variant), context = { source_operation: sourceSchema.properties.operation_name?.const ?? 'UNAVAILABLE', source_result_branch: sourceSchema.properties.hold_branch?.const ?? sourceSchema.properties.branch?.const ?? 'UNAVAILABLE', proof_family: sourceSchema.properties.proof_family?.const ?? 'UNAVAILABLE', branch_class: sourceSchema.properties.branch_class?.const ?? 'UNAVAILABLE', evidence_kind: sourceSchema.properties.evidence_kind?.const ?? 'UNAVAILABLE', target_store: sourceSchema.properties.target_store?.const ?? 'UNAVAILABLE', source_store_variant: row.row_schema_variant, fresh_selection_row_id: sourceSchema.properties.fresh_selection_row_id?.const ?? `r55_${row.identity_field}` }
  let target
  ...
  const selectorId = `selector_r55_${row.identity_field}`, selectorRef = `authority_operation_joint_identity_selector_contexts.rows.${selectorId}`
  syntheticSelectorRows[selectorId] = { schema_version: 'ctrl.g24.joint-identity-selector-context.r55.v1', selector_id: selectorId, source_schema_ref: row.row_schema_ref, source_schema_variant: row.row_schema_variant, source_schema_version: sourceSchema.schema_version, source_field: row.identity_field, native_identity_role: role, exact_source_schema_valid_context: context, exact_target_case: target, exact_case_count: 1, caller_override_or_fallback: 'forbidden' }
  return { selector_ref: selectorRef, context, target }
}
function selectorChoice(row, role) {
  const equality = equalityFor(row, role)
  if (!equality) throw new Error(`R55_equality_missing:${row.identity_kind}:${role}`)
  ...
  if (!selector) throw new Error(`R55_selector_missing:${equality.selector_ref}`)
  ...
  if (!pairs.length) throw new Error(`R55_selector_context_missing:${row.identity_kind}`)
  return { selector_ref: equality.selector_ref, context: pairs[0].context, target: pairs[0].target }
}
function persistedWrapperFor(targetRef) {
  ...
  if (selected.length !== 1) throw new Error(`R55_wrapper_resolution_not_exact_one:${targetRef}:${selected.length}`)
  return selected[0]
}
function concreteTarget(selected) {
  ...
  if (!schema?.properties) throw new Error(`R55_target_schema_missing:${target.target_schema_ref}:${target.target_schema_variant}`)
  if (target.target_native_identity_field !== '$canonical_row_bytes' && !schema.properties[target.target_native_identity_field]) throw new Error(`R55_target_identity_field_missing:${target.target_schema_ref}:${target.target_native_identity_field}`)
  return target
}
function targetIdentity(fixture, target) {
  ...
  if (value === undefined) throw new Error(`R55_target_native_identity_missing:${target.target_schema_ref}:${target.target_native_identity_field}`)
  return value
}
function jointLinkedFixture(row, role, seed) {
  ...
  const targetFixture = finalizeRow(target.target_schema_ref, target.target_schema_variant ?? 'UNAVAILABLE', `${seed}_target`, { ...projectedContext(targetSchema, choice.context), __payload_variant: target.payload_schema_variant })
  ...
  const semanticBytesSha = role === 'companion_bytes_hash' && targetFixture.payload[row.identity_field] !== undefined ? targetFixture.payload[row.identity_field] : defaultBytesSha
  const semanticFingerprint = role === 'companion_fingerprint' && targetFixture.payload[row.identity_field] !== undefined ? targetFixture.payload[row.identity_field] : defaultFingerprint
  if (identity === undefined || semanticFingerprint === undefined) throw new Error(`R55_missing_target_identity_or_fingerprint:${row.identity_kind}`)
  ...
  const equality = equalityFor(row, role)
  const companionBytesFields = role === 'companion_bytes_hash' ? (equality.companion_bytes_fields ?? []).filter(field => field.field === row.identity_field) : role === 'resolved_reference_identity' ? (equality.companion_bytes_fields ?? []).filter(field => targetFixture.payload[field.field] === semanticBytesSha) : []
  const companionFingerprintFields = role === 'companion_fingerprint' ? (equality.companion_fingerprint_fields ?? []).filter(field => field.field === row.identity_field) : role === 'resolved_reference_identity' ? (equality.companion_fingerprint_fields ?? []).filter(field => targetFixture.payload[field.field] === semanticFingerprint) : []
  source.payload[equality.source_field] = identity
  for (const field of companionBytesFields) source.payload[field.field] = semanticBytesSha
  for (const field of companionFingerprintFields) source.payload[field.field] = semanticFingerprint
  ...
  return { selector_ref: choice.selector_ref, deterministic_context_selection: 'unicode_canonical_lowest_source_context_and_target_pair_after_exact_schema_intersection', selector_context: choice.context, projected_source_discriminators: projectedContext(sourceSchema, choice.context), target_schema_ref: target.target_schema_ref, target_schema_variant: target.target_schema_variant ?? 'UNAVAILABLE', target_schema_version: targetSchema.schema_version, target_identity_field: target.target_native_identity_field, target_identity_role: target.target_native_identity_role, target_fixture: compactFixture(targetFixture), target_native_identity: identity, target_native_bytes_sha256: semanticBytesSha, target_native_fingerprint: semanticFingerprint, source_reference_field: equality.source_field, companion_bytes_fields: companionBytesFields, companion_fingerprint_fields: companionFingerprintFields, source_fixture: compactFixture(source), exact_joint_equalities: { source_reference_equals_target_identity: source.payload[equality.source_field] === identity, source_companion_bytes_equal_target_bytes: companionBytesFields.every(field => source.payload[field.field] === semanticBytesSha), source_companion_fingerprints_equal_target_fingerprint: companionFingerprintFields.every(field => source.payload[field.field] === semanticFingerprint) } }
}

r54.authority_operation_persisted_identity_formula_library = {
  schema_version: 'ctrl.g24.persisted-identity-formula-library.r55.v1',
  role_precedence: ROLE_PRECEDENCE,
  canonical_row_bytes_content_address: { schema_version: 'ctrl.g24.persisted-identity-formula.canonical-row-bytes.r55.v1', formula_class: 'native_final_row_content_address', exact_formula: 'lowercase_hex_SHA256_of_exact_final_closed_schema_canonical_row_bytes' },
  declared_content_address: { schema_version: 'ctrl.g24.persisted-identity-formula.declared-content-address.r55.v1', formula_class: 'selected_schema_native_content_address', exact_formula: 'opaque_ref_equals_opaque_bytes_sha256_or_artifact_ref_equals_canonical_payload_bytes_sha256_or_exact_declared_row_ref_preimage_or_nonce_receipt_payload_sha256' },
  declared_row_version: { schema_version: 'ctrl.g24.persisted-identity-formula.declared-row-version.r55.v1', formula_class: 'selected_committed_target_native_row_version', exact_formula: 'selected_store_R49_domain_and_version_plus_ordered_complete_mutable_fields_excluding_row_version_and_native_fingerprint' },
  schema_declared_fingerprint: { schema_version: 'ctrl.g24.persisted-identity-formula.schema-fingerprint.r55.v1', formula_class: 'selected_schema_native_fingerprint', exact_formula: 'selected_fingerprint_authority_domain_order_types_and_declared_codec_over_actual_linked_row' },
  unique_key_component: { schema_version: 'ctrl.g24.persisted-identity-formula.unique-key-component.r55.v1', formula_class: 'native_schema_unique_key_component', exact_formula: 'exact_value_from_final_schema_valid_linked_row' },
  resolved_reference_identity: { schema_version: 'ctrl.g24.persisted-identity-formula.resolved-reference.r55.v1', formula_class: 'joint_source_target_native_identity', exact_formula: 'schema_valid_source_reference_equals_recomputed_identity_from_one_exact_selector_context_and_schema_valid_target_fixture' },
  companion_bytes_hash: { schema_version: 'ctrl.g24.persisted-identity-formula.companion-bytes.r55.v1', formula_class: 'joint_source_target_native_bytes', exact_formula: 'schema_valid_source_companion_bytes_equals_selected_target_native_bytes_sha256' },
  companion_fingerprint: { schema_version: 'ctrl.g24.persisted-identity-formula.companion-fingerprint.r55.v1', formula_class: 'joint_source_target_native_fingerprint', exact_formula: 'schema_valid_source_companion_equals_recomputed_selected_target_schema_native_fingerprint' },
}
...
for (const prior of priorIndex) {
  ...
  if (role === 'canonical_row_bytes_content_address') { expected = source.canonical_row_bytes_sha256; evidence = { execution: 'raw_sha256_final_canonical_row_bytes', exact_operand_b64url: source.canonical_row_bytes_b64url, expected_identity: expected } }
    ...
    if (!native) throw new Error(`R55_native_content_address_missing:${identityKind}`)
    ...
    if (!source.row_version) throw new Error(`R55_native_row_version_missing:${identityKind}`)
...
r54.authority_operation_internal_reference_target_selectors = { schema_version: 'ctrl.g24.internal-reference-target-selectors.r55.v1', row_schema_version: 'ctrl.g24.native-source-schema-intersection-selector.r55.v1', derivation: 'complete_source_schema_validation_of_R54_intersection_contexts', exact_count: Object.keys(selectors).length, exact_source_context_count: Object.values(selectors).reduce((sum, row) => sum + row.exact_case_count, 0), rows: selectors, selector_totality_and_uniqueness_for_every_source_schema_valid_context: true, current_invalid_source_context_count: invalidR53.length, invalid_contexts_are_never_emitted: true, caller_override_meta_target_or_fallback: 'forbidden' }
r54.authority_operation_selector_source_schema_validation_authority = { schema_version: 'ctrl.g24.selector-source-schema-validation-authority.r55.v1', exact_invalid_R54_context_occurrences: invalidR53, exact_invalid_R54_count: invalidR53.length, inherited_R53_rejected_context_count: materializedR54.authority_operation_selector_source_schema_validation_authority.exact_invalid_R53_count, source_schema_valid_context_count: r54.authority_operation_internal_reference_target_selectors.exact_source_context_count, exact_intersection_required: true, complete_recursive_schema_validation_required: true, zero_multiple_or_invalid_context: 'reject_materialization_and_hold_without_write' }
r54.authority_operation_complete_schema_cross_artifact_equality_registry = { ...r54.authority_operation_complete_schema_cross_artifact_equality_registry, schema_version: 'ctrl.g24.complete-schema-cross-artifact-equality-registry.r55.v1', exact_rows: equalityRows, exact_row_count: equalityRows.length, internal_exact_one_count: equalityRows.filter(row => row.exact_one_resolution_required).length, native_source_context_selector_ref: 'authority_operation_internal_reference_target_selectors', native_identity_authority_ref: 'authority_operation_artifact_fingerprint_derivation_authority', every_linked_identity_joint_source_target_fixture: true }
r54.authority_operation_artifact_resolution_authority = { ...r54.authority_operation_artifact_resolution_authority, schema_version: 'ctrl.g24.authority-operation-artifact-resolution-authority.r55.v1', native_identity_authority_ref: 'authority_operation_artifact_fingerprint_derivation_authority', source_constant_selector_ref: 'authority_operation_internal_reference_target_selectors', every_internal_selected_reference_uses_complete_source_schema_validation: true, missing_target_identity_fallback: 'forbidden' }
r54.authority_operation_restart_correlation_authority = { ...r54.authority_operation_restart_correlation_authority, schema_version: 'ctrl.g24.authority-operation-restart-correlation-authority.r55.v1', equality_registry_ref: 'authority_operation_complete_schema_cross_artifact_equality_registry', native_identity_authority_ref: 'authority_operation_artifact_fingerprint_derivation_authority' }
r54.fixture_schema_validator = { schema_version: 'ctrl.g24.fixture-schema-validator.r55.v1', supported_recursive_schema_forms: ['const', 'literal', 'enum', 'type', 'exact_keys', 'required', 'additional_properties_false', 'object_properties', 'schema_ref', 'value_schema', 'union_variant_discriminator', 'array_items', 'min_items', 'max_items', 'unique', 'unique_by', 'string_utf8_byte_limits', 'base64url_max_decoded_bytes', 'sha256', 'sha256_or_exact_literal', 'canonical_timestamp', 'numeric_minimum_maximum', 'declared_cross_field_length_content_address_fingerprint_version_and_joint_target_equalities'], validation_scope: 'every_persisted_identity_source_fixture_and_every_linked_target_fixture_before_native_identity_use', unsupported_or_unresolved_schema_form: 'reject_materialization_and_hold_without_write', producer_and_checker_independent_implementations_required: true }
r54.schema_change_manifest = { schema_version: 'ctrl.g24.trusted-ingress-schema-change-manifest.r55.v1', derivation: 'bounded_exact_extension_from_frozen_R54_complete_schema_validation_native_role_union_and_joint_linked_identity_closure', frozen_parent_sha256: sha(inputBytes), changed_semantic_paths: ['$', '$.materialization', '$.status', '$.supersedes', '$.authority_operation_persisted_identity_formula_library', '$.authority_operation_complete_persisted_identity_authorities', '$.authority_operation_joint_identity_selector_contexts', '$.authority_operation_native_identity_applicability_report', '$.authority_operation_artifact_fingerprint_derivation_authority', '$.authority_operation_internal_reference_target_selectors', '$.authority_operation_selector_source_schema_validation_authority', '$.authority_operation_complete_schema_cross_artifact_equality_registry', '$.authority_operation_artifact_resolution_authority', '$.authority_operation_restart_correlation_authority', '$.fixture_schema_validator', '$.authority_runtime_semantic_manifest_hash_contract', '$.authority_runtime_semantic_reference_field_registry', '$.authority_runtime_semantic_reference_owner_map', '$.authority_runtime_semantic_dependency_owner_map', '$.authority_runtime_semantic_manifest', '$.schema_change_manifest'], removed_semantic_paths: [], frozen_parent_core_must_remain_byte_identical: true, runtime_database_ui_deployment_or_external_action: 'closed' }
r54.required_negative_fixture_families = [...new Set([...r54.required_negative_fixture_families, 'complete_recursive_schema_validation', 'native_identity_role_precedence', 'joint_source_target_identity_splice', 'missing_target_native_identity', 'source_schema_invalid_selector_context'])]
...
r54.authority_runtime_semantic_reference_field_registry = { ...r54.authority_runtime_semantic_reference_field_registry, schema_version: 'ctrl.g24.runtime-semantic-reference-field-registry.r55.v1' }
r54.authority_runtime_semantic_reference_owner_map = { ...r54.authority_runtime_semantic_reference_owner_map, schema_version: 'ctrl.g24.runtime-semantic-reference-owner-map.r55.v1' }
r54.authority_runtime_semantic_dependency_owner_map = { ...r54.authority_runtime_semantic_dependency_owner_map, schema_version: 'ctrl.g24.runtime-semantic-dependency-owner-map.r55.v1' }
r54.authority_runtime_semantic_manifest = { ...r54.authority_runtime_semantic_manifest, schema_version: 'ctrl.g24.runtime-semantic-authority-manifest.r55.v1' }
...
r54.authority_runtime_semantic_manifest_hash_contract = { ...r54.authority_runtime_semantic_manifest_hash_contract, schema_version: 'ctrl.g24.runtime-semantic-manifest-hash-contract.r55.v1', manifest_hash_version: 'ctrl.g24.runtime-semantic-manifest-hash.r55.v1', content_domain_ascii: 'CTRL-G24-R55-MANIFEST-CONTENT', dependency_domain_ascii: 'CTRL-G24-R55-MANIFEST-DEPENDENCY', graph_domain_ascii: 'CTRL-G24-R55-MANIFEST-GRAPH', envelope_domain_ascii: 'CTRL-G24-R55-MANIFEST-ENVELOPE' }
...
const snapshotSha = hash({ domain_ascii: 'CTRL-G24-R55-FINAL-SEMANTIC-SOURCE-SNAPSHOT', exact_paths: sourcePaths, values: sourceSnapshot })
...
r54.authority_runtime_semantic_reference_owner_map = { schema_version: 'ctrl.g24.runtime-semantic-reference-owner-map.r55.v1', source_snapshot_sha256: snapshotSha, exact_row_count: refRows.length, reference_registry_ref: 'authority_runtime_semantic_reference_field_registry', schema_declared_and_exact_path_value_bijection: true }
r54.authority_runtime_semantic_dependency_owner_map = { schema_version: 'ctrl.g24.runtime-semantic-dependency-owner-map.r55.v1', source_snapshot_sha256: snapshotSha, exact_paths: sourcePaths, rows: sourcePaths.map(path => ({ authority_path: path, typed_owner_paths: dependencies[path] })), reference_registry_ref: 'authority_runtime_semantic_reference_field_registry' }
...
const withoutSeal = { schema_version: 'ctrl.g24.runtime-semantic-authority-manifest.r55.v1', source_snapshot_sha256: snapshotSha, exact_paths: sourcePaths, rows: manifestRows, exact_expected_count: manifestRows.length, manifest_graph_sha256: hash({ domain_ascii: hc.graph_domain_ascii, manifest_hash_version: hc.manifest_hash_version, manifest_rows: manifestRows }) }
...
export const materializedR55 = r54
export const materializedR55Output = `${JSON.stringify(finalSnapshot, null, 2)}\n`
export const r55SemanticAuthorityPaths = sourcePaths
const invoked = process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)
if (invoked) {
  const mode = process.argv[2] ?? '--check'
  if (mode === '--write') { writeFileSync(join(root, outputPath), materializedR55Output); console.log(`wrote ${outputPath}`) }
  else if (mode === '--check') { if (readFileSync(join(root, outputPath), 'utf8') !== materializedR55Output) { console.error(`${outputPath} differs from materializer`); process.exit(1) } console.log(`ok: ${outputPath} is the exact fully materialized R55 effective contract`) }
