...
check(
  ...
  state.includes('**CURRENT_NEXT_ACTION:** Freeze and independently attack the fully materialized G24 trusted canonical ingress R32 repair'),
)
check(
  'design state remains inside the trusted-ingress gate',
  designState.includes('freeze and independently attack the fully materialized trusted canonical ingress R32 repair'),
)
