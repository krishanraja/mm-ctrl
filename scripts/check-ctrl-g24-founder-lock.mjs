...
check(
  ...
  state.includes('**CURRENT_NEXT_ACTION:** Submit the repaired [R74 predicate-authority decision]'),
)
check(
  'design state remains inside the trusted-ingress gate',
  designState.includes('independently review the repaired R74 predicate-authority decision') && designState.includes('No machine contract, result-producing evaluation, runtime integration'),
)
