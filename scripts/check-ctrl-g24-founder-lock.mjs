...
check(
  ...
  state.includes('**CURRENT_NEXT_ACTION:** Repair the exact G24 trusted canonical ingress R10 derivation defects as R11'),
)
check(
  'design state remains inside the trusted-ingress gate',
  designState.includes('repair the exact trusted canonical ingress R10 derivation defects as R11'),
)
