...
check(
  ...
  state.includes('**CURRENT_NEXT_ACTION:** Freeze the exact [R84 reconciled authority contract]'),
)
check(
  'design state remains inside the trusted-ingress gate',
  designState.includes('freeze the exact R84 reconciled authority contract') && designState.includes('No semantic evaluator implementation, result-producing success branch, runtime integration'),
)
