...
check(
  ...
  state.includes('**CURRENT_NEXT_ACTION:** Independently attack the exact G24 trusted canonical ingress R6 repair'),
)
check(
  'design state remains inside the trusted-ingress gate',
  designState.includes('independently attack the exact trusted canonical ingress R6 repair'),
)
