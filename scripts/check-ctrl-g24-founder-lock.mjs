...
check(
  ...
  state.includes('**CURRENT_NEXT_ACTION:** Freeze the exact [R83 complete-path predicate contract]'),
)
check(
  'design state remains inside the trusted-ingress gate',
  designState.includes('freeze the exact R83 complete-path predicate contract') && designState.includes('No semantic evaluator implementation, result-producing success branch, runtime integration'),
)
