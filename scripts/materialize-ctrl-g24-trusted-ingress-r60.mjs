...
import { materializedR59 as materializedR54, materializedR59Output as materializedR54Output, r59SemanticAuthorityPaths as r54SemanticAuthorityPaths } from './materialize-ctrl-g24-trusted-ingress-r59.mjs'
import { canonicalR44, ownedSnapshotR44 } from './materialize-ctrl-g24-trusted-ingress-r44.mjs'

const root = process.cwd()
const inputPath = 'project-documentation/ctrl-evolution/g24-trusted-ingress-contract-r59.json'
const outputPath = 'project-documentation/ctrl-evolution/g24-trusted-ingress-contract-r60.json'
const inputBytes = readFileSync(join(root, inputPath), 'utf8')
if (inputBytes !== materializedR54Output) throw new Error('R60_frozen_R59_input_mismatch')
...
function collectConditionalSites(value, path = '$') {
  ...
  for (const rule of value.conditional_rules ?? []) {
    if (typeof rule !== 'string' || !rule) throw new Error(`R60_conditional_rule_not_string:${path}`)
...
function validateOrdering(spec, value, context, path) {
  ...
  if (rule === 'matching_evidence_refs') {
    ...
    const resolvedPairs = context.resolved_evidence_pairs
    if (!Array.isArray(refs) || refs.length !== value.length || !Array.isArray(resolvedPairs) || resolvedPairs.length !== refs.length) throw new Error(`R60_ordering_companion_alignment:${path}`)
    if (new Set(resolvedPairs.map(pair => pair?.evidence_ref)).size !== resolvedPairs.length) throw new Error(`R60_ordering_duplicate_resolved_ref:${path}`)
    for (let index = 0; index < refs.length; index += 1) {
      const pair = resolvedPairs[index]
      if (!pair || pair.evidence_ref !== refs[index] || pair.evidence_fingerprint !== value[index] || !/^[0-9a-f]{64}$/.test(pair.evidence_fingerprint)) throw new Error(`R60_ordering_evidence_pair_mismatch:${path}:${index}`)
    }
    for (let index = 1; index < refs.length; index += 1) if (unsignedUtf8Compare(refs[index - 1], refs[index]) > 0) throw new Error(`R60_ordering_matching_refs:${path}`)
    return
  }
  ...
  if (!supported.has(rule)) throw new Error(`R60_unknown_ordering_authority:${path}:${rule}`)
  const keys = value.map(item => orderingKey(rule, item))
  if (keys.some(key => key === undefined || key === -1)) throw new Error(`R60_ordering_key_unresolved:${path}:${rule}`)
  for (let index = 1; index < keys.length; index += 1) {
    const comparison = typeof keys[index] === 'number' ? keys[index - 1] - keys[index] : unsignedUtf8Compare(keys[index - 1], keys[index])
    if (comparison > 0) throw new Error(`R60_ordering_violation:${path}:${rule}`)
  }
}

r54.schema_version = 'ctrl.g24.trusted-ingress.r60.effective.v1'
r54.status = ['founder_locked_direction', 'headless_kernel_independently_verified', 'trusted_ingress_r1_through_r59_vetoed', 'trusted_ingress_r60_fully_materialized', 'independent_attack_required', 'no_adapter_or_runtime_connection']
r54.supersedes = { commit: '5be0a57b17ea4f1e5f74d555dfe055ec6c10bc70', tree: '046da2aae9e0911e8a2158f536f316431fad4705', human_blob: '4becf0463b058e351268db0f37257d3b0d05e42e', machine_blob: '68443f175edb12a9d1185c722145e381236c807a', qa_blob: '6186325f10d15c715779719b26284d2545df54c1', checker_blob: '8788fd8572374915439c681bb57ec28a70959387', materializer_blob: '6112182d472e1d49cff52ec8a29a8e558a03bd91', founder_checker_blob: 'dac7cd3b3bfe0be14c6bfb8a14d5565ef5da892b', adjudication: 'veto' }
r54.materialization = { ...r54.materialization, schema_version: 'ctrl.g24.trusted-ingress-materialization.r60.v1', frozen_input: { path: inputPath, sha256: sha(inputBytes) }, output_path: outputPath, strict_finalization_dag: ['freeze_R59_input', 'bind_evidence_reference_fingerprint_pairs', 'execute_every_ordering_site', 'bind_intervention_schema_version_by_kind', 'restore_question_display_byte_preservation', 'derive_recursive_selected_fixture_exclusions', 'finalize_nonderived_authorities', 'snapshot_final_semantic_sources', 'derive_reference_owner_and_manifest', 'seal_output'], no_post_snapshot_source_write: true }
r54.authority_operation_intervention_inner_payload_schemas = {
  schema_version: 'ctrl.g24.intervention-inner-payload-schemas.r60.v1',
  question_schema_ref: 'proof_value_schemas.question_contract',
  session: {
    schema_version: 'ctrl.g24.intervention-session-inner-payload.r60.v1',
    type: 'object',
...
function values(spec) {
  ...
  if (spec.enum_ref) {
    ...
    if (!Array.isArray(resolved) || !resolved.length) throw new Error(`R60_enum_ref_missing_or_empty:${spec.enum_ref}`)
    return resolved
  }
...
function referencedSpec(spec) {
  ...
  if (!resolved) throw new Error(`R60_schema_ref_missing:${ref}`)
  return resolved
}
...
function valueForSpec(spec, seed, context = {}) {
  if (!spec || typeof spec !== 'object') return `r60_${seed}`
  ...
  if (pattern === '^[0-9a-f]{64}$') return sha(`R60:${seed}`)
  if (pattern === '^[0-9a-f]{32}$') return sha(`R60:${seed}`).slice(0, 32)
  if (spec.type === 'sha256') return sha(`R60:${seed}`)
  if (spec.type === 'sha256_or_exact_literal') return (spec.exact_literals ?? [])[0] ?? sha(`R60:${seed}`)
  if (spec.type === 'canonical_timestamp') return '2031-01-01T00:00:00.000Z'
  if (spec.type === 'base64url_without_padding') return Buffer.from(`R60:${seed}`, 'utf8').toString('base64url')
  ...
  return `r60_${String(seed).replace(/[^a-z0-9_]/gi, '_').slice(-96)}`.padEnd(minimum, 'x').slice(0, maximum)
}
...
function normalizePayloadForLocalRules(schema, payload, seed) {
  ...
  for (const [field, availability] of Object.entries(payload)) {
    ...
    if (availability === 'available') {
      payload[refField] = sha(`R60:${seed}:${refField}`)
      payload[shaField] = sha(`R60:${seed}:${shaField}`)
    } else {
  ...
  if (rules.has('result_ref_is_nonnull_iff_operation_class_is_use_release_and_equals_the_branch_terminal_consumption_ref')) payload.result_ref = payload.operation_class === 'use_release' ? (payload.result_ref ?? sha(`R60:${seed}:terminal_consumption_ref`)) : null
...
function validateSpec(spec, value, path, context = {}) {
  spec = effectiveSpec(spec)
  if (!spec || typeof spec !== 'object') throw new Error(`R60_invalid_schema_spec:${path}`)
  if (spec.type && !SUPPORTED_TYPES.has(spec.type)) throw new Error(`R60_unknown_type:${path}:${spec.type}`)
  if (!spec.properties && !spec.variants) for (const key of Object.keys(spec)) if (!LEAF_SPEC_KEYS.has(key)) throw new Error(`R60_unknown_schema_spec_key:${path}:${key}`)
  if (spec.type === 'nullable') {
    if (value === null) return
    if (!spec.value_schema) throw new Error(`R60_nullable_schema_missing:${path}`)
  ...
  if (spec.type === 'controlling_watermark_change') { const registry = r54.type_registry.controlling_watermark_change, variant = registry.variants?.[value?.kind_class]; if (!variant) throw new Error(`R60_watermark_variant:${path}`); validateSchema(variant, value, `${path}<${value.kind_class}>`, context); return }
  if (spec.type === 'discriminated_value') { const selected = spec.variants?.[context[spec.discriminator_source_field]]; if (!selected) throw new Error(`R60_discriminated_value:${path}`); validateSpec(selected, value, `${path}<${context[spec.discriminator_source_field]}>`, context); return }
  if (spec.type === 'discriminated_union') { const variant = spec.variants?.[value?.[spec.discriminator]]; if (!variant) throw new Error(`R60_union_variant:${path}`); validateSchema(variant, value, `${path}<${value[spec.discriminator]}>`, context); return }
  if (spec.type === 'operation_discriminated_object') { const operation = context.operation_class; const intent = r54.operation_specs?.[operation]?.intent; if (!intent) throw new Error(`R60_operation_intent:${path}:${operation}`); validateSpec(intent, value, `${path}<${operation}>`, context); return }
  const resolved = referencedSpec(spec)
  ...
  if (Object.hasOwn(spec, 'const') && value !== spec.const) throw new Error(`R60_const:${path}`)
  if (Object.hasOwn(spec, 'literal') && spec.type !== 'sha256_or_exact_literal' && value !== spec.literal) throw new Error(`R60_literal:${path}`)
  const allowed = values(spec)
  if (allowed.length && !allowed.includes(value)) throw new Error(`R60_enum:${path}`)
  ...
  if (acceptedJsonTypes.length && !acceptedJsonTypes.includes('schema_value')) {
    ...
    if (!acceptedJsonTypes.includes(actualJsonType) && !(actualJsonType === 'integer' && acceptedJsonTypes.includes('number'))) throw new Error(`R60_json_type:${path}:${actualJsonType}`)
  }
  if (spec.type === 'sha256' && !(typeof value === 'string' && /^[0-9a-f]{64}$/.test(value))) throw new Error(`R60_sha256:${path}`)
  if (spec.type === 'sha256_or_exact_literal' && !(typeof value === 'string' && (/^[0-9a-f]{64}$/.test(value) || [spec.literal, spec.const, ...(spec.exact_literals ?? [])].filter(item => item !== undefined).includes(value)))) throw new Error(`R60_sha256_or_literal:${path}`)
  if (['identifier', 'human_text', 'literal'].includes(spec.type) && typeof value !== 'string') throw new Error(`R60_string:${path}`)
  if (spec.type === 'identifier' && (value !== value.trim() || value.normalize('NFC') !== value || /[\u0000-\u001f\u007f-\u009f\u200b-\u200f\u202a-\u202e\u2060-\u206f\ufeff]/u.test(value))) throw new Error(`R60_identifier_normalization:${path}`)
  if (spec.type === 'canonical_timestamp' && !strictTimestamp(value)) throw new Error(`R60_time:${path}`)
  if (spec.type === 'base64url_without_padding' && !(typeof value === 'string' && /^[A-Za-z0-9_-]*$/.test(value) && !value.includes('=') && value.length % 4 !== 1 && Buffer.from(value, 'base64url').toString('base64url') === value)) throw new Error(`R60_base64url:${path}`)
  if (spec.type === 'boolean' && typeof value !== 'boolean') throw new Error(`R60_boolean:${path}`)
  if (['integer', 'positive_integer', 'safe_nonnegative_integer', 'nonnegative_integer'].includes(spec.type) && !(Number.isSafeInteger(value) && value >= (spec.type === 'positive_integer' ? 1 : 0))) throw new Error(`R60_integer:${path}`)
  if (spec.type === 'finite_nonnegative_number' && !(Number.isFinite(value) && value >= 0 && !Object.is(value, -0))) throw new Error(`R60_number:${path}`)
  if (spec.minimum !== undefined && value < spec.minimum) throw new Error(`R60_minimum:${path}`)
  if (spec.maximum !== undefined && value > spec.maximum) throw new Error(`R60_maximum:${path}`)
  if (spec.type === 'array') {
    if (!Array.isArray(value)) throw new Error(`R60_array:${path}`)
    if (spec.min_items !== undefined && value.length < spec.min_items) throw new Error(`R60_min_items:${path}`)
    if (spec.max_items !== undefined && value.length > spec.max_items) throw new Error(`R60_max_items:${path}`)
    value.forEach((item, index) => validateSpec(spec.items ?? { type: 'identifier' }, item, `${path}.${index}`, context))
    if (spec.unique && new Set(value.map(item => canonicalR44(item))).size !== value.length) throw new Error(`R60_unique:${path}`)
    if (spec.unique_by && new Set(value.map(item => item?.[spec.unique_by])).size !== value.length) throw new Error(`R60_unique_by:${path}`)
    if (spec.allowed_route_values && value.some(item => !spec.allowed_route_values.includes(item?.route))) throw new Error(`R60_route:${path}`)
    if (Array.isArray(spec.exact_members) && canonicalR44([...value].sort(cp)) !== canonicalR44([...spec.exact_members].sort(cp))) throw new Error(`R60_exact_members:${path}`)
    for (const constraint of spec.item_constraints ?? []) {
      if (constraint === 'string' && value.some(item => typeof item !== 'string')) throw new Error(`R60_item_string:${path}`)
      else if (constraint === 'exact_trimmed_nonempty' && value.some(item => typeof item !== 'string' || !item.trim() || item !== item.trim())) throw new Error(`R60_item_trimmed_nonempty:${path}`)
        ...
        if (value.some(item => reserved.has(item))) throw new Error(`R60_item_reserved:${path}`)
      } else if (!['string', 'exact_trimmed_nonempty', 'not_exact_reserved_default_or_offered_honest_exit'].includes(constraint)) throw new Error(`R60_unknown_item_constraint:${path}:${constraint}`)
    }
  ...
  if (['ordered_identifier_array', 'unicode_sorted_unique_identifier_array'].includes(spec.type)) {
    if (!Array.isArray(value)) throw new Error(`R60_ordered_identifier_array:${path}`)
    for (const [index, item] of value.entries()) validateSpec({ type: 'identifier' }, item, `${path}.${index}`, context)
    if (new Set(value).size !== value.length || canonicalR44([...value].sort(cp)) !== canonicalR44(value)) throw new Error(`R60_ordered_identifier_array_order:${path}`)
  }
  if ((spec.type === 'object' || spec.properties) && (value === null || typeof value !== 'object' || Array.isArray(value))) throw new Error(`R60_object:${path}`)
  if (spec.type === 'object' || spec.properties) validateSchema(spec, value, path, context)
  if (typeof value === 'string') {
    if (spec.valid_unicode_scalar_only && !validUnicodeScalars(value)) throw new Error(`R60_unicode_scalar:${path}`)
    const bytes = Buffer.byteLength(value, 'utf8')
    if (spec.min_utf8_bytes !== undefined && bytes < spec.min_utf8_bytes) throw new Error(`R60_min_utf8_bytes:${path}`)
    if ((spec.max_utf8_bytes ?? spec.max_bytes) !== undefined && bytes > (spec.max_utf8_bytes ?? spec.max_bytes)) throw new Error(`R60_max_utf8_bytes:${path}`)
    const pattern = spec.pattern ?? spec.regex
    if (pattern && !new RegExp(pattern, 'u').test(value)) throw new Error(`R60_pattern:${path}`)
    if (spec.max_decoded_bytes !== undefined && (!(typeof value === 'string' && /^[A-Za-z0-9_-]*$/.test(value) && value.length % 4 !== 1 && Buffer.from(value, 'base64url').toString('base64url') === value) || Buffer.from(value, 'base64url').length > spec.max_decoded_bytes)) throw new Error(`R60_max_decoded_bytes:${path}`)
  }
}
function requireRule(condition, path, kind) { if (!condition) throw new Error(`R60_conditional_${kind}:${path}`) }
function evaluateLocalRule(ruleRow, schema, payload, path) {
  ...
  else throw new Error(`R60_unknown_local_predicate:${kind}`)
}
function validateSchema(schema, payload, path, context = {}) {
  if (!schema?.properties || payload === null || typeof payload !== 'object' || Array.isArray(payload)) throw new Error(`R60_schema_object:${path}`)
  const actual = Object.keys(payload).sort(cp), allowed = (schema.exact_keys ?? Object.keys(schema.properties)).sort(cp)
  if (schema.additional_properties === false && actual.some(field => !allowed.includes(field))) throw new Error(`R60_exact_keys:${path}`)
  for (const field of schema.required ?? []) if (!Object.hasOwn(payload, field)) throw new Error(`R60_required:${path}.${field}`)
  for (const [field, spec] of Object.entries(schema.properties)) if (Object.hasOwn(payload, field)) validateSpec(spec, payload[field], `${path}.${field}`, { ...context, ...payload })
  if (schema.max_canonical_bytes !== undefined && Buffer.byteLength(canonicalR44(payload), 'utf8') > schema.max_canonical_bytes) throw new Error(`R60_max_canonical_bytes:${path}`)
  if (schema.max_bytes !== undefined && Buffer.byteLength(canonicalR44(payload), 'utf8') > schema.max_bytes) throw new Error(`R60_row_max_bytes:${path}`)
  for (const [field, value] of Object.entries(payload)) {
    if (field.endsWith('_length') && Object.hasOwn(payload, field.replace(/_length$/, '_b64url'))) {
      const raw = Buffer.from(payload[field.replace(/_length$/, '_b64url')], 'base64url'); if (value !== raw.length) throw new Error(`R60_length_equality:${path}.${field}`)
    }
  }
  for (const rule of schema.conditional_rules ?? []) {
    const row = ruleByText.get(rule)
    if (!row || row.unknown_or_unimplemented !== 'reject') throw new Error(`R60_conditional_rule_not_closed:${path}:${rule}`)
...
function fingerprint(schema, payload) {
  ...
  for (const field of authority.preimage_order) {
    ...
    else throw new Error(`R60_fingerprint_operand_missing:${schema.fingerprint_ref}:${field}`)
  }
...
function finalizeRow(schemaRef, variant, seed, context = {}) {
  ...
  if (!schema?.properties) throw new Error(`R60_fixture_schema_missing:${schemaRef}:${variant}`)
  ...
  if (rule === 'artifact_ref_equals_opaque_bytes_sha256') {
    const bytes = Buffer.from(`R60-OPAQUE:${seed}`, 'utf8'), digest = sha(bytes)
    ...
    if (!selectedSchema?.properties) throw new Error(`R60_canonical_payload_schema_missing:${selectedPayloadSchemaRef}`)
    ...
    payload.parsed_content_fingerprint = selectedFingerprint?.expected_fingerprint ?? hash({ domain_ascii: 'CTRL-G24-R60-PARSED-CONTENT', canonical_bytes_sha256: digest })
  ...
  if (schema.row_ref_derivation && Array.isArray(schema.row_ref_preimage_included_fields)) {
    ...
    if (!field) throw new Error(`R60_row_ref_field_missing:${schemaRef}:${variant}`)
...
function projectedContext(schema, context, strict = true) {
  ...
  for (const [field, dimension] of Object.entries(mappings)) if (schema.properties?.[field] && Object.hasOwn(context, dimension)) {
    ...
    if (allowed.length && !allowed.includes(context[dimension])) { if (strict) throw new Error(`R60_selector_dimension_not_accepted:${field}:${context[dimension]}`); continue }
...
function syntheticSelector(row, role) {
  ...
  const selectorId = `selector_r60_${row.identity_field}`, selectorRef = `authority_operation_joint_identity_selector_contexts.rows.${selectorId}`
  syntheticSelectorRows[selectorId] = { schema_version: 'ctrl.g24.joint-identity-selector-context.r60.v1', selector_id: selectorId, source_schema_ref: row.row_schema_ref, source_schema_variant: row.row_schema_variant, source_schema_version: sourceSchema.schema_version, source_field: row.identity_field, native_identity_role: role, exact_source_schema_valid_context: context, exact_target_case: target, exact_case_count: 1, every_applicable_dimension_equals_generated_source_fixture: true, reduced_projection_or_UNAVAILABLE_for_present_source_field: 'forbidden', caller_override_or_fallback: 'forbidden' }
  return { selector_ref: selectorRef, context, target }
}
function selectorChoice(row, role) {
  const equality = equalityFor(row, role)
  if (!equality) throw new Error(`R60_equality_missing:${row.identity_kind}:${role}`)
  ...
  if (!selector) throw new Error(`R60_selector_missing:${equality.selector_ref}`)
  ...
  if (!pairs.length) throw new Error(`R60_selector_context_missing:${row.identity_kind}`)
...
function persistedWrapperFor(targetRef) {
  ...
  if (selected.length !== 1) throw new Error(`R60_wrapper_resolution_not_exact_one:${targetRef}:${selected.length}`)
  return selected[0]
}
function concreteTarget(selected) {
  ...
  if (!schema?.properties) throw new Error(`R60_target_schema_missing:${target.target_schema_ref}:${target.target_schema_variant}`)
  if (target.target_native_identity_field !== '$canonical_row_bytes' && !schema.properties[target.target_native_identity_field]) throw new Error(`R60_target_identity_field_missing:${target.target_schema_ref}:${target.target_native_identity_field}`)
  return target
}
function targetIdentity(fixture, target) {
  ...
  if (value === undefined) throw new Error(`R60_target_native_identity_missing:${target.target_schema_ref}:${target.target_native_identity_field}`)
  return value
}
...
function jointLinkedFixture(row, role, seed) {
  ...
  const specialize = value => {
    ...
    copy.authority_companion_target_mapping = {
      schema_version: 'ctrl.g24.authority-companion-target-mapping.r60.v1',
  ...
  if (identity === undefined || defaultFingerprint === undefined) throw new Error(`R60_missing_target_identity_or_fingerprint:${row.identity_kind}`)
  ...
  completeJointRows[completeId] = { schema_version: 'ctrl.g24.complete-joint-equality-fixture.r60.v1', equality_source_schema_ref: equality.source_schema_ref, equality_source_schema_variant: equality.source_schema_variant, equality_source_field: equality.source_field, equality_selector_ref: equality.selector_ref, exact_selector_context: exactSourceDimensions, selected_selector_context: choice.context, target_schema_ref: result.target_schema_ref, target_schema_variant: result.target_schema_variant, target_identity_field: result.target_identity_field, target_fixture: result.target_fixture, source_fixture: result.source_fixture, target_native_identity: identity, target_native_bytes_sha256: defaultBytesSha, target_native_fingerprint: defaultFingerprint, declared_companion_bytes_fields: companionBytesFields, declared_companion_fingerprint_fields: companionFingerprintFields, declared_companion_bytes_count: companionBytesFields.length, declared_companion_fingerprint_count: companionFingerprintFields.length, exact_joint_equalities: result.exact_joint_equalities, every_applicable_selector_dimension_must_equal_source_fixture: true }
  completeJointCache.set(cacheKey, result)
  return specialize(result)
}

r54.authority_operation_persisted_identity_formula_library = {
  schema_version: 'ctrl.g24.persisted-identity-formula-library.r60.v1',
  role_precedence: ROLE_PRECEDENCE,
  canonical_row_bytes_content_address: { schema_version: 'ctrl.g24.persisted-identity-formula.canonical-row-bytes.r60.v1', formula_class: 'native_final_row_content_address', exact_formula: 'lowercase_hex_SHA256_of_exact_final_closed_schema_canonical_row_bytes' },
  declared_content_address: { schema_version: 'ctrl.g24.persisted-identity-formula.declared-content-address.r60.v1', formula_class: 'selected_schema_native_content_address', exact_formula: 'opaque_ref_equals_opaque_bytes_sha256_or_artifact_ref_equals_canonical_payload_bytes_sha256_or_exact_declared_row_ref_preimage_or_nonce_receipt_payload_sha256' },
  declared_row_version: { schema_version: 'ctrl.g24.persisted-identity-formula.declared-row-version.r60.v1', formula_class: 'selected_committed_target_native_row_version', exact_formula: 'selected_store_R49_domain_and_version_plus_ordered_complete_mutable_fields_excluding_row_version_and_native_fingerprint' },
  schema_declared_fingerprint: { schema_version: 'ctrl.g24.persisted-identity-formula.schema-fingerprint.r60.v1', formula_class: 'selected_schema_native_fingerprint', exact_formula: 'selected_fingerprint_authority_domain_order_types_and_declared_codec_over_actual_linked_row' },
  unique_key_component: { schema_version: 'ctrl.g24.persisted-identity-formula.unique-key-component.r60.v1', formula_class: 'native_schema_unique_key_component', exact_formula: 'exact_value_from_final_schema_valid_linked_row' },
  resolved_reference_identity: { schema_version: 'ctrl.g24.persisted-identity-formula.resolved-reference.r60.v1', formula_class: 'joint_source_target_native_identity', exact_formula: 'schema_valid_source_reference_equals_recomputed_identity_from_one_exact_selector_context_and_schema_valid_target_fixture' },
  companion_bytes_hash: { schema_version: 'ctrl.g24.persisted-identity-formula.companion-bytes.r60.v1', formula_class: 'joint_source_target_native_bytes', exact_formula: 'schema_valid_source_companion_bytes_equals_selected_target_native_bytes_sha256' },
  companion_fingerprint: { schema_version: 'ctrl.g24.persisted-identity-formula.companion-fingerprint.r60.v1', formula_class: 'joint_source_target_native_fingerprint', exact_formula: 'schema_valid_source_companion_equals_recomputed_selected_target_schema_native_fingerprint' },
}
...
for (const prior of priorIndex) {
  ...
  if (role === 'canonical_row_bytes_content_address') { expected = source.canonical_row_bytes_sha256; evidence = { execution: 'raw_sha256_final_canonical_row_bytes', exact_operand_b64url: source.canonical_row_bytes_b64url, expected_identity: expected } }
    ...
    if (!native) throw new Error(`R60_native_content_address_missing:${identityKind}`)
    ...
    if (!source.row_version) throw new Error(`R60_native_row_version_missing:${identityKind}`)
...
const orderingHandlers = {
  ...
  matching_evidence_refs: { operand: 'resolved_evidence_pairs', comparator: 'same_length_index_and_exact_ref_fingerprint_equality_to_each_resolved_evidence_row' },
}
if (orderingOccurrences.length !== 21 || orderingOccurrences.some(row => !orderingHandlers[row.exact_ordering_authority])) throw new Error('R60_ordering_inventory_not_closed')
function orderingFixture(occurrence, index) {
  const rule = occurrence.exact_ordering_authority
  let positiveValue, positiveContext = {}, negativeValue, negativeContext = {}
  if (rule === 'unsigned_utf8') { positiveValue = ['a', 'b']; negativeValue = ['b', 'a'] }
  else if (rule === 'unsigned_utf8_effect_key') { positiveValue = [{ effect_key: 'a' }, { effect_key: 'b' }]; negativeValue = [...positiveValue].reverse() }
  else if (rule === 'kind_class_then_base_kind_or_control_id_unsigned_utf8') { positiveValue = [{ kind_class: 'applicable_control', control_id: 'a' }, { kind_class: 'applicable_control', control_id: 'b' }]; negativeValue = [...positiveValue].reverse() }
  else if (rule === 'reuse_enrich_ask_session') { positiveValue = [{ route: 'reuse' }, { route: 'enrich' }]; negativeValue = [...positiveValue].reverse() }
  else if (rule === 'nonlease_final_recheck_failure_set.values') { positiveValue = ['operation_authority_changed', 'payload_changed']; negativeValue = [...positiveValue].reverse() }
  else if (rule === 'matching_evidence_refs') {
    const refs = [`evidence_${String(index).padStart(2, '0')}_a`, `evidence_${String(index).padStart(2, '0')}_b`]
    const fingerprints = refs.map(ref => sha(`R60:${occurrence.source_path}:${ref}`))
    const pairs = refs.map((evidence_ref, pairIndex) => ({ evidence_ref, evidence_fingerprint: fingerprints[pairIndex] }))
    positiveValue = fingerprints; positiveContext = { evidence_refs: refs, resolved_evidence_pairs: pairs }; negativeValue = [...fingerprints].reverse(); negativeContext = structuredClone(positiveContext)
  } else throw new Error(`R60_unknown_ordering_fixture_rule:${rule}`)
  validateOrdering({ ordered_by: rule }, positiveValue, positiveContext, `${occurrence.source_path}:positive`)
  let rejected = false
  try { validateOrdering({ ordered_by: rule }, negativeValue, negativeContext, `${occurrence.source_path}:negative`) } catch { rejected = true }
  if (!rejected) throw new Error(`R60_ordering_negative_not_rejected:${occurrence.source_path}`)
  return { fixture_id: `ordering_${String(index + 1).padStart(2, '0')}`, source_path: occurrence.source_path, exact_ordering_authority: rule, selected_spec_sha256: hash(get(materializedR54, occurrence.source_path.slice(2))), positive: { value: positiveValue, context: positiveContext }, negative: { value: negativeValue, context: negativeContext, expected: 'reject' } }
}
const orderingFixtureRows = orderingOccurrences.map(orderingFixture)
r54.authority_operation_ordering_rule_registry = { schema_version: 'ctrl.g24.ordering-rule-registry.r60.v1', exact_occurrence_count: orderingOccurrences.length, exact_handler_count: Object.keys(orderingHandlers).length, exact_positive_fixture_count: orderingFixtureRows.length, exact_negative_fixture_count: orderingFixtureRows.length, handlers: orderingHandlers, occurrences: orderingOccurrences, executable_occurrence_fixtures: orderingFixtureRows, matching_evidence_pair_authority: { schema_version: 'ctrl.g24.resolved-evidence-pair-ordering.r60.v1', pair_exact_keys: ['evidence_ref', 'evidence_fingerprint'], evidence_ref_unique: true, evidence_ref_must_equal_same_position_source_ref: true, evidence_fingerprint_must_equal_resolved_row_fingerprint: true, missing_duplicate_substituted_or_swapped_pair: 'reject_materialization_and_hold_without_write' }, unknown_ordering_authority: 'reject_materialization_and_hold_without_write', every_occurrence_executes_selected_handler: true }
...
r54.schema_change_manifest.repair_roots = ['resolved_evidence_ref_fingerprint_pairs', 'positive_and_negative_fixture_for_all_21_ordering_sites', 'intervention_payload_schema_version_bound_by_atom_kind', 'question_display_nonblank_with_exact_original_bytes_preserved', 'recursive_actual_selected_schema_conditional_exclusions', 'preserve_R59_array_canonical_inner_exact_effect_trim_and_joint_evidence']
r54.required_negative_fixture_families = [...new Set([...r54.required_negative_fixture_families, 'ordering_every_site_positive_and_negative', 'matching_evidence_fingerprint_swap_missing_substitute_duplicate', 'intervention_payload_schema_version_arbitrary_and_cross_kind', 'question_display_padded_positive_and_blank_negative', 'recursive_exclusion_nested_drop_variant'])]
...
r54.authority_runtime_semantic_reference_field_registry = { ...r54.authority_runtime_semantic_reference_field_registry, schema_version: 'ctrl.g24.runtime-semantic-reference-field-registry.r60.v1' }
r54.authority_runtime_semantic_reference_owner_map = { ...r54.authority_runtime_semantic_reference_owner_map, schema_version: 'ctrl.g24.runtime-semantic-reference-owner-map.r60.v1' }
r54.authority_runtime_semantic_dependency_owner_map = { ...r54.authority_runtime_semantic_dependency_owner_map, schema_version: 'ctrl.g24.runtime-semantic-dependency-owner-map.r60.v1' }
r54.authority_runtime_semantic_manifest = { ...r54.authority_runtime_semantic_manifest, schema_version: 'ctrl.g24.runtime-semantic-authority-manifest.r60.v1' }
...
r54.authority_runtime_semantic_manifest_hash_contract = { ...r54.authority_runtime_semantic_manifest_hash_contract, schema_version: 'ctrl.g24.runtime-semantic-manifest-hash-contract.r60.v1', manifest_hash_version: 'ctrl.g24.runtime-semantic-manifest-hash.r60.v1', content_domain_ascii: 'CTRL-G24-R60-MANIFEST-CONTENT', dependency_domain_ascii: 'CTRL-G24-R60-MANIFEST-DEPENDENCY', graph_domain_ascii: 'CTRL-G24-R60-MANIFEST-GRAPH', envelope_domain_ascii: 'CTRL-G24-R60-MANIFEST-ENVELOPE' }
...
const snapshotSha = hash({ domain_ascii: 'CTRL-G24-R60-FINAL-SEMANTIC-SOURCE-SNAPSHOT', exact_paths: sourcePaths, values: sourceSnapshot })
...
r54.authority_runtime_semantic_reference_owner_map = { schema_version: 'ctrl.g24.runtime-semantic-reference-owner-map.r60.v1', source_snapshot_sha256: snapshotSha, exact_row_count: refRows.length, reference_registry_ref: 'authority_runtime_semantic_reference_field_registry', schema_declared_and_exact_path_value_bijection: true }
r54.authority_runtime_semantic_dependency_owner_map = { schema_version: 'ctrl.g24.runtime-semantic-dependency-owner-map.r60.v1', source_snapshot_sha256: snapshotSha, exact_paths: sourcePaths, rows: sourcePaths.map(path => ({ authority_path: path, typed_owner_paths: dependencies[path] })), reference_registry_ref: 'authority_runtime_semantic_reference_field_registry' }
...
const withoutSeal = { schema_version: 'ctrl.g24.runtime-semantic-authority-manifest.r60.v1', source_snapshot_sha256: snapshotSha, exact_paths: sourcePaths, rows: manifestRows, exact_expected_count: manifestRows.length, manifest_graph_sha256: hash({ domain_ascii: hc.graph_domain_ascii, manifest_hash_version: hc.manifest_hash_version, manifest_rows: manifestRows }) }
...
export const materializedR60 = r54
export const materializedR60Output = `${JSON.stringify(finalSnapshot, null, 2)}\n`
export const r60SemanticAuthorityPaths = sourcePaths
const invoked = process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)
if (invoked) {
  const mode = process.argv[2] ?? '--check'
  if (mode === '--write') { writeFileSync(join(root, outputPath), materializedR60Output); console.log(`wrote ${outputPath}`) }
  else if (mode === '--check') { if (readFileSync(join(root, outputPath), 'utf8') !== materializedR60Output) { console.error(`${outputPath} differs from materializer`); process.exit(1) } console.log(`ok: ${outputPath} is the exact fully materialized R60 effective contract`) }
