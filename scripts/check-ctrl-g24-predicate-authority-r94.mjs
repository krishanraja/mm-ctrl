...
const directory = 'project-documentation/ctrl-evolution/g24-predicate-authority-r94'
...
const humanLabel = human => human === 'krish' ? 'Krish' : human === 'leader-1' ? 'the leader' : 'the answer owner'
function renderReplacementMessage(obligations) {
  const total = obligations.reduce((sum, obligation) => sum + obligation.required_replacements, 0)
  ...
  if (obligations.length === 1) {
    ...
    return `${opening} ${humanLabel(obligation.named_human_id)} must choose ${object}, then review all changes with Krish.`
  }
...
function validateCurrentStatusProjection(value) {
  ...
  if (!Number.isInteger(value.required_replacements) || value.required_replacements < 1 || !Array.isArray(value.responsible_humans) || !value.responsible_humans.length || !unique(value.responsible_humans) || !same(value.responsible_humans, byteSorted(value.responsible_humans)) || !Array.isArray(value.replacement_obligations) || value.replacement_obligations.length !== value.responsible_humans.length) return false
  if (!value.replacement_obligations.every(obligation => exactKeys(obligation, correction.replacement_obligation_required) && typeof obligation.named_human_id === 'string' && obligation.named_human_id.length > 0 && Number.isInteger(obligation.required_replacements) && obligation.required_replacements > 0)) return false
  const obligationHumans = value.replacement_obligations.map(obligation => obligation.named_human_id)
  return unique(obligationHumans) && same(obligationHumans, byteSorted(obligationHumans)) && same(obligationHumans, value.responsible_humans) && value.replacement_obligations.reduce((sum, obligation) => sum + obligation.required_replacements, 0) === value.required_replacements && value.message === renderReplacementMessage(value.replacement_obligations)
}

const expectedTopKeys = {
  effective_resolution: ['schema_version', 'module_id', 'semantic_base', 'imported_exact_sections', 'r94_replacement_sections', 'inheritance_rule', 'unknown_or_conflicting_section', 'authority_still_closed'],
...
function buildFinalReceipt(transitionId, predicateProof) {
  ...
  const authorityProofs = humans.map(human => ({ named_human_id: human, authentication_event_ref: human === 'krish' ? 'auth-r94-krish' : 'auth-r94-leader', authority_event_ref: `final-authority-${transitionId}-${human}` }))
...
if (process.env.R94_EMIT_FINAL_UNSIGNED === '1') {
  ...
  console.log(`R94_UNSIGNED_FINALS=${JSON.stringify(emitted)}`)
  process.exit(0)
}
...
if (process.env.R94_EMIT_MULTI_CHANGE_UNSIGNED === '1') {
  sessionAuthorityService.resetForConformance(); answerAuthorityService.resetForConformance(); acceptedFinalService.resetForConformance()
  const built = buildPredicateProof('accept_intensive_proof'), final = buildFinalReceipt('accept_intensive_proof', built.proof)
  ...
  console.log(`R94_UNSIGNED_MULTI_CHANGE=${JSON.stringify(unsigned)}`)
  process.exit(0)
}
if (process.env.R94_EMIT_MIXED_CHANGE_UNSIGNED === '1') {
  sessionAuthorityService.resetForConformance(); answerAuthorityService.resetForConformance(); acceptedFinalService.resetForConformance()
  const built = buildPredicateProof('accept_intensive_proof'), final = buildFinalReceipt('accept_intensive_proof', built.proof)
  ...
  console.log(`R94_UNSIGNED_MIXED_CHANGE=${JSON.stringify(unsigned)}`)
  process.exit(0)
}
if (process.env.R94_EMIT_DOUBLE_WITHDRAW_UNSIGNED === '1') {
  sessionAuthorityService.resetForConformance(); answerAuthorityService.resetForConformance(); acceptedFinalService.resetForConformance()
  const built = buildPredicateProof('accept_intensive_proof'), final = buildFinalReceipt('accept_intensive_proof', built.proof)
  ...
  console.log(`R94_UNSIGNED_DOUBLE_WITHDRAW=${JSON.stringify(unsigned)}`)
  process.exit(0)
}
...
}
{
  ...
  check('replacement projection accepts one exact discriminated schema', validateCurrentStatusProjection(valid))
  ...
  const exercise = (transitionId, reverseWithdrawal, reverseReanswer) => {
    ...
    for (let position = 0; position < reanswerOrder.length; position += 1) {
      ...
      if (remaining.length) {
        ...
        if (!validateCurrentStatusProjection(status) || status.status !== 'replacement_required' || status.required_replacements !== remaining.length || !same(status.replacement_obligations, obligations) || acceptedFinalService.unsignedRevalidationEvents(acceptedId).length !== 0) return false
      } else {
...
if (failures.length) {
  console.error(`R94 independent conformance failed (${failures.length})`)
  for (const failure of failures) console.error(`- ${failure}`)
  process.exit(1)
}
console.log(`ok: R94 end-to-end bound contract; ${imports.transition_catalogue.length}/13 predicate and final transition paths; ${vectorGroups.reduce((sum, group) => sum + vectors[group].length, 0)} executable vectors; ${totalityProbeCount} generated totality probes`)
