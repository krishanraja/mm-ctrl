...
as_of: 2026-09-11
head: 618bf91
head_scope: G17 Brain adapter primitives, G18 synthetic population, G19 synthetic range lab, G20 Claude bridge contract
lifecycle: live
...
- **G17 to G20 are contract, primitive and preview work, not a production change.** `brain-crypto.ts` and `brain-ingest-core.ts` exist with 13 passing tests but no runtime caller; the isolated Supabase branch needed to prove the atomic write is founder-approved but not created. The 48-account G18 synthetic population and its G19 range lab are merged to `main` behind the existing synthetic-preview flag and gate, unlinked and non-indexable; nothing has been seeded into a database. G20 is a locked contract for capture and a Claude bridge with no connector, write path or UI built yet (`project-documentation/ctrl-evolution/README.md`).
...
- 2026-09-08 **G20 locks universal capture and the Claude bridge as contract only.** Krish confirmed frictionless paste-in capture and working inside Claude as first-class requirements: "the Claude UI is often where I do things." The contract locks two gestures, private-staging capture with a compact receipt, and a purpose-bound, expiring, revocable read-only Claude context capsule served through a private remote MCP connector, with a copied task starter as the honest fallback because no supported interface can prefill Claude's web UI. No connector, write path or UI is authorised yet (PR #392; `project-documentation/ctrl-evolution/g20-universal-capture-claude-bridge-contract.md`).
- 2026-09-08 **G19 synthetic range lab approved as Krish's own dashboard, not the product.** An unlinked, preview-gated lab renders all 48 G18 synthetic accounts so source routing, diagnostic boundaries and hostile content stay legible across desktop and mobile. QA found the desktop page measuring 2,640 pixels against a 900-pixel viewport before the fix. Krish's approval was scoped exactly to "a dashboard for all my customers, for just me to use" (PRs #389, #391; `project-documentation/ctrl-evolution/design/g19-synthetic-population-lab-qa-record.md`).
- 2026-09-08 **G18 built 48 synthetic leaders as failure oracles, not demo copy.** Every account is marked `synthetic_demo` on a reserved `.invalid` domain and records what a diagnostic must notice, must not infer, and its smallest defensible next move, covering multilingual text, prompt injection, corrupt uploads and attempted audience widening; deterministic expansion produces 1,672 input events without inventing more people. Nothing has been seeded into any database (PR #388; `project-documentation/ctrl-evolution/g18-synthetic-population-lab.md`).
- 2026-09-08 **G17 rejects the legacy memory cipher for new Brain data.** The old `memory-crypto.ts` pads text into a key and keeps a published development-key fallback; the new `brain-crypto.ts` requires an exact 32-byte key, AES-256-GCM with context-bound associated data, and hard failure on a missing key or tampering, with 13 focused tests passing. No runtime calls it: the isolated Supabase branch needed to prove the atomic write is founder-approved at $0.01344 an hour but has not been created, because the working session lacks the GitHub `workflow` scope to publish the branch-creation workflow (PR #385; `project-documentation/ctrl-evolution/g17-service-adapter-contract.md`).
- 2026-09-08 **Three Answers pages published**, arguing that task-automation metrics do not measure decision quality, that a compliance checklist does not prove judgement fit, and that AI news noise is a standards problem rather than a curation problem (`src/content/answers/`).
...
- Waiting on Krish: creation of the isolated, data-less Supabase development branch the G17 adapter proof needs, quoted at $0.01344 an hour; blocked separately on the working session getting the GitHub `workflow` scope required to publish the branch-creation workflow.
