...
as_of: 2026-09-23
head: 83ce7c48
head_scope: kept-gather migration merged to main; G16 remains the exact production release
lifecycle: live
...
## Where it is right now (as of 2026-09-23)

- **Live** at `makeyourmindup.ai`. The exact G16 application release remains `860dea0`, Vercel `dpl_2JFRfmZRzUvxbdwXqLunG3eubTgc`, READY and PROMOTED from that SHA (`docs/current/release-state.md`). No later commit has produced a new production deployment receipt.
- **Edge Functions:** 115 directories in the tree. 114 confirmed deployed and ACTIVE by management API readback on 2026-08-21. `live-headlines` version 48 deployed and verified against cache readback on 2026-09-02 (476 items, 473 classified, 12 `damage` dropped). Three changes have no deployment readback recorded here: `video-radar-export` (PR #371, 2026-08-28), the rolling-window merge (PR #375, `edd9045`), and the 2026-09-22 gather-keeping change to `live-headlines` and `cleanup-expired-data` (`a6ce832a`).
- **Tests:** production `main` passes 945 tests in 60 files, zero new type errors against the 94-error legacy baseline; the 2026-09-22 merge separately reports 613 shared edge function tests passing, 19 of them new. CI runs docs, standards, tests, typecheck, build and changed-file lint on every push.
- **Living Brain substrate:** 11 additive production tables are live, empty and disconnected from customer paths, protected by forced RLS, non-anonymous workspace membership, exact-audience grants and authenticated read-only ACLs. G17 through G20 add governing contracts, not runtime: strict row-bound encryption for a future write adapter (G17), a 48-identity synthetic test population as diagnostic oracles (G18), a founder-approved internal range lab that renders them (G19), and a universal capture and Claude-bridge product contract (G20). None connects to a customer path or writes real data (`project-documentation/ctrl-evolution/README.md`).
- **The gather is now kept, not only served.** Three append-only tables (`live_headlines_gather`, `live_headlines_cache_versions`, `model_benchmark_snapshots`) and a per-source run log were merged to `main` on 2026-09-22 (`a6ce832a`) and verified against a local Postgres 16. No migration has yet run against the shared production project, so no gather history exists there yet; the served feed itself is unchanged.
...
- **Waiting on evidence, not code:** deployment of `video-radar-export`, of the PR #375 change, and application of the 2026-09-22 gather-keeping migration to the shared project.

## What changed recently

- 2026-09-22 **The gather is kept, not only the feed.** `live-headlines` fetches several hundred AI stories a day and serves twenty; until now the rejected majority never existed as data, so volume, share of voice, publisher lead and lag, and any audit of the product's own filters were unanswerable by construction. Three append-only record tables now hold every article and its verdict, every cache version, and a daily model-benchmark snapshot; `cleanup-expired-data`, which had deleted nothing since it was written because it targeted a table no migration creates, now targets the real one behind a flag. Merged to `main` (`a6ce832a`); 613 shared edge function tests pass including 19 new ones; no migration has been applied to the shared project (`CHANGELOG.md`).
- 2026-09-08 **G20 confirms capture and a Claude bridge as first-class.** Krish: "We need to make this really easy for me to work in and in Claude... the ability to paste stuff in really quickly and easily, and the ability to prompt stuff out to Claude, would be good because the Claude UI is often where I do things." The governing contract locks two gestures, `Add to Brain` and `Use in Claude`, on the existing context-circulation substrate. No connector, write path or UI is authorised yet (`project-documentation/ctrl-evolution/g20-universal-capture-claude-bridge-contract.md`).
- 2026-09-08 **G17 through G19 build the adapter, the test population and the lab that renders it.** The G17 service-adapter contract freezes strict row-bound AES-GCM encryption; one isolated Supabase development branch is founder-approved but not created because the coding session's GitHub token lacks the separately-approved `workflow` scope. G18 defines 48 fictional leaders and 1,672 deterministic events as test oracles, not demo claims. G19 renders them: Krish approved the 48-account range lab on 8 September 2026 as "his internal dashboard across all customers," subject to a scrollbar fix, after 61 deterministic and React checks plus 8 browser checks against a protected preview (`project-documentation/ctrl-evolution/g17-service-adapter-contract.md`, `g18-synthetic-population-lab.md`).
...
- Next engineering gate: the G17 service-adapter branch, blocked on the coding session receiving separately-approved GitHub `workflow` scope. Once created, apply the G16 migrations, run the committed multi-identity behavioural suite on a writable connection, then seed the G18 synthetic population before any approved Brain UI consumes real substrate data.
- Waiting on Krish: a deployment readback for `video-radar-export`, the PR #375 change, and application of the 2026-09-22 gather-keeping migration (`a6ce832a`) to the shared production project, then a line in `docs/current/release-state.md`.
