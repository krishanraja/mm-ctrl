...
as_of: 2026-09-25
head: 83ce7c48
head_scope: G16 application release receipt unchanged; adds G17-G20 evolution contracts and the merged-not-applied gather-capture migration
lifecycle: live
...
- **Merged but not applied:** four append-only capture tables (`live_headlines_gather`, `live_headlines_cache_versions`, `model_benchmark_snapshots`, `live_headlines_gather_runs`) and a `cleanup-expired-data` fix are merged to `main` at `83ce7c48`. The migration has not reached the shared production database and no Edge Function deployment readback is recorded here; the served feed is unchanged (`CHANGELOG.md`, 2026-09-22).
- **Edge Functions:** 115 directories in the tree. 114 confirmed deployed and ACTIVE by management API readback on 2026-08-21. `live-headlines` version 48 deployed and verified against cache readback on 2026-09-02 (476 items, 473 classified, 12 `damage` dropped). Three changes have no deployment readback recorded here: `video-radar-export` (PR #371, 2026-08-28), the rolling-window merge (PR #375, `edd9045`), and the 2026-09-22 gather-capture change above.
- **Tests:** production `main` passes 945 tests in 60 files, with zero new type errors against the 94-error legacy baseline. The 2026-09-22 change adds 613 shared edge function tests including 19 new ones, verified locally, not yet folded into the `main` production count recorded here. CI runs docs, standards, tests, typecheck, build and changed-file lint on every push.
- **Living Brain substrate:** 11 additive production tables are live, empty and disconnected from customer paths. Forced RLS, non-anonymous workspace membership, exact-audience grants and authenticated read-only ACLs protect them. G17 added the encryption and ingest-core primitives for a service-side write adapter (PR #385), still a provisional contract with no database branch, function, key, workspace or row created. The writable multi-identity behavioural test remains future work (`project-documentation/ctrl-evolution/g16-workspace-audience-canary.md`, `g17-service-adapter-contract.md`).
- **Synthetic Brain lab:** an internal, noindexed range harness at `/operator/lab/synthetic-population/:accountId` accepts only 48 deterministic G18 synthetic identities; database seeding and diagnostic model evaluation are pending (`docs/current/features.md`, `project-documentation/ctrl-evolution/g18-synthetic-population-lab.md`).
...
- 2026-09-22 **The gather is kept, not only the feed** (commit `a6ce832a`, merged `83ce7c48`). Why: `live-headlines` fetches several hundred AI stories a day and serves twenty; every filter in between was right for the feed, but the feed was the only output, so the rejected majority never existed as data and volume, share of voice, publisher lag and any audit of the filters were unanswerable by construction. Four new append-only tables (enforced by trigger, not convention) record every article with its verdict and an absolute `published_at`, every version of a cached day, the Artificial Analysis benchmark board daily, and per-source gather counts that show the shared pool went dark for about four weeks to 2026-08-05 with nothing reporting it. The same pass found `cleanup-expired-data` had deleted nothing since it was written because it targeted `ai_cache`, a table no migration creates; the real table is `ai_response_cache`, and the delete stays off behind `CLEANUP_PRUNE_AI_CACHE` pending a person's decision. Merged to `main`; not applied to the shared production database, no deployment readback recorded (`CHANGELOG.md`).
- 2026-09-08 **G20 closes the capture-and-Claude-bridge question in the founder's own words** (PR #392). Krish: "We need to make this really easy for me to work in and in Claude, really quick and easy... the Claude UI is often where I do things." The contract rules out CTRL replacing Claude: CTRL keeps the governed Brain, Claude stays a preferred thinking surface, and material moves through private staging and scoped, time-bounded context capsules rather than a second canonical Brain. No connector, write path or UI implementation is authorised by this record (`project-documentation/ctrl-evolution/g20-universal-capture-claude-bridge-contract.md`).
- 2026-09-08 **G17 Brain adapter primitives, and the synthetic population built to prove them before anything real does** (PRs #385, #388, #389, #391). G17 adds the encryption and ingest-core primitives for the service-side write adapter the Brain substrate has been waiting on since G16 landed, still a provisional contract with no database branch, function, key, workspace or row created. Alongside it, a 48-identity deterministic synthetic corpus and an internal, `noindex` range lab (gated behind the same synthetic-preview flag as Decision Bench) test the Brain and its diagnostics before any real account reaches them (`project-documentation/ctrl-evolution/g17-service-adapter-contract.md`, `g18-synthetic-population-lab.md`).
...
- Next engineering gate: one designated synthetic Brain workspace, the G17 encryption and ingest-core primitives wired into a connected server-side write adapter (still no database branch, function, key, workspace or row created), the committed multi-identity database suite on a writable non-customer connection, and a read-only projection before any approved Brain UI consumes real substrate data.
- Waiting on Krish: applying the 2026-09-22 gather-capture migration (`20260922100000_ctrl_keeps_what_it_gathered.sql`) to the shared production database, and a deployment readback for the `live-headlines` and `cleanup-expired-data` changes that write to it.
